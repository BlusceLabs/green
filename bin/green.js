...
function greenBinaryName(platform = process.platform) {
  return platform === 'win32' ? 'green.exe' : 'green';
}
...
const nativePath = join(packageRoot, greenBinaryName());
...
if (!existsSync(nativePath)) {
  ...
  console.error(
    '[green] No native binary found next to the npm wrapper.\n' +
      ...
      (ranByBun
        ? 'You installed with Bun, which does not run dependency lifecycle scripts\n' +
          ...
          '\n'
        : '') +
      'If that fails, build from source: https://github.com/BlusceLabs/green\n' +
      '(go run ./cmd/green, requires Go 1.25+).',
  );
  process.exit(1);
}
...
if (localControlHelpers) {
  env.green_LOCAL_CONTROL_HELPERS = localControlHelpers;
} else {
  delete env.green_LOCAL_CONTROL_HELPERS;
}
...
if (child.error) {
  console.error(`[green] Failed to launch wrapper target: ${child.error.message}`);
  process.exit(1);
}
