---
description: Extending green — how to write AGENTS.md files, custom specialists, skills, hooks, MCP servers, and plugins for the open-source CLI coding agent.
...
green is an open-source terminal coding agent. Out of the box it does the obvious things — read, edit, run, search — but the design point of the project is that **every surface is configurable**. This document is the user-facing guide for that configuration.

If you only want to *use* green, the [README](README.md) is enough. This page is for the other three jobs:

1. Tell the agent about *your* project (drop an `AGENTS.md` in your repo).
2. Add new specialist sub-agents.
3. Wire green into the rest of your toolchain (MCP, skills, hooks, plugins).

## 1. Drop a project `AGENTS.md`

When green starts in a directory, it looks for project-level instructions and injects them into the system prompt. The lookup walks from your current working directory **up to the nearest git root** and reads the first matching file at each level — general rules at the repo root, more specific rules in sub-trees. Files are labeled with their directory in the prompt (e.g. `## Project guidelines (services/api/AGENTS.md)`).
...
| `./green.md` | Brand-specific alias. Same format, lower priority. |
| `./.green/AGENTS.md` | Project-local, hidden, gitignored. Personal notes that stay out of git. |
...
Both files use the same format. YAML frontmatter is optional; the markdown body is loaded as instructions for the agent. green reads the file once at session start, so changes take effect on the next `green` launch — not mid-session.
...
- Keep each file under ~8 KiB. green caps the **total** across all matched files at 32 KiB; everything past the cap is dropped.
...
- In a monorepo, drop a narrower `AGENTS.md` in each sub-tree (e.g. `services/api/AGENTS.md`). green picks those up automatically when you launch from inside the sub-tree.
...
For preferences that follow *you*, not a specific repo (tone, tooling habits, workflow), drop a `green.md` in your user config directory: `~/.config/green/green.md` on Linux/macOS, `%AppData%\Roaming\green\green.md` on Windows — the same directory as `config.json` and your personal specialists. Same format and 8 KiB cap as the project files above, and the same case-insensitive basename match.

This file is injected as its own `## User guidelines` section, before the project's `AGENTS.md`/`green.md`, and is labeled as personal preference in the prompt: project guidelines are the later, more specific instruction and take precedence over it when the two conflict.

## 2. Custom specialists

Specialists are green's sub-agents. Three scopes, in priority order:

| Scope | Path | Shared? |
| --- | --- | --- |
| Built-in | compiled into green | yes — `worker`, `explorer`, `code-review` |
| User | `~/.config/green/specialists/*.md` | no — your machine only |
| Project | `./.green/specialists/*.md` | yes — the repo team |
...
```bash
green specialist list
green specialist show api-reviewer
green specialist create api-reviewer \
    --project \
    --description "Reviews API changes" \
    --tools read-only,plan \
    --prompt "$(cat api-reviewer.md)"
green specialist edit api-reviewer --project
green specialist delete api-reviewer --project
green specialist path                       # prints the resolved specialists directory
```

The full format spec (frontmatter fields, tool scopes, prompt conventions) is in [`docs/SPECIALISTS.md`](docs/SPECIALISTS.md).

> **Roadmap.** An in-UI specialist manager (create / edit / delete / preview) is on the backlog. Today you use the `green specialist` CLI subcommands above.
...
Discovery root: `$green_SKILLS_DIR` → `$XDG_DATA_HOME/green/skills` → `~/.local/share/green/skills/`. A missing directory is fine — green just reports "no skills".

```
~/.local/share/green/skills/
  run-benchmarks/
    SKILL.md
  write-changelog/
    SKILL.md
...
Only `name` and `description` are recognized in the frontmatter today. The `name` defaults to the directory name. Within a single skills root, duplicate names are resolved by lexicographic directory order. During an agent run, green loads the default skills directory before plugin skill roots; earlier roots win name collisions silently. Plugin-declared skills (section 6) are merged into the active agent run at plugin activation time, so bundled skills appear in the available skills list and can be loaded with the `skill` tool.
...
- User: `~/.config/green/hooks.json`
- Project: `./.green/hooks.json`
  ...
  "hooks": [
    {
      ...
      "event": "beforeTool",
      "matcher": "bash",
      "command": "/usr/local/bin/green-hook-block-rmrf.sh",
      "enabled": true
    },
    {
      "id": "log-session",
      "event": "sessionStart",
      "command": "/usr/local/bin/green-hook-log.sh",
      "enabled": true
    }
  ]
}
...
```bash
#!/usr/bin/env bash
# /usr/local/bin/green-hook-block-rmrf.sh
set -euo pipefail
payload="$(cat)"
...
A hook's exit code decides what happens next: `0` continues, non-green blocks the tool call (`beforeTool`) or surfaces an error (`afterTool`). Hook execution is recorded in the audit log; the audit is reachable from the agent's view of past actions, not from a dedicated `green doctor` check.

> **Roadmap.** An in-UI hooks manager is on the backlog. Today you edit the JSON directly.

## 5. MCP — Model Context Protocol

green is both an **MCP client** (it can call external MCP servers) and an **MCP server** (other agents can call its tools).
...
```bash
green mcp add docs --type stdio -- docs-mcp --port 7777
green mcp add github --type http --url https://api.example.com/mcp \
    --header "Authorization=Bearer YOUR_TOKEN_HERE"
green mcp list
green mcp check docs
green mcp remove github
green mcp oauth login github
```
...
so changing a server target can require `green mcp oauth login <server>` again.
OAuth server names ending in `.<32 hex chars>` are reserved for token storage.

### As a server — expose green's tools to another agent

```bash
green serve --mcp
```

The server speaks MCP over stdio. Configure it from the receiving side as a `stdio` server whose command is `green serve --mcp`.

## 6. Plugins

A plugin is a self-contained directory that bundles tools, hooks, and skills for one capability. Plugins live at:

- User: `~/.config/green/plugins/<id>/`
- Project: `./.green/plugins/<id>/`
...
```bash
green plugins add ./github-pr-review      # copy into ~/.config/green/plugins/ or ./.green/plugins/
green plugins list
green plugins remove github-pr-review    # alias: rm
```
...
> **Roadmap.** An in-UI plugins manager (browse, install, enable / disable) is on the backlog. Today you use the `green plugins` CLI subcommands above.
...
| User config | `~/.config/green/config.json` | Your machine. Never committed. |
| Project config | `./.green/config.json` | The repo. Committed (or not, your call). |
| CLI flags | `--model`, `--mode`, ... | Highest priority, per-invocation. |
| Environment | `green_*` | Provider commands, secrets, skills dir override. |
...
A team that wants every contributor's green to behave the same way commits:

- `AGENTS.md` — project conventions, build commands, do-not-edit lists.
- `.green/config.json` — provider catalog, default model, allowed tools.
- `.green/specialists/api-reviewer.md` — the team's PR-review specialist.
- `.green/hooks.json` — block `rm -rf` and `git push --force` on `beforeTool`.
- `.green/plugins/internal-tooling/` — a plugin that adds the team's internal CLI tools to the agent's toolset.

Each contributor adds only:

- `~/.config/green/config.json` — their personal API keys, theme, default mode.
- `~/.config/green/green.md` — personal preferences that follow them across every project (see section 1).
- `~/.local/share/green/skills/` — personal skills they keep across projects.

That's it. Run `green` from the repo root and the agent has the team's full instruction set, every contributor's personal setup, and nothing else.
...
- [docs/STREAM_JSON_PROTOCOL.md](docs/STREAM_JSON_PROTOCOL.md) — `green exec` I/O contract.
