# green Stream-JSON Protocol

green stream-json is a line-delimited protocol for headless clients such as
...
`green exec --input-format stream-json` accepts these JSONL events from stdin or
`--file`.
...
green combines accepted input event content in order, separated by blank lines.
...
`green exec --output-format stream-json` emits schema-versioned JSONL events.

```json
{ "schemaVersion": 2, "type": "run_start", "runId": "run_20260603_abc123", "sessionId": "green_20260603100000_abc123", "cwd": "/repo", "provider": "openai", "model": "gpt-4.1", "apiModel": "gpt-4.1" }
...
is not pre-approved, green may emit `permission_request` followed by a denied
