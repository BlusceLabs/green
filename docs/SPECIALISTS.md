...
Specialists are named sub-agents that green can delegate focused work to through
...
| Scope | Path | Notes |
| --- | --- | --- |
| Built-in | compiled into green | `worker`, `explorer`, and `code-review` ship with the binary. |
| User | `~/.config/green/specialists/*.md` | Available across local workspaces. |
| Project | `.green/specialists/*.md` | Shared with the current repository when committed. |
...
green specialist show worker
green specialist path

green specialist create api-review \
  --project \
  --description "Reviews API changes" \
  --tools read-only,plan \
  --prompt "Review API changes for compatibility and missing tests."

green specialist edit api-review --project
green specialist delete api-review --project
```
...
If the body is empty and `description` is set, green uses the description as the
system prompt and reports a warning in `green specialist show`.
...
green registers these tools for top-level agent runs:
...
`.green/specialists`, not the user specialist directory.
...
```text
${XDG_DATA_HOME:-~/.local/share}/green/background/
```
...
If green is restarted while a background task is still marked `running`, the new
