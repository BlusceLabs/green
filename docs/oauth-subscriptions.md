...
green supports two distinct things people mean by "log in with OAuth":

1. **OAuth login for a provider/gateway** that issues a *standard* bearer token —
   fully built in (`green auth login …`). green attaches the token to model calls
   automatically.
2. **Using a ChatGPT or Claude *subscription*** (Plus / Pro / Max) instead of a
   ...
   reasons documented below. green ships a convenience preset and this recipe.
...
`Authorization: Bearer …` on its API, configure it with `green_OAUTH_<NAME>_*`
env vars and log in:

```sh
export green_OAUTH_ACME_CLIENT_ID=…
export green_OAUTH_ACME_AUTHORIZE_URL=https://acme.example/oauth/authorize
export green_OAUTH_ACME_TOKEN_URL=https://acme.example/oauth/token
export green_OAUTH_ACME_SCOPES="openid profile"
green auth login acme            # browser (loopback); --device for headless
green auth status
```
...
`green_OAUTH_STORAGE=keyring`) and never logged. See `green auth --help`.
...
- **OpenRouter / xAI / ChatGPT / Hugging Face** are real OAuth: your browser
  ...
  `green_OAUTH_ALLOW_PRESETS=1` or your own `green_OAUTH_XAI_*`; see below.)
- **Device code (headless / SSH):** for a provider that supports it (xAI,
  ...
  (no `DISPLAY`) device code is used automatically; set `green_OAUTH_DEVICE=1`
  to force it anywhere. The CLI equivalent is
  `green auth login <name> --device`.
...
- **OpenRouter (no env needed)** — `green auth openrouter` opens a browser, you
...
- **xAI (Grok) — opt-in preset** — xAI's flow needs an OAuth `client_id`. green
  ...
  Enable it with `export green_OAUTH_ALLOW_PRESETS=1`, then `green auth login xai`
  (browser, or `--device` for headless) works one-click; the token is used directly
  on `api.x.ai/v1`. Without the opt-in, set `green_OAUTH_XAI_CLIENT_ID` (and
  endpoints, or an issuer) yourself via `green_OAUTH_XAI_*`. Either way the preset is
  fully overridable by `green_OAUTH_XAI_*` (env wins), and it requires a
  SuperGrok / X Premium+ subscription; the client_id is an undocumented public
  Grok-CLI client that may change without notice.
- **ChatGPT (Codex) — opt-in preset** — `green auth chatgpt` opens a browser, you
...
- **Hugging Face — opt-in preset, BYO client_id** — `green auth login huggingface`
  ...
  `client_id` via `green_OAUTH_HUGGINGFACE_CLIENT_ID`. Enable the preset with
  `green_OAUTH_ALLOW_PRESETS=1` (or omit it — the BYO client_id path uses
  `client_credentials = none` and doesn't need the opt-in). Free tier has strict
  rate limits; Pro removes them.

Any field of a preset is overridable via `green_OAUTH_<NAME>_*`. For a fully custom
OAuth/OIDC provider, set those env vars (see `green auth --help`) and
`green auth login <name>`.
...
- **OpenAI (ChatGPT):** a "Sign in with ChatGPT" token only works against
  ...
  **First-class path (this version):** `green auth chatgpt` does exactly that
...
So green does **not** call those backends directly or spoof those clients — that
...
`127.0.0.1`. The proxy absorbs the Cloudflare / client-spoofing surface; green
just points at it.
...
```sh
green exec --prompt "say hi"   # routes through the proxy → your ChatGPT plan
```
