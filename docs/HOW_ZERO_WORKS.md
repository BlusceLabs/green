...
This document explains green's architecture at a high level: the layers, module
boundaries, data flow, and runtime concepts behind the interactive TUI and the
headless `green exec` mode.
...
green is a Go CLI application with two main user-facing surfaces:

- **Interactive TUI** (`green`): a Bubble Tea application that renders chat,
  tool cards, permission prompts, model/provider pickers, and session controls.
- **Headless exec** (`green exec ...`): a scriptable command runner that uses the
...
```mermaid
flowchart TD
    User[User / automation] --> CLI[cmd/green -> internal/cli]
    ...
    Agent --> Provider[Provider runtime<br/>internal/greenruntime.Provider]
...
The easiest way to understand green is not as a set of independent packages, but
...
| Package | Role |
| --- | --- |
| `cmd/green` | Tiny binary entrypoint; calls `internal/cli.Run`. |
...
| `internal/greenruntime` | Provider-neutral message, tool-call, stream, usage, and image types. Providers implement `Provider.StreamCompletion`. |
...
| `internal/streamjson` | Machine-readable protocol for `green exec --input-format/--output-format stream-json`. |

## Startup Flow

### Interactive TUI (`green`)

```mermaid
sequenceDiagram
    participant Main as cmd/green
...
`green exec` uses the same `agent.Run` loop as the TUI, but without interactive
...
3. **Call provider** — sends a provider-neutral `greenruntime.CompletionRequest`.
...
The model never directly mutates the workspace. It asks for tool calls; green
...
it. The model proposes an action, green decides whether that action is allowed to
...
1. **Proposal** — the provider streams assistant content and may emit one or more
   tool calls. green can show the partial tool arguments in the TUI while they are
...
4. **Normalize result** — green turns the outcome into an agent `ToolResult`: a
...
TUI. green controls the loop mechanics: it keeps appending messages, enforces
...
The `Maybe compact context` step in the agent-loop diagram is green's context-window
...
- green estimates request size using message text, images, tool-call arguments, and
...
2. **Reactive compaction** runs after a provider returns a context-limit style
   error. green compacts once and retries that same turn; if the provider still
...
compaction, green keeps the original messages and tries to continue. It does not
...
A common source of confusion is that green has two event loops running at once:
...
- **Stream-JSON** is another surface over the same callbacks: `green exec`
...
Providers are hidden behind this interface in `internal/greenruntime`:
...
green has two related but separate checks:
...
The user-facing TUI and `green exec` mode both route permission requests through
...
Sessions live locally under green's data directory. A session directory contains:
...
remote service. green stores the local event history; providers only receive the
messages needed for the current completion request.

## Stream-JSON Protocol

`green exec --output-format stream-json` emits newline-delimited JSON events for
...
green's extension surfaces are loaded before the agent loop starts:
...
flowchart TD
    ...
    Prompt --> Guidelines[AGENTS.md / green.md]
...
The same request in `green exec` follows the same provider/tool/sandbox path, but
streaming events are written to stdout/stderr instead of rendered in Bubble Tea.

## Mental Model

When you work on green, start by locating the change on the four-layer spine from
