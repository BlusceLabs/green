...
green agent evals are maintainer fixtures for checking coding-agent behavior
...
`green eval` defaults to `validate` mode. In validate mode, the command performs
...
```bash
go run ./cmd/green eval --suite internal/agenteval/testdata/sample_suite.json
```

Use JSON output when another local tool needs the validation summary:

```bash
go run ./cmd/green eval --suite internal/agenteval/testdata/sample_suite.json --json
```

### Run Mode

`green eval run` scores one already-mutated Git workspace. It does not copy
...
go run ./cmd/green eval run \
  ...
  --task document-stream-json-verify-events \
  --workspace /tmp/green-eval-workspace \
  --report-dir /tmp/green-eval-report \
  --json
```

### Bench Mode

`green eval bench` runs the full benchmark harness for one task or a suite. Bench
...
```bash
go run ./cmd/green eval bench \
  --suite internal/agenteval/testdata/sample_suite.json \
  --task document-stream-json-verify-events \
  --work-root /tmp/green-evals \
  --agent-command green exec --cwd {workspace} {prompt}
```
...
go run ./cmd/green eval bench \
  ...
  --work-root /tmp/green-evals \
  --model gpt-5 \
  --agent-command green exec --model {model} --cwd {workspace} {prompt}
```
...
go run ./cmd/green eval bench \
  --suite internal/agenteval/testdata/sample_suite.json \
  --work-root /tmp/green-evals \
  --agent-command green-agent-wrapper --task {task_id} --workspace {workspace} --prompt {prompt}
```
...
Scored reports use contract `green.agenteval.report.v1`.
