# green Task Benchmark

This is the **task** benchmark: how often green completes a real coding task
...
green actually contributes: the agent noticing its own broken edit and fixing it
...
| `commit`         | green commit the run was built from                            |
...
## Integration point: headless `green exec`

The harness drives green through its headless surface — the same path CI uses:

```bash
green exec --output-format stream-json --model <model> [--self-correct] "<task prompt>"
```
...
[`cmd/green-perf-bench/testdata/terminal-bench-sample.json`](../cmd/green-perf-bench/testdata/terminal-bench-sample.json):
...
SUITE=cmd/green-perf-bench/testdata/terminal-bench-sample.json
MODEL=<your-model>

# baseline: self-correct OFF
go run ./cmd/green-perf-bench tasks \
  --suite "$SUITE" --binary ./green --model "$MODEL" \
  --version "$VERSION" --commit "$COMMIT" \
  --output dist/bench/tasks-baseline.json

# self-correct ON (auto-fix needs --auto medium or high; see note below)
go run ./cmd/green-perf-bench tasks \
  --suite "$SUITE" --binary ./green --model "$MODEL" --self-correct \
  --version "$VERSION" --commit "$COMMIT" \
  --output dist/bench/tasks-selfcorrect.json
```

`--version`/`--commit` also read from `green_BENCH_VERSION` / `green_BENCH_COMMIT`
