...
export GOCACHE="${GOCACHE:-/tmp/green-go-cache}"
...
for goos in linux darwin windows; do
  ...
  compile_pkg "$goos" ./cmd/green
done
compile_pkg linux ./cmd/green-linux-sandbox
compile_pkg linux ./cmd/green-seccomp
build_cmd windows ./cmd/green-windows-command-runner green-windows-command-runner.exe
build_cmd windows ./cmd/green-windows-sandbox-setup green-windows-sandbox-setup.exe

case "$(go env GOOS)" in
  linux)
    green_SANDBOX_REAL_SMOKE=1 go test ./internal/sandbox -run 'TestLinuxHelperRealSandboxSmoke|TestLinuxLandlockRealSandboxSmoke' -count=1
    ;;
  darwin)
    go test ./internal/sandbox -run TestSandboxExecProfileAllowsDevNullAndTemp -count=1
    ;;
  windows)
    green_SANDBOX_REAL_SMOKE=1 \
      green_WINDOWS_COMMAND_RUNNER_EXE="$tmpdir/windows-green-windows-command-runner.exe" \
      green_WINDOWS_SANDBOX_SETUP_EXE="$tmpdir/windows-green-windows-sandbox-setup.exe" \
