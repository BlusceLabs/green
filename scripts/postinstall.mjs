#!/usr/bin/env node
// postinstall: fetch the prebuilt `green` binary for this platform from the
// matching GitHub Release and place it next to bin/green.js, which execs it.
//
...
//   green-v{version}-{linux|macos|windows}-{x64|arm64}.{tar.gz|zip}  (+ .sha256)
//
...
//   green_SKIP_DOWNLOAD=1      skip entirely, exit 0 (wrapper will guide if run)
//   green_INSTALL_DRY_RUN=1    print the resolved plan as JSON, no network, exit 0
//   green_INSTALL_PLATFORM=…   override process.platform (linux|darwin|win32|android)
//   green_INSTALL_ARCH=…       override process.arch (x64|arm64)
//   green_REPO=owner/name      override the GitHub repo (default BlusceLabs/green)
//   green_GITHUB_BASE_URL=…    override the download host (default https://github.com)
...
const REPO = process.env.green_REPO || 'BlusceLabs/green';
const BASE = (process.env.green_GITHUB_BASE_URL || 'https://github.com').replace(/\/+$/, '');
...
const ALLOW_INSECURE = process.env.green_ALLOW_INSECURE_DOWNLOAD === '1';
...
function warnSkip(message) {
  ...
  // bin/green.js reports a clear "no native binary" message if the user runs green.
  console.error(`[green] ${message}`);
  process.exit(0);
}
...
if (process.env.green_SKIP_DOWNLOAD === '1') {
  warnSkip('green_SKIP_DOWNLOAD=1 set — skipping native binary download.');
}

const rawPlatform = process.env.green_INSTALL_PLATFORM || process.platform;
const rawArch = process.env.green_INSTALL_ARCH || process.arch;
...
if (!platform || !arch) {
  warnSkip(
    `no prebuilt binary for ${rawPlatform}/${rawArch}. Build from source: ` +
      `https://github.com/${REPO} (go run ./cmd/green).`,
  );
}
...
const binaryName = platform === 'windows' ? 'green.exe' : 'green';
const tag = `v${VERSION}`;
const assetName = `green-v${VERSION}-${platform}-${arch}.${ext}`;
...
const optionalBinaries =
  platform === 'windows'
    ? ['green-windows-command-runner.exe', 'green-windows-sandbox-setup.exe']
    : platform === 'linux'
      ? ['green-linux-sandbox', 'green-seccomp']
      : [];

if (process.env.green_INSTALL_DRY_RUN === '1') {
...
const markerPath = join(packageRoot, '.green-binary-version');
  ...
  if (!ALLOW_INSECURE && !url.startsWith('https://')) {
    fail(
      `refusing insecure download origin for ${url}: only https:// is allowed ` +
        `(set green_ALLOW_INSECURE_DOWNLOAD=1 to override for local testing).`,
    );
  }
  let response;
  try {
  ...
  if (!response.ok) {
    fail(
      `download failed (HTTP ${response.status}) for ${url} (release tag ${tag}). ` +
        ...
        `build from source: https://github.com/${REPO} (go run ./cmd/green).`,
    );
  }
...
async function main() {
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'green-install-'));
  try {
    ...
    for (const name of optionalBinaries) {
      ...
      if (!source) {
        console.error(
          `[green] note: optional helper ${name} was not in ${assetName}; ` +
    ...
    console.error(`[green] installed ${binaryName} ${VERSION} (${platform}-${arch}, ${sizeKb} KB).`);
  } finally {
