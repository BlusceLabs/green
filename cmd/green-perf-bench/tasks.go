...
// taskOptions configures the `green-perf-bench tasks` subcommand: the reproducible
// Terminal-Bench-style harness that runs green headlessly against a task set and
...
func runTasksCommand(args []string, getenv func(string) string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[green] Task benchmark failed: "+err.Error())
		return 1
	}
	...
	if !options.DryRun {
		...
		if binary == "" {
			_, _ = fmt.Fprintln(stderr, "[green] Task benchmark failed: --binary is required unless --dry-run is set")
			return 2
		}
...
func parseTaskArgs(args []string, getenv func(string) string) (taskOptions, error) {
	options := taskOptions{
		Version: strings.TrimSpace(getenv("green_BENCH_VERSION")),
		Commit:  strings.TrimSpace(getenv("green_BENCH_COMMIT")),
	}
...
func taskHelpText() string {
	return strings.Join([]string{
		"Usage: green-perf-bench tasks [options]",
		"",
		"Runs green headlessly against a Terminal-Bench-style task set and records a",
		...
		"  --binary <path>     Path to the `green` binary (required unless --dry-run)",
		"  --version <v>       Record the green version (default: $green_BENCH_VERSION)",
		"  --commit <sha>      Record the green commit (default: $green_BENCH_COMMIT)",
