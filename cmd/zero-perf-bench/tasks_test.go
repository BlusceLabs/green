...
func TestParseTaskArgsReadsFlagsAndEnv(t *testing.T) {
	env := func(key string) string {
		return map[string]string{
			"green_BENCH_COMMIT":  "deadbeef",
			"green_BENCH_VERSION": "9.9.9",
		}[key]
	}
	...
	if options.Binary != "/usr/local/bin/green" || options.Output != "dist/bench.json" {
...
func TestParseTaskArgsFlagsOverrideEnv(t *testing.T) {
	env := func(key string) string {
		return map[string]string{"green_BENCH_COMMIT": "fromenv"}[key]
	}
...
func TestRunTasksCommandWritesRecordWithoutBinary(t *testing.T) {
	...
	// needing a real green binary or model.
...
func TestRunTasksCommandRejectsMissingSuiteFile(t *testing.T) {
	...
	if code == 0 {
		t.Fatalf("expected non-green exit for missing suite file")
	}
	if stderr.Len() == 0 {
...
func TestTasksSubcommandRoutedFromMain(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "green-perf-bench tasks") {
