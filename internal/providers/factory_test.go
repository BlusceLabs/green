...
func TestNewCreatesOpenAIProviderWithFactoryOptions(t *testing.T) {
	...
	if transport.request.Header.Get("User-Agent") != "green-factory-test" {
...
func TestNewThreadsCustomProviderHeaders(t *testing.T) {
	...
	if transport.request.Header.Get("HTTP-Referer") != "https://green.dev" {
...
func TestPromptCacheKeyOnlyOnOfficialOpenAI(t *testing.T) {
	requestWithSession := greenruntime.CompletionRequest{
		Messages:       []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "hello"}},
...
func TestNewRoutesChatGPTCatalogToCodexProvider(t *testing.T) {
	...
	t.Setenv("green_OAUTH_TOKENS_PATH", t.TempDir()+"/tokens.json")
...
// returns a Store on it. Pinning green_OAUTH_STORAGE matters as much as the
// path: an inherited "keyring" value would send NewStore to the OS keychain
// and ignore green_OAUTH_TOKENS_PATH entirely, making the test read/write the
