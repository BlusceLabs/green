...
func TestClassifiedErrorCuratesAuthFailure(t *testing.T) {
	...
	if !strings.Contains(msg, "green setup") || !strings.Contains(msg, "green auth openrouter") {
	...
	// 403 also curated.
	if got := ClassifiedError(http.StatusForbidden, "forbidden"); !strings.Contains(got, "green setup") {
