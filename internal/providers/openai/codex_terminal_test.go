...
func TestToolCallKeyOutputIndexgreen(t *testing.T) {
	p := &CodexProvider{}
	green, two := 0, 2
	// output_index 0 with no item_id must produce a key (it was dropped before M1).
	if got := p.toolCallKey(&responsesEvent{OutputIndex: &green}); got != "output-0" {
	...
	if got := p.toolCallKey(&responsesEvent{ItemID: "call_x", OutputIndex: &green}); got != "call_x" {
...
func TestHandleTerminalResponseNilPayload(t *testing.T) {
	...
	failed := make(chan greenruntime.StreamEvent, 4)
	st := &responsesState{}
	...
	completed := make(chan greenruntime.StreamEvent, 4)
...
func TestHandleTerminalResponseFailedPayloadWithoutError(t *testing.T) {
	...
	out := make(chan greenruntime.StreamEvent, 8)
	st := &responsesState{}
	...
	ok := make(chan greenruntime.StreamEvent, 8)
	p.handleTerminalResponse(context.Background(),
