...
type responsesReasoning struct {
	...
	// phase produces green visible output and reads as a hang in the UI.
...
func (p *CodexProvider) buildResponsesRequest(request greenruntime.CompletionRequest) (*responsesRequest, error) {
...
func (p *CodexProvider) userInputItem(msg greenruntime.Message) inputItem {
	parts := []contentItem{}
	if msg.Content != "" {
...
func (p *CodexProvider) assistantInputItems(msg greenruntime.Message) ([]inputItem, error) {
	items := []inputItem{}
	if msg.Content != "" {
...
func (p *CodexProvider) streamResponses(
	...
	if !state.done {
		...
		providerio.SendEvent(ctx, events, greenruntime.StreamEvent{
			...
			FinishReason: greenruntime.FinishReasonLength,
		})
	}
}
...
// it into green or more runtime events. Returns false to stop the scan
...
func (p *CodexProvider) emitResponsesEvent(
	...
	case responsesEventOutputTextDelta:
		if event.Delta != "" {
			providerio.SendEvent(ctx, events, greenruntime.StreamEvent{
				Type:    greenruntime.StreamEventText,
				Content: event.Delta,
			})
		}
		return true
...
func (p *CodexProvider) handleTerminalResponse(
	...
	if event.Response == nil {
		...
		if event.Type == responsesEventFailed {
			...
			providerio.SendEvent(ctx, events, greenruntime.StreamEvent{Type: greenruntime.StreamEventDone})
		}
		state.done = true
		return false
	}
