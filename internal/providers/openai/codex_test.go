...
func drainCodexEvents(t *testing.T, stream <-chan greenruntime.StreamEvent) {
	t.Helper()
	for ev := range stream {
		if ev.Type == greenruntime.StreamEventError {
...
func TestCodexProviderStreamsReasoningSummaryDeltas(t *testing.T) {
	...
	// reasoning phase produces green visible output and reads as a hang.
	var rec codexRequest
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages:        []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "solve it"}},
		ReasoningEffort: "high",
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
	...
	for ev := range stream {
		switch ev.Type {
		case greenruntime.StreamEventReasoning:
			reasoning = append(reasoning, ev.Content)
		case greenruntime.StreamEventText:
			text = append(text, ev.Content)
		case greenruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
		}
	}
...
func TestCodexProviderParsesResponsesTextDeltas(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "say hi"}},
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
	}
	...
	for ev := range stream {
		...
		case greenruntime.StreamEventUsage:
			usage = &ev.Usage
		case greenruntime.StreamEventDone:
			gotDone = true
		case greenruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
		}
...
func TestCodexProviderParsesResponsesToolCalls(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "weather in SF?"}},
		Tools: []greenruntime.ToolDefinition{
	...
	for ev := range stream {
		switch ev.Type {
		case greenruntime.StreamEventToolCallStart:
			starts++
			startName = ev.ToolName
		case greenruntime.StreamEventToolCallDelta:
			deltas++
			fragments = append(fragments, ev.ArgumentsFragment)
		case greenruntime.StreamEventToolCallEnd:
			ends++
			endName = ev.ToolName
		case greenruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
...
func TestCodexProviderSendsAssistantToolCallsAsInputItems(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleUser, Content: "weather in SF?"},
			{
			...
			{Role: greenruntime.MessageRoleTool, ToolCallID: "call-1", Content: "72F and sunny"},
			{Role: greenruntime.MessageRoleUser, Content: "thanks"},
		},
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
...
func TestCodexProviderEmitsLengthFinishWhenStreamEndsWithoutCompletion(t *testing.T) {
	...
	var gotDone bool
	for ev := range stream {
		if ev.Type == greenruntime.StreamEventDone {
			gotDone = true
	...
	if finishReason != greenruntime.FinishReasonLength {
		t.Fatalf("finish reason = %q, want %q", finishReason, greenruntime.FinishReasonLength)
	}
}
