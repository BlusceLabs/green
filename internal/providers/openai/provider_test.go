...
func TestStreamCompletionPostsChatCompletionRequest(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			...
			{Role: greenruntime.MessageRoleUser, Content: "user"},
			{
				Role:    greenruntime.MessageRoleAssistant,
				Content: "using a tool",
				ToolCalls: []greenruntime.ToolCall{{
					ID:        "call_1",
					Name:      "read_file",
					Arguments: `{"path":"README.md"}`,
				}},
			},
			{Role: greenruntime.MessageRoleTool, Content: "contents", ToolCallID: "call_1"},
		},
		Tools: []greenruntime.ToolDefinition{{
			Name:        "read_file",
	...
	if gotUserAgent != "green-test" {
		t.Fatalf("user agent = %q, want green-test", gotUserAgent)
	}
...
func TestStreamCompletionOmitsAuthAndToolsWhenEmpty(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "hi"}},
	})
	if err != nil {
		t.Fatalf("StreamCompletion returned error: %v", err)
	}
...
func TestStreamCompletionAppliesCustomAuthAndHeaders(t *testing.T) {
	...
	provider, err := New(Options{
		...
		AuthScheme:    "Token",
		CustomHeaders: map[string]string{"HTTP-Referer": "https://green.dev"},
	})
	if err != nil {
		t.Fatalf("New returned error: %v", err)
	}
...
func TestStreamCompletionEmitsTextUsageAndDone(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSE(w, `{"choices":[{"delta":{"content":"green"}}],"usage":{"prompt_tokens":12,"completion_tokens":5,"prompt_tokens_details":{"cached_tokens":3}}}`)
		writeSSE(w, `[DONE]`)
	})

	events := collectProviderEvents(t, provider)
	assertEvent(t, events[0], greenruntime.StreamEventText, "hello ")
	assertEvent(t, events[1], greenruntime.StreamEventText, "green")
	if events[2].Type != greenruntime.StreamEventUsage || events[2].Usage.PromptTokens != 12 || events[2].Usage.CompletionTokens != 5 || events[2].Usage.CachedInputTokens != 3 {
		t.Fatalf("unexpected usage event: %#v", events[2])
	}
	if events[3].Type != greenruntime.StreamEventDone {
...
func TestStreamCompletionEmitsReasoningContentDeltas(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	reasoning := eventsOfType(events, greenruntime.StreamEventReasoning)
	if len(reasoning) != 2 {
	...
	if text := eventsOfType(events, greenruntime.StreamEventText); len(text) != 0 {
...
func TestStreamCompletionEmitsReasoningBeforeRegularContent(t *testing.T) {
	...
	assertEvent(t, events[0], greenruntime.StreamEventReasoning, "Thinking. ")
	assertEvent(t, events[1], greenruntime.StreamEventText, "Answer.")
	if events[2].Type != greenruntime.StreamEventDone {
...
func TestStreamCompletionPreservesLiteralThinkTagsByDefault(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	assertEvent(t, events[0], greenruntime.StreamEventText, "show <think>literal</think> markup")
	if reasoning := eventsOfType(events, greenruntime.StreamEventReasoning); len(reasoning) != 0 {
...
func TestStreamCompletionSplitsInlineThinkTagsFromContent(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	assertEvent(t, events[0], greenruntime.StreamEventReasoning, "private reasoning")
	assertEvent(t, events[1], greenruntime.StreamEventText, "public answer")
	if events[2].Type != greenruntime.StreamEventDone {
		t.Fatalf("third event = %#v, want done", events[2])
	}
}
...
func TestStreamCompletionSplitsInlineThinkTagsAcrossChunks(t *testing.T) {
	...
	text := eventsOfType(events, greenruntime.StreamEventText)
...
func TestStreamCompletionBuffersToolArgsUntilIDAndNameArrive(t *testing.T) {
	...
	if events[0].Type != greenruntime.StreamEventToolCallStart || events[0].ToolCallID != "call_1" || events[0].ToolName != "read_file" {
		t.Fatalf("unexpected start event: %#v", events[0])
	}
	if events[1].Type != greenruntime.StreamEventToolCallDelta || events[1].ToolCallID != "call_1" || events[1].ArgumentsFragment != `{"path":"README.md"}` {
		t.Fatalf("unexpected delta event: %#v", events[1])
	}
	if events[2].Type != greenruntime.StreamEventToolCallEnd || events[2].ToolCallID != "call_1" {
...
func TestStreamCompletionTracksMultipleToolCallsByIndex(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSE(w, `{"choices":[{"delta":{"tool_calls":[{"index":1,"id":"call_b","function":{"name":"grep","arguments":"\"green\"}"}}]},"finish_reason":"tool_calls"}]}`)
		writeSSE(w, `[DONE]`)
	})

	events := collectProviderEvents(t, provider)
	starts := eventsOfType(events, greenruntime.StreamEventToolCallStart)
	deltas := eventsOfType(events, greenruntime.StreamEventToolCallDelta)
	ends := eventsOfType(events, greenruntime.StreamEventToolCallEnd)
	...
	if deltas[1].ToolCallID != "call_b" || deltas[1].ArgumentsFragment != `{"query":"green"}` {
...
func TestStreamCompletionClassifiesHTTPErrorsAndRedactsToken(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(events) != 1 || events[0].Type != greenruntime.StreamEventError {
...
func TestStreamCompletionEmitsErrorWhenContextCancels(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(ctx, greenruntime.CompletionRequest{})
	if err != nil {
		t.Fatalf("StreamCompletion returned setup error: %v", err)
	}
	<-requestStarted
...
func TestStreamCompletionFlushesBufferedContentWhenContextCancels(t *testing.T) {
	...
	events := []greenruntime.StreamEvent{}
	select {
	...
	if len(events) != 3 ||
		events[0].Type != greenruntime.StreamEventText || events[0].Content != "visible " ||
		events[1].Type != greenruntime.StreamEventText || events[1].Content != "<thi" ||
		events[2].Type != greenruntime.StreamEventError {
		t.Fatalf("events = %#v, want text, buffered text, then context error", events)
	}
	if done := eventsOfType(events, greenruntime.StreamEventDone); len(done) != 0 {
...
func collectProviderEvents(t *testing.T, provider *Provider) []greenruntime.StreamEvent {
	t.Helper()
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{})
	if err != nil {
		t.Fatalf("StreamCompletion returned setup error: %v", err)
...
func readAll(stream <-chan greenruntime.StreamEvent) []greenruntime.StreamEvent {
	events := []greenruntime.StreamEvent{}
...
func drain(stream <-chan greenruntime.StreamEvent) {
	for range stream {
	}
}
...
func assertEvent(t *testing.T, event greenruntime.StreamEvent, eventType greenruntime.StreamEventType, content string) {
	t.Helper()
...
func eventsOfType(events []greenruntime.StreamEvent, eventType greenruntime.StreamEventType) []greenruntime.StreamEvent {
	matching := []greenruntime.StreamEvent{}
...
func TestStreamCompletionDoesNotHangOnEOFWithOpenToolCall(t *testing.T) {
	...
	for {
		...
		case event, ok := <-stream:
			if !ok {
				if len(eventsOfType(events, greenruntime.StreamEventToolCallEnd)) != 1 {
					t.Fatalf("events = %#v, want one tool-call-end on EOF", events)
				}
				if len(eventsOfType(events, greenruntime.StreamEventDone)) != 1 {
...
func TestStreamCompletionSkipsNamelessToolCallOnEOF(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	if len(eventsOfType(events, greenruntime.StreamEventToolCallStart)) != 0 {
		t.Fatalf("events = %#v, want no start for nameless tool call", events)
	}
	if len(eventsOfType(events, greenruntime.StreamEventToolCallDelta)) != 0 {
		t.Fatalf("events = %#v, want no delta for nameless tool call", events)
	}
	if len(eventsOfType(events, greenruntime.StreamEventToolCallEnd)) != 0 {
...
func TestStreamCompletionIdleTimeoutAbortsStalledStream(t *testing.T) {
	...
	done := make(chan []greenruntime.StreamEvent, 1)
	go func() { done <- readAll(stream) }()
	var events []greenruntime.StreamEvent
	select {
	case events = <-done:
	...
	for _, e := range events {
		if e.Type == greenruntime.StreamEventText && e.Content == "hi" {
			gotText = true
		}
		if e.Type == greenruntime.StreamEventError && strings.Contains(strings.ToLower(e.Error), "idle") {
			gotIdleError = true
		}
	}
	if !gotText {
...
func TestStreamCompletionSurfacesLengthFinishReason(t *testing.T) {
	...
	var sawDone bool
	for _, e := range events {
		if e.Type == greenruntime.StreamEventDone {
			sawDone = true
	...
	if doneReason != greenruntime.FinishReasonLength {
		t.Fatalf("done FinishReason = %q, want %q", doneReason, greenruntime.FinishReasonLength)
	}

	// And it round-trips through the runtime collector as Truncated.
	collected := greenruntime.CollectStream(context.Background(), replay(events))
	if !collected.Truncated() || collected.FinishReason != greenruntime.FinishReasonLength {
...
func TestStreamCompletionSurfacesContentFilterFinishReason(t *testing.T) {
	...
	for _, e := range events {
		if e.Type == greenruntime.StreamEventDone {
			sawDone = true
			if e.FinishReason != greenruntime.FinishReasonContentFilter {
				t.Fatalf("done FinishReason = %q, want %q", e.FinishReason, greenruntime.FinishReasonContentFilter)
			}
		}
	}
	if !sawDone {
...
func TestStreamCompletionJoinsMultiLineDataFields(t *testing.T) {
	...
	var text string
	for _, e := range events {
		if e.Type == greenruntime.StreamEventText {
			text += e.Content
		}
		if e.Type == greenruntime.StreamEventError {
...
func replay(events []greenruntime.StreamEvent) <-chan greenruntime.StreamEvent {
...
func TestStreamCompletionEmitsDroppedOnNamelessToolCall(t *testing.T) {
	...
	for _, e := range events {
		if e.Type == greenruntime.StreamEventToolCallDropped {
			dropped = true
		}
		if e.Type == greenruntime.StreamEventToolCallStart {
			started = true
		}
	}
	if started {
...
func TestMapMessageBuildsImageURLContentParts(t *testing.T) {
	msg := mapMessage(greenruntime.Message{
		Role:    greenruntime.MessageRoleUser,
		Content: "describe these",
		Images: []greenruntime.ImageBlock{
...
func TestMapMessageImageOnlyOmitsTextPart(t *testing.T) {
	msg := mapMessage(greenruntime.Message{
		Role:    greenruntime.MessageRoleUser,
		Content: "",
		Images:  []greenruntime.ImageBlock{{MediaType: "image/png", Data: []byte("A")}},
	})
	parts, ok := msg.Content.([]contentPart)
	if !ok {
		t.Fatalf("Content type = %T, want []contentPart", msg.Content)
...
func TestMapMessageTextOnlyKeepsStringContent(t *testing.T) {
	msg := mapMessage(greenruntime.Message{Role: greenruntime.MessageRoleUser, Content: "hi"})
	if got, ok := msg.Content.(string); !ok || got != "hi" {
		t.Fatalf("Content = %#v, want string \"hi\"", msg.Content)
	}
	empty := mapMessage(greenruntime.Message{Role: greenruntime.MessageRoleAssistant, Content: ""})
...
func TestMapMessageNonUserRolesNeverCarryImages(t *testing.T) {
	images := []greenruntime.ImageBlock{{MediaType: "image/png", Data: []byte("ABC")}}
	for _, role := range []greenruntime.MessageRole{
		greenruntime.MessageRoleAssistant,
		greenruntime.MessageRoleTool,
		greenruntime.MessageRoleSystem,
	} {
		msg := mapMessage(greenruntime.Message{Role: role, Content: "plain", Images: images})
...
func TestOpenAIRequestEmptyContentHandling(t *testing.T) {
	...
	req := provider.openAIRequest(greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleUser, Content: "hi"},
			// Degenerate empty assistant turn (e.g. a sub-agent that failed with no
			// output): must be dropped, not sent.
			{Role: greenruntime.MessageRoleAssistant, Content: "   "},
			// Assistant with tool calls but no text: kept, content present as "".
			{Role: greenruntime.MessageRoleAssistant, Content: "", ToolCalls: []greenruntime.ToolCall{{
				ID: "call_1", Name: "read_file", Arguments: "{}",
			}}},
			// Tool result with empty content: kept, content present as "".
			{Role: greenruntime.MessageRoleTool, Content: "", ToolCallID: "call_1"},
		},
	})
...
// and green_DISABLE_PROMPT_CACHE_KEY is the env kill switch for any endpoint.
func TestOpenAIRequestPromptCacheKey(t *testing.T) {
	...
	req = provider.openAIRequest(greenruntime.CompletionRequest{Messages: messages})
	...
	req = compat.openAIRequest(greenruntime.CompletionRequest{
		Messages:       messages,
		PromptCacheKey: "sess_123",
	})
	...
	t.Setenv("green_DISABLE_PROMPT_CACHE_KEY", "1")
	req = provider.openAIRequest(greenruntime.CompletionRequest{
		Messages:       messages,
		PromptCacheKey: "sess_123",
	})
	...
	// truthy values flip the toggle (same parsing as green_FORMAT_ON_WRITE).
	for _, value := range []string{"0", "false", "FALSE"} {
		t.Setenv("green_DISABLE_PROMPT_CACHE_KEY", value)
		req = provider.openAIRequest(greenruntime.CompletionRequest{
			Messages:       messages,
			PromptCacheKey: "sess_123",
		})
		if req.PromptCacheKey != "sess_123" {
			t.Fatalf("green_DISABLE_PROMPT_CACHE_KEY=%q must be a no-op; PromptCacheKey = %q", value, req.PromptCacheKey)
		}
	}
}
