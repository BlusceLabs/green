...
type toolState struct {
	...
	parseThinkTags bool
	// finishReason holds the normalized terminal stop reason (greenruntime
...
type streamEventEmitter func(greenruntime.StreamEvent)
...
func (state *toolState) emitContent(ctx context.Context, events chan<- greenruntime.StreamEvent, content string) {
	state.emitContentWith(content, func(event greenruntime.StreamEvent) {
		sendEvent(ctx, events, event)
	})
}

func (state *toolState) flushContent(ctx context.Context, events chan<- greenruntime.StreamEvent) {
	state.flushContentWith(func(event greenruntime.StreamEvent) {
		sendEvent(ctx, events, event)
	})
}

func (state *toolState) flushBufferedContent(events chan<- greenruntime.StreamEvent) {
	state.flushContentWith(func(event greenruntime.StreamEvent) {
...
func (state *toolState) emitContentWith(content string, emit streamEventEmitter) {
	if !state.parseThinkTags {
		emit(greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: content})
		return
	}
	state.think.push(content, func(eventType greenruntime.StreamEventType, text string) {
		emit(greenruntime.StreamEvent{Type: eventType, Content: text})
	})
}
...
func (state *toolState) flushContentWith(emit streamEventEmitter) {
	...
	state.think.flush(func(eventType greenruntime.StreamEventType, text string) {
		emit(greenruntime.StreamEvent{Type: eventType, Content: text})
	})
}

func (splitter *thinkTagSplitter) push(content string, emit func(greenruntime.StreamEventType, string)) {
	if content == "" {
		return
	}
...
func (splitter *thinkTagSplitter) flush(emit func(greenruntime.StreamEventType, string)) {
	splitter.drain(true, emit)
}

func (splitter *thinkTagSplitter) drain(final bool, emit func(greenruntime.StreamEventType, string)) {
	for {
		tag := thinkOpenTag
...
func (splitter *thinkTagSplitter) emitCurrent(emit func(greenruntime.StreamEventType, string), text string) {
	if text == "" {
		return
	}
	eventType := greenruntime.StreamEventText
	if splitter.inThinking {
		eventType = greenruntime.StreamEventReasoning
	}
	emit(eventType, text)
}
...
func (state *toolState) closeOpen(ctx context.Context, events chan<- greenruntime.StreamEvent) {
	state.closeOpenWith(func(event greenruntime.StreamEvent) {
		sendEvent(ctx, events, event)
	})
}

func (state *toolState) closeBufferedOpen(events chan<- greenruntime.StreamEvent) {
	state.closeOpenWith(func(event greenruntime.StreamEvent) {
		sendBufferedEvent(events, event)
	})
}
	...
	for _, index := range indexes {
		...
		if call.id == "" || call.name == "" {
			if call.id != "" || call.name != "" || call.arguments != "" {
				call.ended = true
				emit(greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallDropped})
			}
			continue
		}
		if !call.started {
			call.started = true
			emit(greenruntime.StreamEvent{
				Type:       greenruntime.StreamEventToolCallStart,
				ToolCallID: call.id,
				ToolName:   call.name,
			})
