...
type Options struct {
	...
	// MaxTokens caps the model's output tokens. green omits the cap (the model's
	...
	// enabled; the green_DISABLE_PROMPT_CACHE_KEY env kill switch still applies
...
func (provider *Provider) stream(ctx context.Context, body []byte, events chan<- greenruntime.StreamEvent) {
	...
	if err != nil {
		...
		if ctxErr := ctx.Err(); ctxErr != nil {
			sendEvent(ctx, events, greenruntime.StreamEvent{Type: greenruntime.StreamEventError, Error: provider.redact("provider stream error: " + ctxErr.Error())})
			return
		}
		...
		if humanized, ok := providerio.UpstreamUnreachable(err.Error()); ok {
			sendEvent(ctx, events, greenruntime.StreamEvent{Type: greenruntime.StreamEventError, Error: provider.redact(humanized)})
			return
		}
		sendEvent(ctx, events, greenruntime.StreamEvent{Type: greenruntime.StreamEventError, Error: provider.redact("provider stream error: " + err.Error())})
		return
	}
	defer func() {
	...
	if errors.Is(err, providerio.ErrStreamIdle) || errors.Is(err, providerio.ErrStreamStalled) {
		...
		sendEvent(ctx, events, greenruntime.StreamEvent{
	...
	if !state.done {
		...
		sendEvent(ctx, events, greenruntime.StreamEvent{Type: greenruntime.StreamEventDone, FinishReason: state.finishReason})
	}
}
...
func (provider *Provider) emitPayload(ctx context.Context, data string, state *toolState, events chan<- greenruntime.StreamEvent) bool {
...
func (provider *Provider) emitChunk(
	...
	chunk streamChunk,
	state *toolState,
	events chan<- greenruntime.StreamEvent,
) {
	for _, choice := range chunk.Choices {
		if reasoning := choice.Delta.reasoningText(); reasoning != "" {
			sendEvent(ctx, events, greenruntime.StreamEvent{
				Type:    greenruntime.StreamEventReasoning,
				Content: reasoning,
			})
		}
	...
	if chunk.Usage != nil {
		sendEvent(ctx, events, greenruntime.StreamEvent{
			Type: greenruntime.StreamEventUsage,
...
func sendEvent(ctx context.Context, events chan<- greenruntime.StreamEvent, event greenruntime.StreamEvent) {
	select {
	case <-ctx.Done():
		if event.Type == greenruntime.StreamEventError {
			select {
			case events <- event:
			default:
			}
...
func sendBufferedEvent(events chan<- greenruntime.StreamEvent, event greenruntime.StreamEvent) {
	select {
	case events <- event:
	default:
	}
...
func (provider *Provider) openAIRequest(request greenruntime.CompletionRequest) chatCompletionRequest {
	...
	mapped := chatCompletionRequest{
		...
		// which silently greenes token accounting.
	...
	// endpoint can still force-omit via green_DISABLE_PROMPT_CACHE_KEY=1.
...
// promptCacheKeyDisabled reports whether the green_DISABLE_PROMPT_CACHE_KEY
// kill switch is set to a truthy value. "0" and "false" (any case) are
// no-ops, matching how green_FORMAT_ON_WRITE parses boolean flags, so an
// explicitly-disabled toggle never flips the behavior it names.
func promptCacheKeyDisabled() bool {
	value := strings.TrimSpace(os.Getenv("green_DISABLE_PROMPT_CACHE_KEY"))
...
func mapMessage(message greenruntime.Message) chatMessage {
	mapped := chatMessage{
	...
	if len(message.Images) == 0 || message.Role != greenruntime.MessageRoleUser {
