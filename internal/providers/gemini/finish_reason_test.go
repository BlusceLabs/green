...
func TestMapFinishReasonNonNormal(t *testing.T) {
	...
	for _, cf := range []string{"SAFETY", "RECITATION", "IMAGE_SAFETY", "PROHIBITED_CONTENT", "BLOCKLIST", "SPII"} {
		if got := mapFinishReason(cf); got != greenruntime.FinishReasonContentFilter {
			t.Errorf("%q → %q, want content_filter (M3)", cf, got)
		}
	}
	if got := mapFinishReason("MAX_TOKENS"); got != greenruntime.FinishReasonLength {
...
func TestStreamCompletionSurfacesRecitationFinishReason(t *testing.T) {
	...
	for _, e := range events {
		if e.Type == greenruntime.StreamEventDone {
			sawDone = true
			if e.FinishReason != greenruntime.FinishReasonContentFilter {
				t.Fatalf("RECITATION done FinishReason = %q, want %q (M3)", e.FinishReason, greenruntime.FinishReasonContentFilter)
			}
		}
	}
	if !sawDone {
...
func TestStreamCompletionEmitsDroppedOnNamelessFunctionCallPart(t *testing.T) {
	...
	for _, e := range events {
		switch e.Type {
		case greenruntime.StreamEventToolCallDropped:
			dropped++
...
func TestStreamCompletionDoesNotDropValidFunctionCall(t *testing.T) {
	...
	for _, e := range events {
		...
		if e.Type == greenruntime.StreamEventToolCallStart && e.ToolName == "read_file" {
			sawStart = true
		}
	}
	if !sawStart {
