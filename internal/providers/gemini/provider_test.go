...
func TestStreamCompletionPostsGenerateContentRequest(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleSystem, Content: "You are green."},
			{Role: greenruntime.MessageRoleUser, Content: "Read the file."},
			{
			...
			{Role: greenruntime.MessageRoleTool, Content: "file contents", ToolCallID: "call_1"},
			{Role: greenruntime.MessageRoleUser, Content: "Now grep for green."},
		},
	...
	if mergedUserParts[1].(map[string]any)["text"] != "Now grep for green." {
...
func TestStreamCompletionCapturesThoughtSignatureAndSkipsThoughtText(t *testing.T) {
	...
	for _, event := range events {
		if event.Type == greenruntime.StreamEventText && strings.Contains(event.Content, "internal reasoning") {
...
func TestGeminiRequestReplaysThoughtSignature(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleUser, Content: "go"},
			{
				Role:      greenruntime.MessageRoleAssistant,
				ToolCalls: []greenruntime.ToolCall{{ID: "call_1", Name: "grep", Arguments: `{"pattern":"x"}`, Signature: "sig-xyz"}},
			},
			{Role: greenruntime.MessageRoleTool, Content: "result", ToolCallID: "call_1"},
		},
	})
	if err != nil {
		t.Fatalf("StreamCompletion returned error: %v", err)
...
func TestStreamCompletionAppliesCustomAuthAndHeaders(t *testing.T) {
	...
	provider, err := New(Options{
		...
		AuthScheme:    "Bearer",
		CustomHeaders: map[string]string{"X-Tenant": "green"},
	})
	if err != nil {
		t.Fatalf("New returned error: %v", err)
	}
...
func TestStreamCompletionEmitsTextUsageAndReasoningTokens(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSE(w, `{"candidates":[{"content":{"parts":[{"text":" green"}]}}],"usageMetadata":{"promptTokenCount":25,"candidatesTokenCount":15,"thoughtsTokenCount":3,"cachedContentTokenCount":7}}`)
	})

	events := collectProviderEvents(t, provider)
	want := []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventText, Content: "Hello"},
		{Type: greenruntime.StreamEventText, Content: " green"},
		{Type: greenruntime.StreamEventUsage, Usage: greenruntime.Usage{InputTokens: 25, OutputTokens: 18, PromptTokens: 25, CompletionTokens: 18, ReasoningTokens: 3, CachedInputTokens: 7}},
...
func TestStreamCompletionEmitsCandidateFunctionCalls(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		writeSSE(w, `{"candidates":[{"content":{"parts":[{"functionCall":{"id":"call_1","name":"read_file","args":{"path":"src/index.ts"}}},{"functionCall":{"id":"call_2","name":"grep","args":{"pattern":"green"}}}]}}]}`)
	})

	events := collectProviderEvents(t, provider)
	want := []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call_1", ToolName: "read_file"},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call_1", ArgumentsFragment: `{"path":"src/index.ts"}`},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call_1"},
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call_2", ToolName: "grep"},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call_2", ArgumentsFragment: `{"pattern":"green"}`},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call_2"},
		{Type: greenruntime.StreamEventDone},
	}
	if !reflect.DeepEqual(events, want) {
		t.Fatalf("events = %#v, want %#v", events, want)
...
func TestStreamCompletionUsesSyntheticToolIDsWhenMissing(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		writeSSE(w, `{"functionCalls":[{"name":"grep","args":{"pattern":"green"}}]}`)
	})
...
func TestStreamCompletionClassifiesHTTPAndPromptBlockErrors(t *testing.T) {
	...
	if len(events) != 1 || events[0].Type != greenruntime.StreamEventError || !strings.HasPrefix(events[0].Error, "auth error:") {
	...
	if len(events) != 1 || events[0].Type != greenruntime.StreamEventError || !strings.Contains(events[0].Error, "Content blocked: blocked by policy") {
...
func TestStreamCompletionStopsOnMalformedStreamToolArgs(t *testing.T) {
	...
	if len(eventsOfType(events, greenruntime.StreamEventDone)) != 0 {
...
func TestStreamCompletionRejectsMalformedHistoryBeforeDispatch(t *testing.T) {
	...
	_, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{{Role: greenruntime.MessageRoleTool, Content: "missing id"}},
	})
	...
	_, err = provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleUser, Content: "call tool"},
			{
				Role:      greenruntime.MessageRoleAssistant,
				ToolCalls: []greenruntime.ToolCall{{ID: "call_1", Name: "read_file", Arguments: `"src/index.ts"`}},
			},
		},
	})
...
func validRequest() greenruntime.CompletionRequest {
	return greenruntime.CompletionRequest{
...
// parameters green emits (schemaToRuntimeMap always writes additionalProperties)
