...
func TestStreamCompletionPostsMessagesRequest(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			...
			{Role: greenruntime.MessageRoleTool, Content: "file contents", ToolCallID: "toolu_1"},
		},
	...
	if sysBlock["type"] != "text" || sysBlock["text"] != "You are green." {
...
func TestStreamCompletionEmitsTextUsageAndDone(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSEEvent(w, "content_block_delta", `{"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":" green"}}`)
	...
	want := []greenruntime.StreamEvent{
		...
		{Type: greenruntime.StreamEventUsage, Usage: greenruntime.Usage{InputTokens: 25, OutputTokens: 15, PromptTokens: 25, CompletionTokens: 15}},
...
func TestStreamCompletionReportsCacheTokens(t *testing.T) {
	...
	var usage *greenruntime.Usage
	for _, event := range collectProviderEvents(t, provider) {
		if event.Type == greenruntime.StreamEventUsage {
			u := event.Usage
			usage = &u
		}
	}
...
func TestStreamCompletionCapturesThinkingBlocksForReplay(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	done := eventsOfType(events, greenruntime.StreamEventDone)
	if len(done) != 1 {
	...
	stream, err := provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleUser, Content: "go"},
			{
				Role:      greenruntime.MessageRoleAssistant,
				Content:   "I will grep.",
				ToolCalls: []greenruntime.ToolCall{{ID: "toolu_1", Name: "grep", Arguments: `{"pattern":"x"}`}},
				Reasoning: []greenruntime.ReasoningBlock{{Provider: providerName, Type: "thinking", Text: "reasoning", Signature: "sig-abc"}},
			},
			{Role: greenruntime.MessageRoleTool, Content: "result", ToolCallID: "toolu_1"},
		},
	})
	if err != nil {
		t.Fatalf("StreamCompletion returned error: %v", err)
...
func TestStreamCompletionEmitsToolUseBlocks(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	want := []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "toolu_1", ToolName: "read_file"},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "toolu_1", ArgumentsFragment: `{"path":`},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "toolu_1", ArgumentsFragment: `"src/index.ts"}`},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "toolu_1"},
		{Type: greenruntime.StreamEventDone},
	}
	if !reflect.DeepEqual(events, want) {
		t.Fatalf("events = %#v, want %#v", events, want)
...
func TestStreamCompletionClosesOpenToolCallOnEOF(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSEEvent(w, "content_block_delta", `{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"pattern\":\"green\"}"}}`)
	})
...
func TestStreamCompletionRejectsMalformedHistoryBeforeDispatch(t *testing.T) {
	...
	_, err = provider.StreamCompletion(context.Background(), greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			...
			{
				Role:      greenruntime.MessageRoleAssistant,
				ToolCalls: []greenruntime.ToolCall{{ID: "toolu_1", Name: "read_file", Arguments: `"src/index.ts"`}},
			},
		},
	})
