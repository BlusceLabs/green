...
func captureRequestBody(t *testing.T, request greenruntime.CompletionRequest) map[string]any {
	t.Helper()
...
func TestUserTextOnlyTurnUnchanged(t *testing.T) {
	body := captureRequestBody(t, greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleUser, Content: "Describe this."},
		},
	})
		...
		Messages: []greenruntime.Message{
			{
				...
				Images:  []greenruntime.ImageBlock{{MediaType: "image/png", Data: raw}},
			},
		},
	})
	blocks := firstUserContentBlocks(t, body)
...
func TestUserImageOnlyTurnEmits(t *testing.T) {
	...
	body := captureRequestBody(t, greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{
				Role:   greenruntime.MessageRoleUser,
				Images: []greenruntime.ImageBlock{{MediaType: "image/jpeg", Data: raw}},
			},
		},
	})
