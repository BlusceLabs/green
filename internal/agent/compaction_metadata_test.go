...
func TestCompactMessagesReturnsMetadataForManualCompaction(t *testing.T) {
	messages := []greenruntime.Message{
		...
		{Role: greenruntime.MessageRoleAssistant, Content: "recent answer"},
		{Role: greenruntime.MessageRoleUser, Content: "latest question"},
	}
	...
	result, err := CompactMessages(messages, CompactionOptions{
		...
		Summarize: func(toSummarize []greenruntime.Message) (string, error) {
			captured = append([]greenruntime.Message(nil), toSummarize...)
	...
	if result.Messages[1].Role != greenruntime.MessageRoleUser {
