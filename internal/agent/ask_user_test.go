...
func providerCallingAskUserThenAnswer(arguments string, answer string) *mockProvider {
	return &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "ask_user"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: arguments},
...
func TestRunAskUserReturnsHandlerAnswers(t *testing.T) {
	...
	if toolMessage.Role != greenruntime.MessageRoleTool || toolMessage.ToolCallID != "call-1" {
