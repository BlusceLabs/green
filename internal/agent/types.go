...
type Message = greenruntime.Message
type Provider = greenruntime.Provider
type ToolCall = greenruntime.ToolCall
type Usage = greenruntime.Usage
...
type Options struct {
	...
	// can detect a file changed on disk outside green since it was last read. nil
...
type Result struct {
	...
	// that produced FinalAnswer: greenruntime.FinishReasonLength when the output
...
func (result Result) TruncationNotice() string {
	...
	case greenruntime.FinishReasonLength:
		return "Response was cut off at the output token limit and may be incomplete. " +
			"Raise the model's max output tokens or ask green to continue."
	case greenruntime.FinishReasonContentFilter:
