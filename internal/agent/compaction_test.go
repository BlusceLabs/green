...
func TestCompactKeepsSystemAndPreservedSuffix(t *testing.T) {
	messages := []greenruntime.Message{
		{Role: greenruntime.MessageRoleSystem, Content: "system prompt"},
		{Role: greenruntime.MessageRoleUser, Content: "first question"},
		{Role: greenruntime.MessageRoleAssistant, Content: "first answer"},
		{Role: greenruntime.MessageRoleUser, Content: "second question"},
		{Role: greenruntime.MessageRoleAssistant, Content: "second answer"},
		{Role: greenruntime.MessageRoleUser, Content: "most recent question"},
	}

	var captured []greenruntime.Message
	summarizeCalls := 0
	result, err := Compact(messages, CompactionOptions{
		PreserveLast: 2,
		Summarize: func(toSummarize []greenruntime.Message) (string, error) {
			summarizeCalls++
	...
	if result[0].Role != greenruntime.MessageRoleSystem || result[0].Content != "system prompt" {
		t.Fatalf("expected system message preserved at head, got %#v", result[0])
	}
	// Next is the injected summary as a single user message.
	if result[1].Role != greenruntime.MessageRoleUser {
...
func TestCompactSuffixNeverStartsWithToolResult(t *testing.T) {
	...
	messages := []greenruntime.Message{
		{Role: greenruntime.MessageRoleSystem, Content: "system prompt"},
		{Role: greenruntime.MessageRoleUser, Content: "do the thing"},
		{Role: greenruntime.MessageRoleAssistant, Content: "ok", ToolCalls: []greenruntime.ToolCall{{ID: "1", Name: "read_file"}}},
		{Role: greenruntime.MessageRoleTool, Content: "file contents", ToolCallID: "1"},
		{Role: greenruntime.MessageRoleAssistant, Content: "here is the result", ToolCalls: []greenruntime.ToolCall{{ID: "2", Name: "read_file"}}},
		{Role: greenruntime.MessageRoleTool, Content: "more contents", ToolCallID: "2"},
	}
...
func TestCompactNoopWhenTooFewMessages(t *testing.T) {
	...
	result, err := Compact(messages, CompactionOptions{
		PreserveLast: 8,
		Summarize: func([]greenruntime.Message) (string, error) {
			called = true
			return "x", nil
		},
	})
...
func TestCompactPropagatesSummarizeError(t *testing.T) {
	messages := []greenruntime.Message{
		{Role: greenruntime.MessageRoleSystem, Content: "system"},
		{Role: greenruntime.MessageRoleUser, Content: "a"},
		{Role: greenruntime.MessageRoleAssistant, Content: "b"},
		{Role: greenruntime.MessageRoleUser, Content: "c"},
		{Role: greenruntime.MessageRoleAssistant, Content: "d"},
		{Role: greenruntime.MessageRoleUser, Content: "e"},
	}
...
func (p *scriptedSummaryProvider) StreamCompletion(_ context.Context, _ greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	i := p.calls
	p.calls++
	if i < len(p.scripts) && p.scripts[i] != "" {
		return streamEvents([]greenruntime.StreamEvent{{Type: greenruntime.StreamEventError, Error: p.scripts[i]}}), nil
	}
	return streamEvents([]greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventText, Content: "PARTIAL"},
		{Type: greenruntime.StreamEventDone},
	}), nil
}

func TestSummarizeWithFallbackPropagatesNonContextReduceError(t *testing.T) {
	msgs := []greenruntime.Message{
		{Role: greenruntime.MessageRoleUser, Content: "a"},
		{Role: greenruntime.MessageRoleAssistant, Content: "b"},
	}
...
func TestEstimateTokensMonotonic(t *testing.T) {
	small := []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "short"}}
	large := []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: strings.Repeat("x", 4000)}}
	...
	grown := append([]greenruntime.Message{}, large...)
	grown = append(grown, greenruntime.Message{Role: greenruntime.MessageRoleAssistant, Content: "more"})
...
func TestEstimateTokensCountsToolCallsAndResults(t *testing.T) {
	plain := []greenruntime.Message{{Role: greenruntime.MessageRoleAssistant, Content: "hi"}}
	withCall := []greenruntime.Message{{
		Role:      greenruntime.MessageRoleAssistant,
		Content:   "hi",
		ToolCalls: []greenruntime.ToolCall{{ID: "1", Name: "read_file", Arguments: strings.Repeat("a", 400)}},
	}}
...
func TestEstimateTokensCountsImages(t *testing.T) {
	...
	plain := []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "look"}}
	withImage := []greenruntime.Message{{
		Role:    greenruntime.MessageRoleUser,
		Content: "look",
		Images:  []greenruntime.ImageBlock{{MediaType: "image/png", Data: []byte("tiny")}},
	}}
	twoImages := []greenruntime.Message{{
		Role:    greenruntime.MessageRoleUser,
		Content: "look",
...
func TestEstimateToolDefTokensCountsDefinitions(t *testing.T) {
	...
	one := []greenruntime.ToolDefinition{{
		Name:        "read_file",
	...
	two := append(append([]greenruntime.ToolDefinition{}, one...), greenruntime.ToolDefinition{
		Name:        "write_file",
...
func (provider *summarizeRecordingProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	...
	if len(request.Tools) == 0 {
		...
		return streamEvents([]greenruntime.StreamEvent{
			{Type: greenruntime.StreamEventText, Content: "COMPACTED SUMMARY"},
			{Type: greenruntime.StreamEventDone},
		}), nil
	}
...
func streamEvents(events []greenruntime.StreamEvent) <-chan greenruntime.StreamEvent {
...
func TestRunNoCompactionWhenContextWindowgreen(t *testing.T) {
...
func (provider *reactiveProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	provider.requests = append(provider.requests, request)
	if len(request.Tools) == 0 {
		provider.summarizeCalls++
		return streamEvents([]greenruntime.StreamEvent{
			{Type: greenruntime.StreamEventText, Content: "SUMMARY"},
			{Type: greenruntime.StreamEventDone},
		}), nil
	}
	...
	case provider.turnRequests == 2 && !provider.failedOnce:
		...
		return streamEvents([]greenruntime.StreamEvent{
			{Type: greenruntime.StreamEventError, Error: "This model's maximum context length is 1000 tokens. Please reduce the length of the messages."},
		}), nil
	default:
...
func (provider *midStreamReactiveProvider) StreamCompletion(_ context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	if len(request.Tools) == 0 {
		provider.summarizeCalls++
		return streamEvents([]greenruntime.StreamEvent{
			{Type: greenruntime.StreamEventText, Content: "SUMMARY"},
	...
	case provider.turnRequests == 2 && !provider.failedOnce:
		...
		return streamEvents([]greenruntime.StreamEvent{
			{Type: greenruntime.StreamEventText, Content: provider.partialText},
			{Type: greenruntime.StreamEventError, Error: "This model's maximum context length is 1000 tokens. Please reduce the length of the messages."},
		}), nil
	default:
		return streamEvents(textTurn(provider.finalText)), nil
...
func toolTurnWithText(text string, callID string, toolName string, args string) []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventText, Content: text},
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: callID, ToolName: toolName},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: callID, ArgumentsFragment: args},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: callID},
		{Type: greenruntime.StreamEventDone},
	}
}
...
func TestCompactNeverProducesConsecutiveUserMessages(t *testing.T) {
	msgs := []greenruntime.Message{
		{Role: greenruntime.MessageRoleSystem, Content: "sys"},
		{Role: greenruntime.MessageRoleUser, Content: "u1"},
		{Role: greenruntime.MessageRoleAssistant, Content: "a1"},
		{Role: greenruntime.MessageRoleUser, Content: "u2"},
		{Role: greenruntime.MessageRoleAssistant, Content: "a2"},
		{Role: greenruntime.MessageRoleUser, Content: "u3-latest"},
	}
	...
	for i := 1; i < len(out); i++ {
		if out[i].Role == greenruntime.MessageRoleUser && out[i-1].Role == greenruntime.MessageRoleUser {
...
func TestRecoverNoopDoesNotConsumeReactiveBudget(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]greenruntime.StreamEvent{{
		{Type: greenruntime.StreamEventText, Content: "SUMMARY"}, {Type: greenruntime.StreamEventDone},
	}}}
	...
	big := []greenruntime.Message{
		{Role: greenruntime.MessageRoleSystem, Content: "sys"},
		{Role: greenruntime.MessageRoleUser, Content: strings.Repeat("u", 4000)},
		{Role: greenruntime.MessageRoleAssistant, Content: strings.Repeat("a", 4000)},
		{Role: greenruntime.MessageRoleUser, Content: "u2"},
		{Role: greenruntime.MessageRoleAssistant, Content: "a2"},
		{Role: greenruntime.MessageRoleUser, Content: "u3"},
	}
...
func TestRecoverDisabledIsNoop(t *testing.T) {
	...
	msgs := []greenruntime.Message{{Role: greenruntime.MessageRoleUser, Content: "x"}}
	called := false
	// recover must not invoke the provider/summarize when disabled, even on a
	// context-limit error string.
	got, retried, err := st.recover(context.Background(), &mockProvider{turns: [][]greenruntime.StreamEvent{{
		{Type: greenruntime.StreamEventText, Content: "should not be called"}, {Type: greenruntime.StreamEventDone},
