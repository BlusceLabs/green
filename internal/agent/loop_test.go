...
type mockProvider struct {
	turns    [][]greenruntime.StreamEvent
	requests []greenruntime.CompletionRequest
}

func (provider *mockProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	provider.requests = append(provider.requests, request)

	events := []greenruntime.StreamEvent{{Type: greenruntime.StreamEventDone}}
	if len(provider.turns) >= len(provider.requests) {
		events = provider.turns[len(provider.requests)-1]
	}

	ch := make(chan greenruntime.StreamEvent, len(events))
	...
	dispatcher := hooks.NewDispatcher(hooks.DispatcherOptions{
		Config: hooks.Config{
			...
			Hooks: []hooks.Definition{
				{ID: "green.session-start", Event: hooks.EventSessionStart, Command: "green-missing-hook-command", Enabled: true},
				{ID: "green.session-end", Event: hooks.EventSessionEnd, Command: "green-missing-hook-command", Enabled: true},
			},
		},
		Audit: audit,
	})
	provider := &mockProvider{turns: [][]greenruntime.StreamEvent{{
		{Type: greenruntime.StreamEventText, Content: "done"},
		{Type: greenruntime.StreamEventDone},
	}}}
...
func agentNativeBackendStub() sandbox.Backend {
	return sandbox.Backend{
		...
		Available:       true,
		Executable:      "/nonexistent/green-linux-sandbox-stub",
...
func TestRunRetriesShellUnsandboxedAfterSandboxNamespaceLimitedOutput(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"ps aux"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "done"},
...
func TestRunUsesEscalatedJustificationForPermissionPrompt(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"ps aux","sandbox_permissions":"require_escalated","justification":"Need host process visibility."}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
		},
...
func TestRunUsesUserFacingEscalatedFallbackForPermissionPrompt(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "exec_command"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"ps aux","sandbox_permissions":"require_escalated"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
		},
...
func TestRunRetriesNetworkDeniedShellWithNetworkGrant(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"node server.js"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunRetriesShellUnsandboxedAfterSandboxDeniedExit(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"npm install --save http-server"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunDoesNotRetryUnsandboxedWhenDeniedReadsActive(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"touch cache-file"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunReturnsProviderText(t *testing.T) {
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{{
			{Type: greenruntime.StreamEventText, Content: "hello"},
			{Type: greenruntime.StreamEventText, Content: " green"},
			{Type: greenruntime.StreamEventDone},
		}},
	}
	...
	if result.FinalAnswer != "hello green" {
	...
	assertMessage(t, provider.requests[0].Messages[0], greenruntime.MessageRoleSystem, "")
	assertMessage(t, provider.requests[0].Messages[1], greenruntime.MessageRoleUser, "say hi")
}

func TestRunDoesNotPersistReasoningAsAssistantText(t *testing.T) {
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{{
			{Type: greenruntime.StreamEventReasoning, Content: "private reasoning"},
			{Type: greenruntime.StreamEventText, Content: "public answer"},
			{Type: greenruntime.StreamEventDone},
		}},
	}
	...
	if last.Role != greenruntime.MessageRoleAssistant || last.Content != "public answer" {
...
func TestRunReportsTruncationFinishReason(t *testing.T) {
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{{
			{Type: greenruntime.StreamEventText, Content: "partial answer"},
			{Type: greenruntime.StreamEventDone, FinishReason: greenruntime.FinishReasonLength},
		}},
	}
	...
	if result.FinishReason != greenruntime.FinishReasonLength {
		t.Fatalf("FinishReason = %q, want %q", result.FinishReason, greenruntime.FinishReasonLength)
	}
	if !result.Truncated() {
...
func TestResultTruncationNotice(t *testing.T) {
	cases := map[string]struct {
		...
		"length":         {greenruntime.FinishReasonLength, true},
		"content_filter": {greenruntime.FinishReasonContentFilter, true},
...
func TestRunEmitsTextDeltas(t *testing.T) {
	...
	if strings.Join(deltas, "|") != "hello| green" {
...
func TestRunEmitsUsageEvents(t *testing.T) {
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{{
			{Type: greenruntime.StreamEventUsage, Usage: greenruntime.Usage{PromptTokens: 12, CompletionTokens: 5, CachedInputTokens: 2}},
			{Type: greenruntime.StreamEventText, Content: "done"},
			{Type: greenruntime.StreamEventDone},
		}},
	}

	var usages []greenruntime.Usage
	_, err := Run(context.Background(), "track usage", provider, Options{
		Registry: tools.NewRegistry(),
		OnUsage:  func(usage greenruntime.Usage) { usages = append(usages, usage) },
	})
...
func TestRunRejectsLocalWebFetchBeforePermissionRequest(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "web_fetch"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"url":"http://localhost:8000/index.html"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunAdvertisesPromptedWebSearchInAutoMode(t *testing.T) {
	t.Setenv("green_WEBSEARCH_BASE_URL", "https://search.example/api")
	registry := tools.NewRegistry()
...
func TestRunRequestsPermissionBeforeWebSearchExecution(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "web_search"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"query":"private workspace detail"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunRejectsFilteredToolCalls(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call_1", ToolName: "read_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call_1", ArgumentsFragment: `{"path":"README.md"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call_1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "done"},
...
func TestRunExecutesToolCallThroughRegistry(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "read_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"notes.txt"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "read done"},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
	...
	lastMessage := provider.requests[1].Messages[len(provider.requests[1].Messages)-1]
	assertMessage(t, lastMessage, greenruntime.MessageRoleTool, "alpha")
	if lastMessage.ToolCallID != "call-1" {
		t.Fatalf("expected tool_call_id call-1, got %q", lastMessage.ToolCallID)
	}
...
func TestRunSanitizesMalformedToolCallArgumentsBeforeRetry(t *testing.T) {
		...
		turns: [][]greenruntime.StreamEvent{
			{
				...
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"README.md"`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "recovered"},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
	...
	var assistantCall *greenruntime.ToolCall
	var toolParseError string
	for _, message := range provider.requests[1].Messages {
		if message.Role == greenruntime.MessageRoleAssistant && len(message.ToolCalls) > 0 {
			assistantCall = &message.ToolCalls[0]
		}
		if message.Role == greenruntime.MessageRoleTool && strings.Contains(message.Content, "Failed to parse arguments for read_file") {
...
func TestRunRecoversFirstObjectFromConcatenatedToolArgs(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "read_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"README.md"}{"path":"AGENTS.md"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
	...
	for _, message := range provider.requests[1].Messages {
		if message.Role == greenruntime.MessageRoleTool {
...
func TestRunDefersSelfCorrectFeedbackUntilAfterToolBatch(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"notes.txt","content":"hello"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "read_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":"existing.txt"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "done"},
...
func TestRunBatchesSelfCorrectOncePerTurn(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"a.txt","content":"hello"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":"b.txt","content":"hello"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: greenruntime.StreamEventDone},
			},
			{
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == greenruntime.MessageRoleUser && strings.Contains(m.Content, "Verification failed after your edit") {
			feedbackCount++
		}
	}
	if feedbackCount != 1 {
...
func TestRunSessionAllowSkipsMatchingPromptWithoutPersistentGrant(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"notes.txt","content":"first","overwrite":true}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":"notes.txt","content":"second","overwrite":true}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunCommandPrefixApprovalSkipsLaterMatchingBashPrompt(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo prefix-ok"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"command":"echo prefix-ok again"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunCommandPrefixApprovalCoversSegmentedShellWithSafeTail(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			...
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: fmt.Sprintf(`{"command":%q}`, segmentedCommand)},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunPersistentCommandPrefixApprovalSkipsFutureSessionPrompt(t *testing.T) {
	...
	firstProvider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo persist-ok"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "first done"},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
	...
	secondProvider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"command":"echo persist-ok again"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "second done"},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
...
func TestRunPersistentCommandPrefixStillPromptsForNetwork(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"curl https://example.com"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunApprovedNetworkBashPromptAppliesTurnNetworkGrant(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":` + quoteJSONString(command) + `}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunDoesNotOfferPrefixApprovalForUnsafeBashCommand(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo hi && npm install","prefix_rule":["echo"]}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunPromptsForDestructiveShellInsteadOfSandboxDeny(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo rm -rf /"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func (p cancelMidStreamProvider) StreamCompletion(_ context.Context, _ greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	p.cancel()
	return make(chan greenruntime.StreamEvent), nil
}
...
func TestRunSessionAllowsLaterOutsideWorkspaceWrite(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":` + quoteJSONString(outside) + `,"content":"first"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":` + quoteJSONString(outside) + `,"content":"second","overwrite":true}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunRequestsFinalAnswerAfterMaxTurns(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			...
			{
				{Type: greenruntime.StreamEventText, Content: "I read notes.txt and found alpha."},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
	...
	if lastMessage.Role != greenruntime.MessageRoleUser || !strings.Contains(lastMessage.Content, "tool-turn limit") {
...
func providerCallingWritePathThenAnswer(path string, answer string) *mockProvider {
	return &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":` + quoteJSONString(path) + `,"content":"hello"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: answer},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
...
func TestRunAppendsConfirmationPolicyToSystemPrompt(t *testing.T) {
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{{
			{Type: greenruntime.StreamEventText, Content: "ok"},
			{Type: greenruntime.StreamEventDone},
		}},
	}
	...
	if system.Role != greenruntime.MessageRoleSystem {
	...
	for _, marker := range []string{"You are green", "Testing gate"} {
...
func TestSystemPromptEmbedsConfirmationPolicy(t *testing.T) {
	...
	if !strings.HasPrefix(prompt, "You are green") {
...
func TestSpecDraftDeniesBashToolCalls(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"printf ran > ran.txt"}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
...
func TestRunStopsWhenSubmitSpecReturnsReviewControl(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{{
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "exit-1", ToolName: specmode.SubmitToolName},
			{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "exit-1", ArgumentsFragment: `{"title":"Implementation Plan","plan":"# Goal\n\nAdd implementation plan."}`},
			{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "exit-1"},
			{Type: greenruntime.StreamEventDone},
		}},
	}
	...
	if !strings.Contains(result.FinalAnswer, ".green/specs/") {
		t.Fatalf("final answer should mention saved spec, got %q", result.FinalAnswer)
	}
}

func assertMessage(t *testing.T, message greenruntime.Message, role greenruntime.MessageRole, contentContains string) {
	t.Helper()
...
func TestRunRetriesOnDroppedToolCall(t *testing.T) {
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventText, Content: "Let me write the files."},
				{Type: greenruntime.StreamEventToolCallDropped},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: "All done."},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
	...
	var fedback bool
	for _, m := range provider.requests[1].Messages {
		if m.Role == greenruntime.MessageRoleUser && strings.Contains(strings.ToLower(m.Content), "tool name") {
			fedback = true
		}
	}
	if !fedback {
...
func TestRunSurfacesDroppedToolCallAlongsideValidCall(t *testing.T) {
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == greenruntime.MessageRoleTool && m.ToolCallID == "call-1" {
			sawToolResult = true
		}
	}
	if !sawToolResult {
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == greenruntime.MessageRoleUser && strings.Contains(strings.ToLower(m.Content), "malformed") {
...
func TestRunAppendsAbortedPlaceholderForUnexecutedToolCallsOnGuardStop(t *testing.T) {
	...
	haltingTurn := []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "flaky-stop", ToolName: "flaky"},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "flaky-stop"},
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "flaky-2", ToolName: "flaky"},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "flaky-2"},
		{Type: greenruntime.StreamEventDone},
	}
	...
	for _, message := range result.Messages {
		if message.Role != greenruntime.MessageRoleAssistant {
			continue
		}
...
func TestRunScrubsSecretsFromToolOutput(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]greenruntime.StreamEvent{
		{
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "leak"},
			{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "c1"},
			{Type: greenruntime.StreamEventDone},
		},
		{
			{Type: greenruntime.StreamEventText, Content: "done"},
...
func TestRunDoesNotFlagCleanToolOutput(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]greenruntime.StreamEvent{
		...
		{{Type: greenruntime.StreamEventText, Content: "done"}, {Type: greenruntime.StreamEventDone}},
	}}
	var captured ToolResult
