...
const fallbackSystemPrompt = "You are green, a terminal coding agent. Help with the current workspace and use tools when needed."
...
var projectContextFiles = []string{"AGENTS.md", "green.md", ".green/AGENTS.md"}

// userContextFile is the per-user instruction file, resolved under
// config.UserConfigDir()/green/ alongside the rest of green's per-user config
// (config.json, commands, specialists) so users can keep personal guidance out
// of individual repositories.
const userContextFile = "green.md"
...
func buildSystemPrompt(options Options) string {
	...
	// project's AGENTS.md/green.md is the later, more specific instruction
...
// userGuidelines returns the per-user green.md instructions block, if present.
// The file lives in config.UserConfigDir()/green/ next to green's other
// per-user config; the basename match is case-insensitive so a file saved as
// green.md still resolves on case-sensitive filesystems, mirroring the project
...
// prompt than the project's AGENTS.md/green.md (see buildSystemPrompt), and
...
func userGuidelines() string {
	...
	match := findCaseInsensitiveFile(filepath.Join(configDir, "green"), userContextFile)
	if match == "" {
		return ""
	}
	data, err := os.ReadFile(match)
	...
	return "## User guidelines (" + filepath.Base(match) + ")\n\n" +
		...
		"Where they conflict with a repository's project guidelines below (AGENTS.md/green.md), the project guidelines take precedence.\n\n" +
		content
}
