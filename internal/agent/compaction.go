...
// The BPE tokenizers green targets fold a run's leading space into the following
...
// staying allocation- and dependency-free. green still receives the exact count
...
func estimateTokens(messages []greenruntime.Message) int {
	total := 0
...
func estimateToolDefTokens(tools []greenruntime.ToolDefinition) int {
	total := 0
...
func Compact(messages []greenruntime.Message, opts CompactionOptions) ([]greenruntime.Message, error) {
...
func CompactMessages(messages []greenruntime.Message, opts CompactionOptions) (CompactionResult, error) {
	...
	systemEnd := 0
	for systemEnd < len(messages) && messages[systemEnd].Role == greenruntime.MessageRoleSystem {
		systemEnd++
	}
	...
	compacted := make([]greenruntime.Message, 0, systemEnd+1+(len(messages)-boundary))
	compacted = append(compacted, messages[:systemEnd]...)
	compacted = append(compacted, greenruntime.Message{
...
// systemEnd.
func safeSuffixBoundary(messages []greenruntime.Message, systemEnd int, boundary int) int {
	...
	for boundary > systemEnd && messages[boundary].Role != greenruntime.MessageRoleAssistant {
		boundary--
	}
	return boundary
}
...
type compactionState struct {
	...
	// later turns compact nearer to real capacity. green is treated as 1.0.
...
func (state *compactionState) maybeCompact(
	...
	// CHEAP FIRST STAGE: reclaim context at green token/latency cost by pruning
...
func (state *compactionState) recover(
	...
	tools []greenruntime.ToolDefinition,
	errorMessage string,
) (compacted []greenruntime.Message, retried bool, err error) {
	if !state.enabled {
...
func summarizeClosure(ctx context.Context, provider Provider, onUsage func(Usage)) func([]greenruntime.Message) (string, error) {
	return func(toSummarize []greenruntime.Message) (string, error) {
...
func summarizeWithFallback(ctx context.Context, provider Provider, messages []greenruntime.Message, onUsage func(Usage)) (string, error) {
	...
	reduced, reduceErr := summarizeMessagesOnce(ctx, provider, []greenruntime.Message{
		{Role: greenruntime.MessageRoleUser, Content: combined},
	}, onUsage)
	if reduceErr != nil {
...
func summarizeMessagesOnce(ctx context.Context, provider Provider, messages []greenruntime.Message, onUsage func(Usage)) (string, error) {
	request := greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleSystem, Content: summaryInstructions},
			{Role: greenruntime.MessageRoleUser, Content: "Summarize this conversation:\n\n" + renderTranscript(messages)},
		},
	...
	collected := greenruntime.CollectStreamWithOptions(ctx, stream, greenruntime.CollectOptions{OnUsage: onUsage})
	if collected.Error != "" {
...
func renderTranscript(messages []greenruntime.Message) string {
