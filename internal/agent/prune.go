...
// Stale tool-output pruning reclaims context at green token/latency cost before
...
func pruneStaleToolOutput(messages []greenruntime.Message, preserveLast int) ([]greenruntime.Message, int) {
	if len(messages) == 0 {
		return messages, 0
	}
	if preserveLast < 0 {
	...
	for i := len(messages) - 1; i >= 0; i-- {
		msg := messages[i]
		if msg.Role != greenruntime.MessageRoleTool {
			continue
		}
	...
	out := make([]greenruntime.Message, len(messages))
	copy(out, messages)
	reclaimed := 0
...
func toolNameForResult(messages []greenruntime.Message, index int) string {
