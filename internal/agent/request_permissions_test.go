...
func TestRequestPermissionsTurnGrantAllowsLaterToolAndCleansUp(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "grant-1", ToolName: tools.RequestPermissionsToolName},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "grant-1", ArgumentsFragment: `{"reason":"Need to write outside the workspace.","permissions":{"file_system":{"write":[` + quoteJSONString(target) + `]}}}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "grant-1"},
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "write-1", ToolName: "write_file"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "write-1", ArgumentsFragment: `{"path":` + quoteJSONString(target) + `,"content":"carrot\n","overwrite":true}`},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "write-1"},
...
func tempDirOutsideDefaultTemp(t *testing.T) string {
	t.Helper()
	dir, err := os.MkdirTemp(".", ".green-sandbox-outside-")
	if err != nil {
...
func requestPermissionsOnlyProvider(arguments string, finalAnswer string) *mockProvider {
	...
	return &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "grant-1", ToolName: tools.RequestPermissionsToolName},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "grant-1", ArgumentsFragment: arguments},
				{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "grant-1"},
				{Type: greenruntime.StreamEventDone},
			},
			{
				{Type: greenruntime.StreamEventText, Content: finalAnswer},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
