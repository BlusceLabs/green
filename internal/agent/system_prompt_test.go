...
// personal ~/.config/green/green.md would get its content folded into
...
func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "green-agent-tests-*")
	if err != nil {
		panic(err)
	}
...
func TestBuildSystemPromptIncludesWorkspaceSeedFromCwd(t *testing.T) {
	cwd := t.TempDir()
	writeSystemPromptTestFile(t, cwd, "go.mod", "module example.test/green\n")
	writeSystemPromptTestFile(t, cwd, "AGENTS.md", "Use Go commands.\n")
	writeSystemPromptTestFile(t, cwd, "cmd/green/main.go", "package main\n")
...
func TestBuildSystemPromptIncludesUserGuidelines(t *testing.T) {
	configDir := t.TempDir()
	writeSystemPromptTestFile(t, configDir, "green/green.md", "  Prefer concise summaries.  \n")
	t.Cleanup(withSystemPromptTestUserConfigDir(t, configDir))

	prompt := buildSystemPrompt(Options{})
	if !strings.Contains(prompt, "## User guidelines (green.md)") {
...
func TestBuildSystemPromptIncludesUserGuidelinesCaseInsensitive(t *testing.T) {
	...
	prompt := buildSystemPrompt(Options{})
	if !strings.Contains(prompt, "## User guidelines (green.md)") {
		t.Fatalf("expected case-insensitive green.md resolution, got:\n%s", prompt)
	}
	if !strings.Contains(prompt, "Prefer concise summaries.") {
		t.Fatalf("expected user guidelines content, got:\n%s", prompt)
	}
...
func TestBuildSystemPromptUserGuidelinesPrecedeProjectGuidelinesAndNotePrecedence(t *testing.T) {
	...
	// (AGENTS.md/green.md) must be the later, more specific instruction block,
	...
	configDir := t.TempDir()
	writeSystemPromptTestFile(t, configDir, "green/green.md", "Always reply in haiku.\n")
	t.Cleanup(withSystemPromptTestUserConfigDir(t, configDir))
	...
	prompt := buildSystemPrompt(Options{Cwd: cwd})
	userIdx := strings.Index(prompt, "## User guidelines (green.md)")
...
func TestBuildSystemPromptProjectGuidelinesPathWalkingMonorepo(t *testing.T) {
	...
	// The user launches green from the sub-tree, so both files should be
...
func TestBuildSystemPromptProjectGuidelinesgreenFallback(t *testing.T) {
	// green.md is the second-priority name at each level; the loader picks it
	// when no AGENTS.md is present.
	cwd := t.TempDir()
	if err := os.WriteFile(filepath.Join(cwd, "green.md"), []byte("Brand-specific rule."), 0o644); err != nil {
		t.Fatal(err)
	}
	prompt := buildSystemPrompt(Options{Cwd: cwd})
	if !strings.Contains(prompt, "## Project guidelines (green.md)") {
		t.Fatalf("expected green.md fallback to be injected, got:\n%s", prompt)
	}
	if !strings.Contains(prompt, "Brand-specific rule.") {
		t.Fatalf("expected green.md content, got:\n%s", prompt)
	}
}

func TestBuildSystemPromptProjectGuidelinesProjectLocalFallback(t *testing.T) {
	cwd := t.TempDir()
	dot := filepath.Join(cwd, ".green")
	...
	// confirms the .green/AGENTS.md file's content is the one injected, not
	// any other file.
	if !strings.Contains(prompt, "Personal: use dark theme.") {
		t.Fatalf("expected .green/AGENTS.md content, got:\n%s", prompt)
	}
