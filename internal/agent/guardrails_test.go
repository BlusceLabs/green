...
func emptyTurn() []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{{Type: greenruntime.StreamEventDone}}
}

// textTurn produces a turn with visible assistant text.
func textTurn(content string) []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventText, Content: content},
		{Type: greenruntime.StreamEventDone},
	}
}

// reasoningTurn produces live reasoning without visible assistant text.
func reasoningTurn(content string) []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventReasoning, Content: content},
		{Type: greenruntime.StreamEventDone},
	}
}

// toolTurn produces a turn that calls a named tool with the given args JSON.
func toolTurn(callID string, toolName string, args string) []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{
...
func countUserMessagesContaining(messages []greenruntime.Message, needle string) int {
	count := 0
	for _, message := range messages {
		if message.Role == greenruntime.MessageRoleUser && strings.Contains(message.Content, needle) {
			count++
		}
	}
	return count
...
func TestGuardStateResetsToolOnlyStreakOnEmptyNonToolTurn(t *testing.T) {
	...
	toolOnly := greenruntime.CollectedStream{
		ToolCalls: []greenruntime.ToolCall{{ID: "call", Name: "read_file", Arguments: `{}`}},
	}

	for range toolOnlyProgressReminderAt - 1 {
		state.observeTurn(toolOnly)
	}
	state.observeTurn(greenruntime.CollectedStream{})
	state.observeTurn(toolOnly)
...
func TestRunInjectsToolOnlyProgressReminder(t *testing.T) {
	...
	turns := make([][]greenruntime.StreamEvent, 0, toolOnlyProgressReminderAt+1)
	...
	for _, message := range provider.requests[toolOnlyProgressReminderAt].Messages {
		if message.Role == greenruntime.MessageRoleUser && strings.Contains(message.Content, toolOnlyProgressReminderMarker) {
			found = true
		}
	}
	if !found {
...
func repeatedFlakyTurns(n int) [][]greenruntime.StreamEvent {
	turn := []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "c", ToolName: "flaky"},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "c"},
		{Type: greenruntime.StreamEventDone},
	}
	turns := make([][]greenruntime.StreamEvent, 0, n)
	for i := 0; i < n; i++ {
...
func TestRunInjectsToolFailureHintWithSchema(t *testing.T) {
	...
	for _, m := range provider.requests[2].Messages {
		if m.Role == greenruntime.MessageRoleUser && strings.Contains(m.Content, toolFailureHintMarker) {
			found = true
