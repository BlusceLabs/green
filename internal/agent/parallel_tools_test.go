...
func probeCallEvents(callID, toolName, id string) []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: callID, ToolName: toolName},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: callID, ArgumentsFragment: fmt.Sprintf(`{"id":%q}`, id)},
...
func TestRunExecutesConsecutiveReadsConcurrently(t *testing.T) {
	...
	turnOne = append(turnOne, greenruntime.StreamEvent{Type: greenruntime.StreamEventDone})
