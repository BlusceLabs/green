...
type imageEchoProvider struct {
	seen []greenruntime.Message
}

func (p *imageEchoProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	if p.seen == nil {
		p.seen = append([]greenruntime.Message{}, request.Messages...)
	}
	events := make(chan greenruntime.StreamEvent)
	close(events)
	return events, nil
}
...
func TestRunSeedsImagesIntoUserTurn(t *testing.T) {
	...
	images := []greenruntime.ImageBlock{{MediaType: "image/png", Data: []byte{0x89, 0x50}}}
	...
	if user.Role != greenruntime.MessageRoleUser {
