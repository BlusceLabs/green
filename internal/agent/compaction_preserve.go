...
func extractLatestPlan(messages []greenruntime.Message) string {
...
func recentEdits(messages []greenruntime.Message) []skillEntry {
	...
	for _, message := range messages {
		if message.Role != greenruntime.MessageRoleTool || message.ToolCallID == "" {
			continue
		}
...
func loadedSkills(messages []greenruntime.Message) []skillEntry {
...
// result text. ToolResult.Meta is not part of greenruntime.Message history, so the
// rendered "Loaded N tools" output is the durable transcript format.
func loadedToolSchemas(messages []greenruntime.Message) []skillEntry {
	...
	for _, message := range messages {
		if message.Role != greenruntime.MessageRoleTool || !toolSearchIDs[message.ToolCallID] {
			continue
		}
...
func projectInstructionEntries(messages []greenruntime.Message) []skillEntry {
	bodyBySource := map[string]string{}
	sourceOrder := make([]string, 0)
	for _, message := range messages {
		if message.Role != greenruntime.MessageRoleUser {
			continue
		}
...
func appendPreservedState(summary string, middle []greenruntime.Message) string {
...
func latestSummaryContent(messages []greenruntime.Message) string {
	for i := len(messages) - 1; i >= 0; i-- {
		m := messages[i]
		if m.Role == greenruntime.MessageRoleUser && strings.HasPrefix(strings.TrimSpace(m.Content), summaryLabel) {
			return m.Content
		}
	}
	return ""
