...
// green value (escalation disabled by default).
...
func TestToolResultRequestedModelFieldExists(t *testing.T) {
	...
	// Keep the greenruntime import load-bearing so the file compiles standalone.
	_ = greenruntime.MessageRoleAssistant
}
...
func escalateThenAnswerTurns(answer string) [][]greenruntime.StreamEvent {
	return [][]greenruntime.StreamEvent{
		{
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "escalate"},
...
func TestRunSwitchesProviderOnEscalationRequest(t *testing.T) {
	...
	secondProvider := &mockProvider{turns: [][]greenruntime.StreamEvent{
		{
			{Type: greenruntime.StreamEventText, Content: "answered on the upgraded model"},
			{Type: greenruntime.StreamEventDone},
		},
	}}
...
func TestRunEscalationSwitcherErrorIsNonFatal(t *testing.T) {
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == greenruntime.MessageRoleUser && strings.Contains(strings.ToLower(m.Content), "could not switch") {
			sawNote = true
		}
	}
	if !sawNote {
...
func TestRunSwitchesAtMostOncePerTurn(t *testing.T) {
	...
	firstProvider := &mockProvider{turns: [][]greenruntime.StreamEvent{
		{
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "escalate"},
			{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "c1"},
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "c2", ToolName: "escalate"},
			{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "c2"},
			{Type: greenruntime.StreamEventDone},
		},
	}}
	secondProvider := &mockProvider{turns: [][]greenruntime.StreamEvent{
