...
func TestSelfReportedIncompletionMatching(t *testing.T) {
	cases := []struct {
		...
		{"You asked if I could work autonomously on green if you gave me a GitHub account — I was honest that my sandbox had no repo, no build system, and no network/integration wired up yet, so I couldn't actually do it at the time.", false},
