...
func TestToolResultLoadedToolsField(t *testing.T) {
	...
	// Default green value is nil for an ordinary result.
...
func toolDefinitionByName(definitions []greenruntime.ToolDefinition, name string) *greenruntime.ToolDefinition {
...
func assertNoDeferredDiscoveryMessage(t *testing.T, request greenruntime.CompletionRequest) {
	t.Helper()
	for _, message := range request.Messages {
		if message.Role == greenruntime.MessageRoleUser && strings.Contains(message.Content, "Deferred tools:") {
...
func legacyToolDefinitions(registry *tools.Registry, permissionMode PermissionMode, options Options) []greenruntime.ToolDefinition {
	definitions := make([]greenruntime.ToolDefinition, 0)
...
func TestRunLoadsDeferredToolThenAdvertisesNextTurn(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]greenruntime.StreamEvent{
		{ // turn 1: call load_signal
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "load_signal"},
	...
	for _, m := range result.Messages {
		if m.Role == greenruntime.MessageRoleUser && strings.Contains(m.Content, "Tool discovery") {
...
func TestRunReactiveRetryKeepsLoadedDeferredToolAndDiscovery(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]greenruntime.StreamEvent{
		...
		{ // turn 2: mid-stream context-limit error -> reactive compaction
			{Type: greenruntime.StreamEventError, Error: "prompt is too long: 250000 tokens > 200000 maximum"},
		},
	...
	var alpha *greenruntime.ToolDefinition
...
func (provider *connectErrorProvider) StreamCompletion(_ context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
