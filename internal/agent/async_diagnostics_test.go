...
func TestRunDeliversAsyncDiagnosticsNudgeNextTurn(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			{
				{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "fake_edit"},
				{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{}`},
	...
	for _, message := range provider.requests[1].Messages {
		if message.Role == greenruntime.MessageRoleTool && strings.Contains(message.Content, "boom") {
	...
	found := false
	for _, message := range provider.requests[1].Messages {
		if message.Role == greenruntime.MessageRoleUser && strings.HasPrefix(message.Content, asyncDiagnosticsNudge) {
...
func TestRunFinalizationGateDeliversLateDiagnostics(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]greenruntime.StreamEvent{
			...
			{
				{Type: greenruntime.StreamEventText, Content: "done after fix"},
				{Type: greenruntime.StreamEventDone},
			},
		},
	}
