...
func planUpdateTurn(status string) greenruntime.CollectedStream {
	return greenruntime.CollectedStream{ToolCalls: []greenruntime.ToolCall{
...
func TestPlanReminderFiresOnTurnStaleness(t *testing.T) {
	...
	for range stalePlanTurnThreshold {
		state.observeTurn(greenruntime.CollectedStream{Text: "still working…"})
	}
...
func TestPlanReminderSkipsTurnStalenessWhenComplete(t *testing.T) {
	...
	for range stalePlanTurnThreshold + 3 {
		state.observeTurn(greenruntime.CollectedStream{Text: "wrapping up…"})
	}
...
func TestPlanUpdateResetsTurnStaleness(t *testing.T) {
	...
	for range stalePlanTurnThreshold - 1 {
		state.observeTurn(greenruntime.CollectedStream{Text: "working…"})
	}
