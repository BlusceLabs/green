...
func Run(ctx context.Context, prompt string, provider Provider, options Options) (result Result, err error) {
	...
	toolDefCache := map[string]greenruntime.ToolDefinition{}
	...
	for turn := 0; turn < maxTurns; turn++ {
		...
		if nudge := postEditDiagnostics.drain(ctx); nudge != "" {
			messages = append(messages, greenruntime.Message{
		...
		recoverStreamError := func(collected greenruntime.CollectedStream) (greenruntime.CollectedStream, error) {
			...
			if compacted, retried, retryErr := compactor.recover(ctx, provider, messages, request.Tools, collected.Error); retried {
				...
				retryRequest := greenruntime.CompletionRequest{
					Messages:        copyMessages(messages),
					Tools:           exposed,
					ReasoningEffort: options.ReasoningEffort,
					PromptCacheKey:  options.SessionID,
		...
		collected := greenruntime.CollectStreamWithOptions(ctx, stream, forwardingOpts)
		if collected.Error != "" {
		...
		for attempt := 1; attempt <= maxStreamStallRetries &&
			...
			collected = greenruntime.CollectStreamWithOptions(ctx, retryStream, forwardingOpts)
		}
		if collected.Error != "" {
		...
		for index, call := range collected.ToolCalls {
			...
			messages = append(messages, greenruntime.Message{
				Role:       greenruntime.MessageRoleTool,
...
func finalAnswerAfterMaxTurns(ctx context.Context, provider Provider, messages []greenruntime.Message, toolDefs []greenruntime.ToolDefinition, options Options) (string, []greenruntime.Message, string) {
	finalMessages := copyMessages(messages)
	finalMessages = append(finalMessages, greenruntime.Message{
		Role:    greenruntime.MessageRoleUser,
	...
	stream, err := streamWithReconnect(ctx, provider, greenruntime.CompletionRequest{
	...
	collected := greenruntime.CollectStreamWithOptions(ctx, stream, greenruntime.CollectOptions{
...
func executeToolCall(ctx context.Context, registry *tools.Registry, call ToolCall, permissionMode PermissionMode, options Options) (ToolResult, error) {
	...
	// beforeTool hooks may veto the call before it runs (a non-green exit blocks).
	if toolFound {
	...
	result := registry.RunWithOptions(ctx, call.Name, args, tools.RunOptions{
		...
		// a file that changed on disk outside green since it was last read.
...
// that exits non-green vetoes the call: the returned bool is true and the tool
...
func partitionTools(registry *tools.Registry, permissionMode PermissionMode, options Options, loaded map[string]bool) ([]greenruntime.ToolDefinition, string) {
...
func partitionToolsCached(registry *tools.Registry, permissionMode PermissionMode, options Options, loaded map[string]bool, defCache map[string]greenruntime.ToolDefinition) ([]greenruntime.ToolDefinition, string) {
	...
	if !active {
		definitions := make([]greenruntime.ToolDefinition, 0, len(visible))
	...
	eager := make([]greenruntime.ToolDefinition, 0, len(visible))
	loadedTail := make([]greenruntime.ToolDefinition, 0)
	...
	definitions := make([]greenruntime.ToolDefinition, 0, len(eager)+1+len(loadedTail))
	definitions = append(definitions, eager...)
	definitions = append(definitions, greenruntime.ToolDefinition{
		Name:        loader.Name(),
...
func cachedRuntimeToolDefinition(defCache map[string]greenruntime.ToolDefinition, tool tools.Tool) greenruntime.ToolDefinition {
	if defCache == nil {
...
func runtimeToolDefinition(tool tools.Tool) greenruntime.ToolDefinition {
	return greenruntime.ToolDefinition{
		Name:        tool.Name(),
...
func copyMessages(messages []Message) []Message {
	...
	for index, message := range messages {
		...
		if message.Reasoning != nil {
			copied[index].Reasoning = append([]greenruntime.ReasoningBlock{}, message.Reasoning...)
		}
		...
		copied[index].Images = greenruntime.CloneImageBlocks(message.Images)
	}
	return copied
}
