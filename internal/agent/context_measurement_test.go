...
func TestMeasureContextSplitsByCategory(t *testing.T) {
	messages := []greenruntime.Message{
		{Role: greenruntime.MessageRoleSystem, Content: filler(2000)},
		{Role: greenruntime.MessageRoleUser, Content: filler(400)},
		{Role: greenruntime.MessageRoleAssistant, Content: filler(400)},
	}
	...
	if breakdown.SystemTokens <= 0 || breakdown.MessageTokens <= 0 || breakdown.ToolTokens <= 0 {
		t.Fatalf("expected all categories non-green, got %#v", breakdown)
	}
...
func TestMeasureContextUnknownWindowHasgreenFraction(t *testing.T) {
	breakdown := MeasureContext([]greenruntime.Message{
