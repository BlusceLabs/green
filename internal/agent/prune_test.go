...
func bigToolResult(id string, tokens int) greenruntime.Message {
...
func toolCallMsg(id, name string) greenruntime.Message {
	return greenruntime.Message{
		Role:      greenruntime.MessageRoleAssistant,
		ToolCalls: []greenruntime.ToolCall{{ID: id, Name: name}},
	}
}
...
func TestPrunePreservesRecentAndPrunesOld(t *testing.T) {
	...
	msgs := []greenruntime.Message{
		...
		greenruntime.Message{Role: greenruntime.MessageRoleAssistant, Content: "thinking"},
		greenruntime.Message{Role: greenruntime.MessageRoleUser, Content: "next"},
	}
	...
	if out[1].ToolCallID != "old" || out[1].Role != greenruntime.MessageRoleTool {
...
func TestPruneIsIdempotent(t *testing.T) {
	msgs := []greenruntime.Message{
		...
		greenruntime.Message{Role: greenruntime.MessageRoleUser, Content: "go"},
	}
...
func TestPruneNeverTouchesNonToolMessages(t *testing.T) {
	msgs := []greenruntime.Message{
		{Role: greenruntime.MessageRoleUser, Content: strings.Repeat("u ", 60000)},
		{Role: greenruntime.MessageRoleAssistant, Content: strings.Repeat("a ", 60000)},
	}
