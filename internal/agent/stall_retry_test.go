...
func (p *stallProvider) StreamCompletion(_ context.Context, _ greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	n := atomic.AddInt32(&p.calls, 1)
	ch := make(chan greenruntime.StreamEvent, 5)
	if n <= p.stallBefore {
		if p.partialText != "" {
			ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: p.partialText}
		}
		if p.reasoningText != "" {
			ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventReasoning, Content: p.reasoningText}
		}
		if p.partialToolCall != "" {
			...
			ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "tc_1", ToolName: p.partialToolCall}
			ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "tc_1", ArgumentsFragment: `{"path":"x.html","content":"<!doctype`}
		}
		ch <- greenruntime.StreamEvent{
