...
func TestRecentEditsExtractsPathsAndNotes(t *testing.T) {
	messages := []greenruntime.Message{
		...
		{Role: greenruntime.MessageRoleTool, ToolCallID: "e1", Content: "Wrote internal/foo.go (12 lines)"},
		{Role: greenruntime.MessageRoleAssistant, ToolCalls: []greenruntime.ToolCall{
			{ID: "e2", Name: "edit_file", Arguments: `{"path":"internal/bar.go","old_string":"a","new_string":"b"}`},
		}},
		{Role: greenruntime.MessageRoleTool, ToolCallID: "e2", Content: "Applied edit to internal/bar.go"},
	}
...
func TestCompactionPreservesRecentEdits(t *testing.T) {
	messages := []greenruntime.Message{
		...
		{Role: greenruntime.MessageRoleUser, Content: "add a flag"},
		{Role: greenruntime.MessageRoleAssistant, Content: "editing", ToolCalls: []greenruntime.ToolCall{
			{ID: "e1", Name: "write_file", Arguments: `{"path":"cmd/main.go","content":"..."}`},
		}},
		{Role: greenruntime.MessageRoleTool, ToolCallID: "e1", Content: "Wrote cmd/main.go (adds --version flag)"},
		{Role: greenruntime.MessageRoleAssistant, Content: "done"},
