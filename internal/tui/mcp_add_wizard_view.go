...
func (wizard *mcpAddWizardState) render(width int) string {
	...
	lines := []string{
		greenTheme.faint.Render(mcpAddWizardStepLine(wizard.step)),
	...
	block := styledBlockFillTitle(overlayWidth, "Add MCP Server", lines, greenTheme.lineStrong, lipgloss.NewStyle())
...
func (wizard *mcpAddWizardState) renderNameStep(width int) []string {
	...
	return []string{
		greenTheme.accent.Render("Server Name"),
		fitStyledLine(greenTheme.ink.Render("> "+value), width),
		greenTheme.faint.Render("Default: " + mcpAddWizardTypes[clampInt(wizard.selectedType, 0, len(mcpAddWizardTypes)-1)].Label),
		greenTheme.faint.Render("Use lowercase letters, numbers, dashes, or underscores."),
	}
}

func (wizard *mcpAddWizardState) renderTypeStep(width int) []string {
	lines := []string{greenTheme.accent.Render("Server Type")}
	for index, item := range mcpAddWizardTypes {
		...
		lines = append(lines, fillPaletteLine(surface(greenTheme.ink).Render(line), width, surface))
	}
	return lines
}
...
func (wizard *mcpAddWizardState) renderEndpointStep(width int) []string {
	...
	return []string{
		greenTheme.accent.Render(title),
		fitStyledLine(greenTheme.ink.Render("> "+value), width),
	}
}
...
func (wizard *mcpAddWizardState) renderHeaderStep(width int) []string {
	...
	return []string{
		greenTheme.accent.Render("Add header"),
		fitStyledLine(greenTheme.ink.Render("> "+redaction.RedactString(value, redaction.Options{})), width),
		greenTheme.faint.Render(`Paste "Key: Value" or "Key=Value".`),
	}
}

func (wizard *mcpAddWizardState) renderConfirmStep(width int) []string {
	lines := []string{
		greenTheme.accent.Render("Review setup"),
		"server: " + greenTheme.ink.Render(wizard.serverName),
		"type: " + greenTheme.ink.Render(strings.ToUpper(wizard.serverType)),
	}
	if source := strings.TrimSpace(wizard.sourceLabel); source != "" {
		lines = append(lines, "source: "+greenTheme.ink.Render(source))
	}
	if sourceURL := strings.TrimSpace(wizard.sourceURL); sourceURL != "" {
		lines = append(lines, "docs: "+greenTheme.ink.Render(redactMCPWizardDisplayValue(sourceURL)))
	}
	if wizard.isRemote() {
		lines = append(lines, "url: "+greenTheme.ink.Render(redactMCPWizardDisplayValue(wizard.endpoint)))
		if wizard.headerKey != "" {
			lines = append(lines, "header: "+greenTheme.ink.Render(wizard.headerKey+"=[REDACTED]"))
		}
	} else {
		lines = append(lines, "command: "+greenTheme.ink.Render(redactMCPWizardCommand(wizard.endpoint)))
	}
	if len(wizard.prerequisites) > 0 {
		lines = append(lines, "", greenTheme.accent.Render("Needs"))
		for _, item := range wizard.prerequisites {
			lines = append(lines, "  - "+greenTheme.ink.Render(item))
		}
	}
	lines = append(lines, "", greenTheme.faint.Render("Enter saves and tests the server."))
...
func (wizard *mcpAddWizardState) renderResultStep(width int) []string {
	...
	lines := []string{
		greenTheme.accent.Render(title),
		greenTheme.ink.Bold(true).Render(displayValue(wizard.serverName, "unnamed")) + "  " + greenTheme.faint.Render(state),
		greenTheme.faint.Render(transport),
	}
	if result.Message != "" {
		lines = append(lines, fitStyledLine(greenTheme.red.Render(result.Message), width))
	}
	if !result.Saved {
		lines = append(lines, greenTheme.faint.Render("No config was saved yet."))
	} else {
