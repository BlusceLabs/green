...
// green_THEME, then the persisted config theme. A value is accepted if it is `auto`
...
// ValidThemeArg reports whether s is an acceptable --theme / green_THEME value
...
// applyTheme swaps the active palette (greenTheme) and the globals derived from it
...
// program starts), like every other greenTheme access.
func applyTheme(mode themeMode, hasDarkBackground bool) themeMode {
	...
	// falls back to the dark built-in so a bad value can never leave greenTheme unset.
	entry, ok := lookupTheme(string(resolved))
	if !ok {
		entry, _ = lookupTheme(string(themeDark))
	}
	greenTheme = buildTheme(entry.Palette)
...
// and never strands on a stale preview. It only swaps the global greenTheme via
