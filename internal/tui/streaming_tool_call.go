...
func (m model) streamingToolCallView(width int) string {
	...
	head := toolCardHead(m.streamCallName, d.path, "", headTag, "", "", true, greenTheme.ink, false, width, cardRenderOptions{cwd: m.cwd})
	lines := []string{greenTheme.accent.Render(m.spinnerGlyph()) + " " + head}
