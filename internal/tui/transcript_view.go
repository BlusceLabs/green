...
func (m model) detailedTranscriptFooter(width int) string {
	if copyStatus := strings.TrimSpace(m.copyStatus); copyStatus != "" {
		return rightAlignedLine(greenTheme.ink.Render(copyStatus), width)
	}

	detailKey := labelOr(m.keyBindings.toggleDetailed, "Ctrl+O")
	hint := greenTheme.faint.Render(fmt.Sprintf("Esc close | %s toggle", detailKey))
...
func detailedTranscriptHeader(width int) string {
	title := greenTheme.ink.Bold(true).Render("Transcript")
	hint := greenTheme.faint.Render("detailed")
