...
func TestDisplayPathAbbreviatesHome(t *testing.T) {
	...
	abs := filepath.Join(home, "projects", "green", "main.go")
	if got := displayPath(t.TempDir(), abs); got != "~/projects/green/main.go" {
		t.Fatalf("displayPath under home = %q, want ~/projects/green/main.go", got)
	}
}
...
func TestDisplayPathLeavesRelativeInputUntouched(t *testing.T) {
	if got := displayPath("/work/green", "examples/calc.go"); got != "examples/calc.go" {
...
func TestRedundantConfirmationCollapsesSuccessBody(t *testing.T) {
	...
	row := transcriptRow{
		...
		detail: "Successfully created directory /work/green/examples/calc",
	}
