...
func TestProviderWizardAdvancesProviderAPIKeyAndModelSteps(t *testing.T) {
	...
	assertNotContains(t, view, "green providers add anthropic")
	...
	assertNotContains(t, view, "green providers check")
}
...
func TestProviderWizardPersistsPastedKeyToUserConfig(t *testing.T) {
	...
	t.Setenv("green_CRED_STORAGE", "encrypted-file")
...
// green_PROVIDER exactly like the /model and /provider switch paths — a stale
...
func TestApplyProviderWizardExportsActiveProviderEnv(t *testing.T) {
	...
	m.newProvider = func(config.ProviderProfile) (greenruntime.Provider, error) {
		return &fakeProvider{}, nil
	}
	m.providerWizard = &providerWizardState{
		step:        providerWizardStepModel,
...
// status line and the green_PROVIDER export (which pins spawned children) still
...
func TestApplyProviderWizardPersistFailureLeavesLiveStateUnchanged(t *testing.T) {
	t.Setenv("green_CRED_STORAGE", "encrypted-file") // never touch the real OS keychain: apiKey is secured before the persist fails
	...
	m.newProvider = func(config.ProviderProfile) (greenruntime.Provider, error) { return newProvider, nil }
	m.providerWizard = &providerWizardState{
		step:        providerWizardStepModel,
...
func TestProviderSearchEmptyMatchEnterIsNoop(t *testing.T) {
	...
	if len(next.providerWizard.filteredProviders()) != 0 {
		t.Fatal("expected green filtered providers")
	}
