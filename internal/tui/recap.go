...
// user turn, no tools.
func generateRecap(ctx context.Context, provider greenruntime.Provider, answer string) (string, error) {
	if provider == nil {
	...
	request := greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleSystem, Content: recapSystemPrompt},
			{Role: greenruntime.MessageRoleUser, Content: "Assistant's final answer:\n\n" + cutRunes(answer, recapMaxAnswerChars) + "\n\nOne-sentence recap:"},
		},
	}
