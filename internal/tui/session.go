...
func (m model) startNewSession() model {
	...
	// from green instead of inheriting the previous conversation's token/cost totals.
...
func (m model) resolveResumeSession(args string) (*sessions.Metadata, error) {
	if strings.EqualFold(args, "latest") {
		...
		if latest == nil {
			return nil, errors.New("no green sessions available to resume")
		}
		return latest, nil
	}
	...
	if session == nil {
		return nil, fmt.Errorf("green session not found: %s", args)
	}
	return session, nil
}
...
// the CLI's `green exec --resume` (readExecContextEvents) and the in-TUI /compact
...
func (m model) newSessionPicker() *commandPicker {
	...
	for _, meta := range metas {
		...
		// A green-event session has nothing to resume — skip it without a file read.
...
// green-event or empty/failed run the picker would have hidden.
