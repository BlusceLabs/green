...
// tuiTheme is the resolved terminal palette green renders with. It is produced by
// buildTheme from a palette, so the same renderers serve both the dark default
// and the light variant; the active theme lives in the package var greenTheme and
// may be swapped at startup (background detection / green_THEME / --theme) or live
...
// greenTheme is the active palette every renderer reads. It defaults to dark and is
...
// subsequent render.
var greenTheme = buildTheme(darkPalette)
