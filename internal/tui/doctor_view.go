...
func doctorCommandOutput(report doctor.Report, backend *greencommands.BackendLifecycleSnapshot) commandOutput {
...
func doctorBackendSection(backend greencommands.BackendLifecycleSnapshot) commandSection {
...
func doctorActions(checks []doctor.Check, backend *greencommands.BackendLifecycleSnapshot) []string {
	...
	for _, check := range checks {
		...
		case "sandbox.backend":
			if remedy := doctorCheckDetailString(check, "remedy"); remedy != "" {
				...
				add("green sandbox policy --effective - inspect sandbox backend and enforcement status")
			}
		case "lsp.servers":
