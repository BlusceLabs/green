...
// explicit accessibility/preference switch via green_REDUCED_MOTION, and is also
...
// whereas green_NO_FADE only governs the streaming-text fade.
func reducedMotionEnabled(env func(string) string, profile colorprofile.Profile) bool {
	if v := strings.TrimSpace(env("green_REDUCED_MOTION")); v != "" && v != "0" && !strings.EqualFold(v, "false") {
		return true
	}
...
// init builds the palette once at package load (after greenTheme's var init), so
...
func streamingLineBornAt(visualIndex, visualCount int, lineAges []time.Time, lastActivity time.Time) time.Time {
	...
	// siblings instead of snapping to the green time (= base ink).
	if visualIndex < 0 {
		return time.Time{}
	}
...
func (m model) styleStreamingLine(line string, visualIndex, visualCount int) string {
	...
	if !m.fadeActive || m.lineAges == nil {
		return greenTheme.ink.Render(line)
	}
	bornAt := streamingLineBornAt(visualIndex, visualCount, m.lineAges, m.lastStreamActivity)
	return ageDimLine(line, bornAt, m.now(), greenTheme.ink)
}
