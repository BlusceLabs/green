...
type scriptedProvider struct {
	scripts  [][]greenruntime.StreamEvent
...
func (provider *scriptedProvider) StreamCompletion(
	...
	if len(provider.scripts) == 0 {
		ch := make(chan greenruntime.StreamEvent)
		close(ch)
		return ch, nil
	}
	...
	ch := make(chan greenruntime.StreamEvent, len(provider.scripts[index]))
...
func TestPromptSubmitPersistsTUISessionEvents(t *testing.T) {
	...
	provider := &fakeProvider{events: []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventText, Content: "saved"},
		{Type: greenruntime.StreamEventUsage, Usage: greenruntime.Usage{InputTokens: 10, OutputTokens: 4}},
...
func TestPromptSubmitPersistsToolSessionEvents(t *testing.T) {
	...
	provider := &scriptedProvider{scripts: [][]greenruntime.StreamEvent{
		{
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call_1", ToolName: "read_file"},
			{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call_1", ArgumentsFragment: `{"path":"notes.txt"}`},
			{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call_1"},
			{Type: greenruntime.StreamEventDone},
		},
		{
			{Type: greenruntime.StreamEventText, Content: "read complete"},
			{Type: greenruntime.StreamEventDone},
		},
	}}
	registry := tools.NewRegistry()
...
func TestPromptSubmitPersistsPermissionSessionEvents(t *testing.T) {
	...
	provider := &scriptedProvider{scripts: [][]greenruntime.StreamEvent{
		{
			{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call_write", ToolName: "write_file"},
			{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call_write", ArgumentsFragment: `{"path":"notes.txt","content":"hello"}`},
			{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call_write"},
			{Type: greenruntime.StreamEventDone},
		},
		{
			{Type: greenruntime.StreamEventText, Content: "write blocked"},
			{Type: greenruntime.StreamEventDone},
		},
	}}
	registry := tools.NewRegistry()
...
func newPermissionTestModel(root string, provider greenruntime.Provider, registry *tools.Registry, store *sessions.Store, grantStore *sandbox.GrantStore, runtimeMessages chan<- tea.Msg) model {
...
func writeFileToolScript(callID string, path string, content string) []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: callID, ToolName: "write_file"},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: callID, ArgumentsFragment: `{"path":` + jsonString(path) + `,"content":` + jsonString(content) + `,"overwrite":true}`},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: callID},
		{Type: greenruntime.StreamEventDone},
	}
}

func textScript(text string) []greenruntime.StreamEvent {
	return []greenruntime.StreamEvent{
...
func TestResumeCommandHydratesSessionTranscript(t *testing.T) {
	...
	appendTestEvent(t, store, session.SessionID, sessions.EventToolCall, map[string]any{"id": "call_1", "name": "grep", "arguments": `{"pattern":"green"}`})
	...
	for _, want := range []string{"Resumed green session", session.SessionID, "previous request", "tool call: grep", "permission: grep allow", "tool result: grep error matches", "previous answer", "old error"} {
		if !transcriptContains(next.transcript, want) {
	...
	if !ok || toolCall.tool != "grep" || toolCall.detail != "green" {
	...
	if transcriptContains(next.transcript, "green exec --resume") {
...
func TestEscCancelRecordsSessionError(t *testing.T) {
	store := testSessionStore(t)
	provider := &fakeProvider{events: []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventText, Content: "ignored"},
		{Type: greenruntime.StreamEventDone},
	}}
	m := newModel(context.Background(), Options{
		Provider:     provider,
...
func TestCtrlCCancelsAndFlushesCheckpointSessionEvents(t *testing.T) {
	...
	// The cancelled goroutine still returns its accumulated session events. Deliver
	// that final message; the checkpoint must be persisted even though activeRunID
	// is already greened, and the second Ctrl+C's deferred quit must fire now.
	finalMsg := receiveFinalMessage(t, finalCh)
...
func TestResumedPromptIncludesSessionContext(t *testing.T) {
	...
	appendTestEvent(t, store, session.SessionID, sessions.EventMessage, map[string]any{"role": "assistant", "content": "previous answer"})
	provider := &scriptedProvider{scripts: [][]greenruntime.StreamEvent{{
		{Type: greenruntime.StreamEventText, Content: "continued"},
		{Type: greenruntime.StreamEventDone},
	}}}
	m := newModel(context.Background(), Options{
		Provider:     provider,
	...
	for _, want := range []string{"Continuing green session", session.SessionID, "previous request", "previous answer", "Current user request:", "continue"} {
...
func TestResumeCommandReportsMissingSession(t *testing.T) {
	...
	if !transcriptContains(next.transcript, "green session not found: missing_session") {
