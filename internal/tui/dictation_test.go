...
func TestDictationAuthErrorReopensKeyPrompt(t *testing.T) {
	...
	// provider (preserving its model), not dead-end on "run green auth".
	m := model{}
...
func TestCurrentModelLabel(t *testing.T) {
	...
	d = dictationController{cfg: config.STTConfig{Provider: config.STTProviderLocal, LocalModelPath: "/home/x/.config/green/stt/" + kroko.DirName + "/sherpa-onnx-...-kroko"}}
