...
func renderMCPView(state MCPViewState, width int) string {
	...
	if !hasMCPViewContent(state) {
		lines = append(lines,
			...
			"  › Add MCP server      green mcp add <name> --url <url>",
			"  › Add local stdio MCP green mcp add <name> -- <command> [args...]",
			"  › List configured     green mcp list",
			"",
			"Actions",
	...
	lines = append(lines,
		"",
		"Actions",
		"  add: green mcp add <name> --url <url>",
		"  disconnect: green mcp disable <name>",
		"  reconnect: green mcp enable <name>",
		"  remove: green mcp remove <name>",
	)
	return fitMCPManagerLines(lines, width)
}
...
func mcpServerActionLine(server MCPServerView) string {
	...
	actions := []string{"green mcp check " + name}
	if strings.EqualFold(strings.TrimSpace(server.State), "disabled") {
		actions = append(actions, "green mcp enable "+name)
	} else {
		actions = append(actions, "green mcp disable "+name)
	}
	actions = append(actions, "green mcp remove "+name)
	if strings.EqualFold(strings.TrimSpace(server.Auth), "oauth") {
		actions = append(actions, "green mcp oauth login "+name)
	}
...
func mcpOAuthActionLine(server MCPOAuthServerView) string {
	...
	actions := []string{}
	if server.HasToken {
		actions = append(actions, "green mcp oauth refresh "+name, "green mcp oauth logout "+name)
	} else {
		actions = append(actions, "green mcp oauth login "+name)
	}
	if server.Expired && !server.HasRefreshToken {
		actions = append(actions, "green mcp oauth refresh "+name)
	}
