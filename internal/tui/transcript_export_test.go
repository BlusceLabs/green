...
func TestPlainTranscriptTextSkipsNoise(t *testing.T) {
	...
	if !strings.Contains(out, "you: add a flag") || !strings.Contains(out, "green: Done — added --version.") {
...
func TestHandleExportCommandWritesFile(t *testing.T) {
	...
	if !strings.Contains(string(data), "you: hello") || !strings.Contains(string(data), "green: hi there") {
	...
	if msg := m.handleExportCommand(""); !strings.Contains(msg, "green-transcript-20260704-093000.txt") {
