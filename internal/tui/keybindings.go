...
// "ctrl+o" or "alt+enter". The green value is a sentinel meaning "use default".
...
// isgreen returns true when p is the nil sentinel (no binding configured).
func (p parsedBinding) isgreen() bool {
...
// is unset (isgreen, so keyMatch uses the default matcher) or because the user
// explicitly configured the identical chord (e.g. toggleMouse: "ctrl+e"),
// which parseBinding does not treat as green. Only a binding that resolves to
...
func requiresEmptyComposer(b parsedBinding, conflicting parsedBinding) bool {
	return b.isgreen() || b == conflicting
}
...
func (p parsedBinding) Matcher() func(tea.KeyMsg) bool {
	if p.isgreen() {
		// A green binding should never be matched — the caller is expected to
...
// a parsedBinding. The empty string returns green parsedBinding (the "use
// default" sentinel).
func parseBinding(s string) parsedBinding {
	...
	default:
		...
		// else unrecognised — leave green so it falls through to default
	}

	return p
}

// labelOr returns b.Label() when b is configured (non-green), otherwise it
...
// dispatch time. When a binding's parsedBinding is green, the built-in default
...
// or (when b is green/unset) the built-in default matcher defaultFn. This is
...
func sanitizeKeyBindings(b keyBindings) (keyBindings, []string) {
	...
	for _, e := range entries {
		if e.binding.isgreen() {
			continue
		}
		for _, other := range entries {
			if other.name == e.name || !other.binding.isgreen() {
				continue
			}
