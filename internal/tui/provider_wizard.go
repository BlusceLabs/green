...
import (
	...
	"github.com/BlusceLabs/green/internal/browser"
	...
	"github.com/BlusceLabs/green/internal/provideroauth"
...
// `green providers use`, which reads as the login having vanished.
//
...
// `green` launch from a TUI session can use the baked-in ChatGPT client_id
// without requiring the user to export green_OAUTH_ALLOW_PRESETS themselves.
func buildOAuthPresetEnv() map[string]string {
	...
	env["green_OAUTH_ALLOW_PRESETS"] = "1"
	return env
}
...
func (m model) applyProviderWizard() (model, tea.Cmd) {
	...
	// and the green_PROVIDER export (which pins spawned children) still point at
	// the old one — parent and children would silently run on different providers.
	var nextProvider greenruntime.Provider
	...
	// this, a green_PROVIDER exported by an earlier switch stays pointing at the
...
func (wizard *providerWizardState) render(width int) string {
	...
	lines := []string{
		greenTheme.faint.Render(providerWizardStepLine(wizard)),
		greenTheme.line.Render(strings.Repeat("─", innerWidth)),
	}
	if wizard.err != "" {
		lines = append(lines, greenTheme.red.Render("error: "+wizard.err), "")
	}
	if wizard.oauthPending {
		lines = append(lines, wizard.renderOAuthWaiting(innerWidth)...)
		lines = append(lines,
			greenTheme.line.Render(strings.Repeat("─", innerWidth)),
			greenTheme.faint.Render("Esc cancel"),
		)
		block := styledBlockFillTitle(overlayWidth, "Provider setup", lines, greenTheme.lineStrong, lipgloss.NewStyle())
	...
	lines = append(lines,
		greenTheme.line.Render(strings.Repeat("─", innerWidth)),
		greenTheme.faint.Render(wizard.footer()),
	)
	...
	block := styledBlockFillTitle(overlayWidth, title, lines, greenTheme.lineStrong, lipgloss.NewStyle())
	if width > overlayWidth {
		return indentBlock(block, (width-overlayWidth)/2)
	}
	return block
...
func (wizard *providerWizardState) renderMethodStep(width int) []string {
	options := providerWizardMethodOptions()
	wizard.selectedMethod = clampInt(wizard.selectedMethod, 0, maxInt(0, len(options)-1))
	lines := []string{greenTheme.accent.Render("How do you want to connect?")}
	for index, option := range options {
		surface := transparentSurface
		marker := surface(greenTheme.faintest).Render("  ")
		if index == wizard.selectedMethod {
			surface = greenTheme.onSel
			marker = surface(greenTheme.accent).Render("❯ ")
		}
		lines = append(lines, fitStyledLine(marker+surface(greenTheme.ink).Render(option.label), width))
		lines = append(lines, fitStyledLine("    "+greenTheme.faint.Render(option.subtitle), width))
	}
	return lines
}
...
func (wizard *providerWizardState) renderManageKeyStep(width int) []string {
	...
	lines := []string{greenTheme.accent.Render(name + " already has a saved key")}
	for index, option := range options {
		surface := transparentSurface
		marker := surface(greenTheme.faintest).Render("  ")
...
func (wizard *providerWizardState) renderOAuthWaiting(width int) []string {
	...
	if wizard.oauthDevice {
		lines := []string{
			greenTheme.accent.Render("Device-code sign-in for " + name),
			"",
		}
		if wizard.deviceUserCode == "" {
			return append(lines, fitStyledLine(greenTheme.faint.Render("Requesting a device code..."), width))
		}
		return append(lines,
			fitStyledLine(greenTheme.ink.Render("1. On any device, visit:  ")+greenTheme.accent.Render(wizard.deviceVerificationURI), width),
			fitStyledLine(greenTheme.ink.Render("2. Enter the code:  ")+greenTheme.accent.Bold(true).Render(wizard.deviceUserCode), width),
			"",
			fitStyledLine(greenTheme.faint.Render("Waiting for authorization..."), width),
		)
	}
	return []string{
		greenTheme.accent.Render("Signing in with " + name),
		"",
		fitStyledLine(greenTheme.ink.Render("Opening your browser — approve there, then return here."), width),
		fitStyledLine(greenTheme.faint.Render("Waiting for authorization..."), width),
		"",
		fitStyledLine(greenTheme.faint.Render("If your browser didn't open, run:  "+providerWizardOAuthCLIHint(provider)), width),
	}
}
...
func providerWizardOAuthCLIHint(provider providercatalog.Descriptor) string {
	if provider.OAuthMintsKey {
		return "green auth openrouter"
	}
	return "green auth login " + provider.ID
}
...
func (wizard *providerWizardState) renderProviderStep(width int) []string {
	...
	lines := []string{greenTheme.accent.Render(header)}
	lines = append(lines, wizard.renderProviderSearch(width))
	providers := wizard.filteredProviders()
	if len(providers) == 0 {
		lines = append(lines, greenTheme.faint.Render("  no matching providers"))
		return lines
	}
	...
	if wizard.oauthMode && wizard.oauthErr != "" {
		lines = append(lines, "", greenTheme.red.Render("OAuth login failed: "+wizard.oauthErr))
		if hint := providerWizardOAuthErrHint(wizard.currentProvider()); hint != "" {
			lines = append(lines, greenTheme.faint.Render(hint))
		}
	}
	return lines
}
...
func providerWizardOAuthErrHint(provider providercatalog.Descriptor) string {
	if strings.EqualFold(provider.ID, "huggingface") {
		return "Hugging Face needs your own app: create one at " +
			"https://huggingface.co/settings/applications/new, then set green_OAUTH_HUGGINGFACE_CLIENT_ID."
	}
	return ""
}
...
func (wizard *providerWizardState) renderSelectableProvider(width int, index int, provider providercatalog.Descriptor) string {
	...
	left := marker + surface(greenTheme.ink).Render(name)
	if badge := providerWizardBadge(provider); badge != "" {
		left += surface(greenTheme.faint).Render("   " + badge)
	}
...
func (wizard *providerWizardState) renderEndpointStep(width int) []string {
	...
	return []string{
		greenTheme.accent.Render("Endpoint URL"),
		greenTheme.ink.Render("Enter the API base URL for " + provider.Name + "."),
		greenTheme.faint.Render(providerWizardEndpointHint(provider)),
		input,
	}
}
...
func (wizard *providerWizardState) renderNameStep(width int) []string {
	...
	return []string{
		greenTheme.accent.Render("Provider name"),
		greenTheme.ink.Render("Choose the short label shown in the status bar."),
		greenTheme.faint.Render("Leave blank to use " + name + "."),
...
func (wizard *providerWizardState) renderCredentialStep(width int) []string {
	...
	value := greenTheme.accent.Render("▌") + greenTheme.faint.Render("paste key here")
	if wizard.apiKey != "" {
		value = greenTheme.ink.Render(maskedProviderWizardKey(wizard.apiKey)) + greenTheme.accent.Render("▌")
	}
	input := greenTheme.userPrompt.Render("api key > ") + value
	lines := []string{
		greenTheme.accent.Render("Paste API key"),
		greenTheme.ink.Render(providerWizardCredentialInstruction(env)),
		input,
		greenTheme.faint.Render("Pasted keys are hidden and saved in your user config."),
	}
	if oauth {
		lines = append(lines, greenTheme.accent.Render("or  ctrl+o  to log in with OAuth in the browser (no key needed)"))
	}
	if wizard.oauthErr != "" {
		lines = append(lines, greenTheme.red.Render("OAuth login failed: "+wizard.oauthErr))
	}
...
func (wizard *providerWizardState) renderModelStep(width int) []string {
	...
	lines := []string{greenTheme.accent.Render("Choose a model")}
	if status := wizard.modelStatusText(); status != "" {
		lines = append(lines, greenTheme.faint.Render(status))
	}
	lines = append(lines, wizard.renderModelSearch(width))
	wizard.refreshModels()
	models := wizard.filteredModels()
	if len(models) == 0 {
		lines = append(lines, greenTheme.faint.Render("  no matching models"))
		return lines
	}
	...
	if detail := providerWizardModelDetail(wizard.currentModel()); detail != "" {
		lines = append(lines, fitStyledLine(greenTheme.faint.Render("  "+detail), width))
	}
	return lines
}
...
func (wizard *providerWizardState) renderModelLoadingStep(width int) []string {
	return []string{
		greenTheme.accent.Render("Choose a model"),
		"",
		fitStyledLine(greenTheme.faint.Render("Checking available models..."), width),
		fitStyledLine(greenTheme.faint.Render("Built-in models will be used if discovery fails."), width),
	}
}
...
func providerWizardInputLine(promptText string, value string, placeholder string, width int) string {
	prompt := greenTheme.userPrompt.Render("search > ")
	cursor := greenTheme.accent.Render("▌")
	if promptText != "" {
		prompt = greenTheme.userPrompt.Render(promptText)
	}
	if value == "" {
		return fitStyledLine(prompt+cursor+greenTheme.faint.Render(placeholder), width)
	}
	return fitStyledLine(prompt+greenTheme.ink.Render(value)+cursor, width)
}

func (wizard *providerWizardState) renderTypedModelStep(width int) []string {
	provider := wizard.currentProvider()
	return []string{
		greenTheme.accent.Render("Model name"),
		greenTheme.ink.Render("Enter the model ID this endpoint expects."),
		greenTheme.faint.Render("Examples: gpt-4.1, claude-sonnet-4-5, llama-3.3-70b"),
...
func (wizard *providerWizardState) renderSelectableModel(width int, index int, model providerWizardModel) string {
	...
	left := marker + surface(greenTheme.ink).Render(model.displayLabel())
	return fitStyledLine(left, width)
}
...
func (wizard *providerWizardState) renderDoneStep(width int) []string {
	...
	lines := []string{
		greenTheme.accent.Render("Ready to connect"),
		"",
		greenTheme.ink.Render("Provider    " + provider.Name),
	}
	if providerWizardNeedsEndpoint(provider) {
		lines = append(lines, greenTheme.ink.Render("Endpoint    "+strings.TrimSpace(wizard.baseURL)))
	}
	lines = append(lines,
		greenTheme.ink.Render("Name        "+providerWizardDisplayName(provider, wizard.baseURL, wizard.profileName)),
		greenTheme.ink.Render("Model       "+model.ID),
		greenTheme.ink.Render("Credential  "+providerWizardCredentialLabel(provider, wizard.apiKey)),
		"",
		greenTheme.faint.Render("Press Enter to save and start using this provider."),
	)
	return lines
}
