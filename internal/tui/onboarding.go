...
func (m model) completeSetup() (tea.Model, tea.Cmd) {
	...
	if result.Provider.Name != "" {
		...
		// Export green_PROVIDER alongside the committed profile fields (and the
		...
		// them unconditionally too: applyEnv makes green_PROVIDER WIN over config,
...
func (m model) setupView(width int) string {
	...
	if m.setup.err != "" {
		content = append(content, "", greenTheme.red.Render("error: "+m.setup.err))
		if hint := m.setupErrorAffordance(); hint != "" {
			content = append(content, greenTheme.faint.Render(hint))
		}
	}
...
func (m model) setupStageLines(width int, height int) []string {
	...
	case setupStageSafety:
		return []string{
			greenTheme.ink.Bold(true).Render("Safety"),
			"",
			"green asks before running shell commands or changing files.",
			"Unsafe mode stays off unless you explicitly enable it.",
			"",
			greenTheme.faint.Render("Default: ask before risky work."),
		}
	case setupStageReady:
		return m.setupReadyLines(width)
	default:
		return []string{
			greenTheme.accent.Render("Welcome to green"),
			"",
			greenTheme.ink.Render("A terminal agent for changing real code."),
			greenTheme.faint.Render("Plan changes, edit with approval, run checks, and resume sessions."),
		}
	}
}
...
func (m model) setupReadyLines(width int) []string {
	...
	lines := []string{
		greenTheme.ink.Bold(true).Render("Ready"),
		"",
		"green will save this setup and open chat.",
		"",
	}
	...
	for _, row := range rows {
		...
		line := "  " + greenTheme.faint.Render(label) + "  " + greenTheme.ink.Render(row.value)
		lines = append(lines, padSetupLine(line, rowWidth))
	}

	lines = append(lines,
		"",
		greenTheme.faint.Render("Later, use /provider, /doctor, or /help anytime."),
	)
	return lines
}
...
func (m model) setupModelLines(width int, height int) []string {
	...
	start := selectableListStart(len(models), maxVisible, m.setup.modelIndex)
	lines := []string{
		padSetupLine("  "+greenTheme.ink.Bold(true).Render("Choose a model"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+m.setupModelSearchLine(rowWidth-2), rowWidth),
	}
	if status := m.setupModelStatus(); status != "" {
		lines = append(lines, padSetupLine("  "+greenTheme.faint.Render(status), rowWidth))
	}
	lines = append(lines, blankSetupBlockLine(rowWidth))
	if len(models) == 0 {
		lines = append(lines, padSetupLine("  "+greenTheme.faint.Render("No matching models"), rowWidth))
		return lines
	}
	...
	lines = append(lines,
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+greenTheme.faint.Render(detail), rowWidth),
	)
	return lines
}
...
func (m model) setupTypedModelLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+greenTheme.ink.Bold(true).Render("Choose a model"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+greenTheme.ink.Render("Enter the model ID this endpoint expects."), rowWidth),
		padSetupLine("  "+greenTheme.faint.Render("Examples: gpt-4.1, claude-sonnet-4-5, llama-3.3-70b"), rowWidth),
		blankSetupBlockLine(rowWidth),
...
func (m model) setupModelLoadingLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+greenTheme.ink.Bold(true).Render("Choose a model"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+greenTheme.faint.Render("Checking available models..."), rowWidth),
		padSetupLine("  "+greenTheme.faint.Render("Built-in models will be used if discovery fails."), rowWidth),
	}
}
...
func (m model) setupModelSearchLine(width int) string {
	...
	if query == "" {
		return fitStyledLine(prompt+cursor+greenTheme.faint.Render("model name..."), width)
	}
	return fitStyledLine(prompt+greenTheme.ink.Render(query)+cursor, width)
}
...
func (m model) setupModelRow(width int, index int, model providerWizardModel) string {
	...
	marker := "  "
	style := greenTheme.ink
	if selected {
		marker = "❯ "
		style = greenTheme.accent.Bold(true)
	}
...
func (m model) setupMethodLines(width int) []string {
	...
	lines := []string{
		padSetupLine("  "+greenTheme.ink.Bold(true).Render("How do you want to connect?"), rowWidth),
		blankSetupBlockLine(rowWidth),
	}
	for index, option := range options {
		...
		lines = append(lines, padSetupLine("    "+greenTheme.faint.Render(option.subtitle), rowWidth))
	}
	return lines
}
...
func (m model) setupOAuthWaitingLines(width int) []string {
	...
	if m.setup.oauthDevice {
		lines = []string{greenTheme.ink.Bold(true).Render("Device-code sign-in for " + name), ""}
		if m.setup.deviceUserCode == "" {
			lines = append(lines, greenTheme.faint.Render("Requesting a device code..."))
		} else {
			lines = append(lines,
				"1. On any device, visit:  "+greenTheme.accent.Render(m.setup.deviceVerificationURI),
				"2. Enter the code:  "+greenTheme.accent.Bold(true).Render(m.setup.deviceUserCode),
				"",
				greenTheme.faint.Render("Waiting for authorization..."),
			)
		}
	} else {
		lines = []string{
			greenTheme.ink.Bold(true).Render("Signing in with " + name),
			"",
			"Opening your browser — approve there, then return here.",
			greenTheme.faint.Render("Waiting for authorization..."),
			"",
			greenTheme.faint.Render("If your browser didn't open, run:  " + providerWizardOAuthCLIHint(provider)),
		}
	}
	if m.setup.oauthErr != "" {
		lines = append(lines, "", greenTheme.red.Render("Sign-in failed: "+m.setup.oauthErr))
	}
	return lines
}
...
func (m model) setupProviderLines(width int, height int) []string {
	...
	lines := []string{
		padSetupLine("  "+greenTheme.ink.Bold(true).Render("Choose a provider"), rowWidth),
		blankSetupBlockLine(rowWidth),
	}
	for index, option := range visibleProviders {
		...
		if option.Recommended {
			line += greenTheme.faint.Render("  (recommended)")
		}
		lines = append(lines, padSetupLine(line, rowWidth))
	}
	...
	if m.setup.oauthMode && m.setup.oauthErr != "" {
		lines = append(lines,
			blankSetupBlockLine(rowWidth),
			padSetupLine("  "+greenTheme.red.Render("Sign-in failed: "+m.setup.oauthErr), rowWidth),
		)
	}
	return lines
}
...
func (m model) setupEndpointLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+greenTheme.ink.Bold(true).Render("Endpoint URL"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+greenTheme.ink.Render("Enter the API base URL for "+displayValue(option.Name, option.ID)+"."), rowWidth),
		padSetupLine("  "+greenTheme.faint.Render(providerWizardEndpointHint(provider)), rowWidth),
		blankSetupBlockLine(rowWidth),
...
func (m model) setupNameLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+greenTheme.ink.Bold(true).Render("Provider name"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+greenTheme.ink.Render("Choose the short label shown in green."), rowWidth),
		padSetupLine("  "+greenTheme.faint.Render("Leave blank to use "+name+"."), rowWidth),
		blankSetupBlockLine(rowWidth),
...
func (m model) setupCredentialLines(width int) []string {
	option := m.setupProvider()
	lines := []string{
		greenTheme.ink.Bold(true).Render("Credentials"),
		"",
	}
	...
	lines = append(lines,
		...
		"",
		greenTheme.faint.Render("Saved keys stay in your user config."),
		greenTheme.faint.Render("Blank uses "+envVar+" from your shell."),
	)
	return lines
}
...
func (m model) setupAPIKeyInputLine(width int) string {
	...
	if strings.TrimSpace(input.Value()) == "" {
		return greenTheme.faint.Render(input.Placeholder)
	}
...
func (m model) setupFooter() string {
	if m.setup.oauthPending {
		return greenTheme.faint.Render("Esc cancel")
	}
	switch m.setup.stage {
	case setupStageMethod:
		return greenTheme.faint.Render("↑/↓ choose   ") + greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" continue   q quit")
	case setupStageReady:
		return greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" to save and start chat")
	case setupStageEndpoint:
		return greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" continue   left back")
	case setupStageName:
		return greenTheme.faint.Render("name optional   ") + greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" continue   left back")
	case setupStageCredentials:
		if m.setupCredentialInputActive() {
			return greenTheme.faint.Render("paste key optional   ") + greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" continue   left back")
		}
		return greenTheme.accent.Render("Space") + greenTheme.faint.Render(" to continue")
	case setupStageProvider:
		if m.setup.oauthMode && m.setupProviderDescriptor().OAuthDeviceFlow {
			return greenTheme.faint.Render("↑/↓ choose   ") + greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" sign in   ") + greenTheme.accent.Render("d") + greenTheme.faint.Render(" device code   q quit")
		}
		return greenTheme.faint.Render("↑/↓ choose   ") + greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" continue   q quit")
	case setupStageModel:
		if m.setup.modelLoad {
			return greenTheme.faint.Render("checking models...")
		}
		return greenTheme.faint.Render("↑/↓ choose   type search   ") + greenTheme.accent.Render("Enter") + greenTheme.faint.Render(" continue")
	case setupStageWelcome:
		return greenTheme.accent.Render("Space") + greenTheme.faint.Render(" to set up green")
	default:
		return greenTheme.accent.Render("Space") + greenTheme.faint.Render(" to continue")
	}
}
...
func (m model) setupProgressText() string {
	...
	return greenTheme.faint.Render(fmt.Sprintf("%d/%d", position+1, len(stages)))
}
