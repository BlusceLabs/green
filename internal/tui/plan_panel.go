// plan_panel.go renders the sticky plan panel for the green TUI. The panel
...
func (s *planPanelState) updateFromItems(items []tools.PlanItem, now time.Time) {
	...
	// its timers instead of resetting its elapsed clock to green mid-progress.
...
func (m model) renderPlanSummaryLine(width int) string {
	...
	if state.isComplete() {
		return greenTheme.green.Render(fmt.Sprintf("✓ PLAN · %d/%d complete", done, total))
	}
	...
	return greenTheme.accent.Render(label + truncateStep(current, room))
}
...
func renderPlanHeader(state planPanelState, spinnerView string, done, total int, elapsed time.Duration) string {
	...
	if state.isComplete() {
		return greenTheme.green.Render(fmt.Sprintf("✓ PLAN COMPLETE · %d/%d · %s", done, total, formatElapsedSeconds(elapsed)))
	}
	return greenTheme.accent.Render(fmt.Sprintf("%s PLAN · %s · %d/%d · %s", spinnerView, current, done, total, formatElapsedSeconds(elapsed)))
}
...
func renderPlanStepLine(step planStep, now time.Time, maxContent int) string {
	...
	case "completed":
		...
		body = greenTheme.muted.Render(content)
	...
	case "failed":
		...
		body = greenTheme.red.Render(content)
		timeStr = formatElapsedSeconds(step.completedAt.Sub(step.startedAt))
	default: // pending
		icon = greenTheme.faint.Render("○")
		body = greenTheme.faint.Render(content)
	}
	line := "  " + icon + " " + body
	if timeStr != "" {
		line += " " + greenTheme.faint.Render(timeStr)
	}
	return line
}
