...
func (m model) titleBar(width int) string {
	...
	cwdOnly := greenTheme.faint.Render(shortenPath(m.cwd))
	compactLeft := cwdOnly
	if branchOnly != "" {
	...
	if window := m.modelContextWindow(m.modelName); window > 0 {
		ctx = greenTheme.faint.Render(" · " + formatContextWindow(window))
	}
	...
	rule := greenTheme.line.Render(strings.Repeat("─", width))
	return line + "\n" + rule
}

func (m model) titleWorkspaceSegment() string {
	cwd := greenTheme.faint.Render(shortenPath(m.cwd))
	parts := []string{}
...
func (m model) titleWorkspaceSegmentShort() string {
	...
	if branch != "" {
		icon := greenTheme.muted.Render("")
		parts = append(parts, icon+" "+greenTheme.muted.Render(middleTruncate(branch, 22)))
	}
	if pr := m.titlePRSegment(); pr != "" {
		parts = append(parts, pr)
	}
...
func (m model) titleBranchSegment() string {
	...
	return greenTheme.muted.Render("") + " " + greenTheme.muted.Render(branch)
}
...
func (m model) titleModelSegment() string {
	...
	switch {
	case provider == "" && model == "":
		return greenTheme.muted.Render("no provider")
	case model == "":
		return greenTheme.ink.Render(provider)
	case provider == "":
		return greenTheme.ink.Render(model)
	default:
		return greenTheme.ink.Render(provider + "/" + model)
	}
}
...
func (m model) composerDividerLine(width int) string {
	...
	meta := greenTheme.muted.Render(model)
	metaWidth := lipgloss.Width(meta)
	if width < 8 {
		return greenTheme.lineStrong.Render(strings.Repeat("─", width))
	}
	if width < metaWidth+4 {
		return greenTheme.lineStrong.Render("╰" + strings.Repeat("─", width-2) + "╯")
	}
	rule := strings.Repeat("─", width-metaWidth-4)
	return greenTheme.lineStrong.Render("╰"+rule+" ") + meta + greenTheme.lineStrong.Render(" ╯")
}
...
func (m model) statusLine(width int) string {
	tier := widthTier(width)
	separator := greenTheme.line.Render(" │ ")
	prefix := "  "
	...
	left := prefix + greenTheme.accent.Render("●") + " " + modeStyle.Render(modeText)

	if tier == tierTiny {
		if m.exitConfirmActive {
			return fitStyledLine(prefix+greenTheme.amber.Render("●")+" "+greenTheme.amber.Render(ctrlCExitConfirmText), width)
		}
		if m.cancelConfirmActive {
			return fitStyledLine(prefix+greenTheme.amber.Render("●")+" "+greenTheme.amber.Render(escCancelConfirmText), width)
		}
	...
	if m.reasoningEffort != "" {
		left += greenTheme.muted.Render(" · ") + greenTheme.accent.Render(string(m.reasoningEffort))
	}
	if m.exitConfirmActive {
		left = prefix + greenTheme.amber.Render("●") + " " + greenTheme.amber.Render(ctrlCExitConfirmText)
	} else if m.cancelConfirmActive {
		left = prefix + greenTheme.amber.Render("●") + " " + greenTheme.amber.Render(escCancelConfirmText)
		...
		left = prefix + greenTheme.accent.Render("⬇ ") + greenTheme.muted.Render(m.dictation.downloadStatus)
		...
		if voice := m.voiceModeIndicator(); voice != "" {
			left += greenTheme.muted.Render(" · ") + voice
		}
		if summary := m.backgroundTerminalSummary(); summary != "" {
			left += separator + greenTheme.muted.Render(summary)
		}
	}
	...
	if !m.exitConfirmActive && !m.cancelConfirmActive {
		if loopSummary := m.loopFooterSummary(); loopSummary != "" {
			left += separator + greenTheme.accent.Render("↻ ") + greenTheme.muted.Render(loopSummary)
		}
	}
	...
	if usage != "" {
		rightGroups = append(rightGroups, greenTheme.muted.Render(usage))
	}
...
func (m model) modeLabel() (string, lipgloss.Style) {
	...
	case agent.PermissionModeUnsafe:
		return "unsafe", greenTheme.modeUnsafe
	default:
...
func (m model) contextFillPercent() (pct, used, window int, style lipgloss.Style, ok bool) {
	...
	style = greenTheme.green
	switch {
	case ratio >= 0.90:
		style = greenTheme.red
	case ratio >= 0.75:
		style = greenTheme.amber
	}
...
func renderSuggestionPalette(items []selectableListItem, selected, width int, title, query, footer string) string {
	...
	searchPrefix := transparentSurface(greenTheme.ink).Render(strings.Repeat(" ", searchInset))
	...
	lines = append(lines, greenTheme.line.Render(strings.Repeat("─", innerWidth)))
	...
	for index, item := range visible {
		...
		label := surface(greenTheme.ink).Render(labelText)
		pad := surface(greenTheme.ink).Render(strings.Repeat(" ", maxInt(0, labelWidth-lipgloss.Width(labelText))))
		...
		if desc := strings.TrimSpace(item.Description); desc != "" {
			...
			if truncated := truncateRunes(desc, maxInt(0, descWidth)); truncated != "" {
				line += surface(greenTheme.faint).Render("  " + truncated)
			}
		}
	...
	if len(visible) == 0 {
		...
		lines = append(lines, fillPaletteLine(searchPrefix+greenTheme.faint.Render(message), innerWidth, transparentSurface))
	}

	if footer = strings.TrimSpace(footer); footer != "" {
		lines = append(lines, greenTheme.line.Render(strings.Repeat("─", innerWidth)))
		line := greenTheme.faint.Render(footer)
		lines = append(lines, fillPaletteLine(line, innerWidth, transparentSurface))
	}
	return styledBlockFillTitle(paletteWidth, strings.TrimSpace(title), lines, greenTheme.lineStrong, lipgloss.NewStyle())
}

func styledBlockFillTitle(width int, title string, lines []string, borderStyle lipgloss.Style, fill lipgloss.Style) string {
	return styledBlockFillTitleStyled(width, title, lines, borderStyle, fill, greenTheme.ink.Bold(true))
}
...
func renderSuggestionSearchLine(query string, width int) string {
	...
	label := greenTheme.userPrompt.Render("search > ")
	valueWidth := maxInt(1, width-lipgloss.Width(label))
	value := greenTheme.ink.Render(truncateRunes(query, valueWidth))
...
func fillPaletteLine(line string, width int, surface func(lipgloss.Style) lipgloss.Style) string {
	...
	if pad > 0 {
		line += surface(greenTheme.ink).Render(strings.Repeat(" ", pad))
	}
	return line
}
...
func (m model) pickerOverlay(width int) string {
	...
	for index, item := range visible {
		...
		if item.Group != "" && item.Group != lastGroup {
			lines = append(lines, greenTheme.accent.Render(item.Group))
		...
		switch {
		case item.Local:
			left += surface(greenTheme.blue).Render("● ")
		case item.Remote:
			left += surface(greenTheme.accent).Render("● ")
		}
		if item.Favorite {
			left += surface(greenTheme.accent).Render("* ")
		}
		left += surface(greenTheme.ink).Render(item.Label)
		right := ""
		if item.Meta != "" {
			right = surface(greenTheme.faintest).Render(item.Meta)
		}
		...
		line := left + surface(greenTheme.ink).Render(strings.Repeat(" ", maxInt(1, gap))) + right
		lines = append(lines, fitStyledLine(line, innerWidth))
	}
	if len(visible) == 0 {
		if m.picker.loading {
			lines = append(lines, greenTheme.faint.Render("Fetching available models…"))
		} else {
			lines = append(lines, greenTheme.faint.Render("  no matching items"))
		}
	}
	// Hints live in the footer (a separator + faint keys), matching the /model
	// picker and the other bordered boxes.
	lines = append(lines, greenTheme.line.Render(strings.Repeat("─", innerWidth)))
	lines = append(lines, greenTheme.faint.Render("↑/↓ move   Enter select   Esc close"))
	return centerRenderedBlock(styledBlockFillTitle(overlayWidth, title, lines, greenTheme.lineStrong, lipgloss.NewStyle()), width)
}
...
func (m model) modelPickerOverlay(width int) string {
	...
	if status := strings.TrimSpace(m.modelPickerLoadError); status != "" {
		lines = append(lines, fillPaletteLine(searchPrefix+greenTheme.faint.Render(status), innerWidth, transparentSurface))
	}
	lines = append(lines, greenTheme.line.Render(strings.Repeat("─", innerWidth)))
	lastGroup := ""
	for index, item := range visible {
		if item.Group != "" && item.Group != lastGroup {
			lines = append(lines, fillPaletteLine(greenTheme.accent.Bold(true).Render(item.Group), innerWidth, transparentSurface))
			lastGroup = item.Group
		}
		lines = append(lines, renderModelPickerRow(innerWidth, start+index == m.picker.selected, item))
	}
	if len(visible) == 0 {
		lines = append(lines, fillPaletteLine(searchPrefix+greenTheme.faint.Render("no matching models"), innerWidth, transparentSurface))
	}
	if item, ok := m.picker.current(); ok {
		if detail := modelPickerItemDetail(item); detail != "" {
			lines = append(lines, greenTheme.line.Render(strings.Repeat("─", innerWidth)))
			lines = append(lines, fillPaletteLine(searchPrefix+greenTheme.faint.Render(detail), innerWidth, transparentSurface))
		}
	}
	lines = append(lines, greenTheme.line.Render(strings.Repeat("─", innerWidth)))
	footer := "↑/↓ move   Enter select   Ctrl+F favorite   Esc close"
	lines = append(lines, fillPaletteLine(greenTheme.faint.Render(footer), innerWidth, transparentSurface))
	title := strings.TrimSpace(m.picker.title)
	if title == "" {
...
func (m model) modelPickerLoadingOverlay(width int) string {
	...
	innerWidth := maxInt(1, overlayWidth-4)
	lines := []string{
		fillPaletteLine(greenTheme.faint.Render("Checking available models..."), innerWidth, transparentSurface),
		fillPaletteLine(greenTheme.faint.Render("Built-in models will be used if discovery fails."), innerWidth, transparentSurface),
		greenTheme.line.Render(strings.Repeat("─", innerWidth)),
		fillPaletteLine(greenTheme.faint.Render("Esc close"), innerWidth, transparentSurface),
	}
	title := strings.TrimSpace(m.picker.title)
	if title == "" {
		title = "Choose a model"
...
func renderModelPickerRow(width int, selected bool, item pickerItem) string {
	...
	left := marker + surface(greenTheme.ink).Render(prefix+label)
