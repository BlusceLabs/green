...
func renderRecapRow(row transcriptRow, width int) string {
	return fitStyledLine(greenTheme.faint.Render("※ recap: "+strings.TrimSpace(row.text)), width)
}
...
// — cmd/ctrl+click on an edited file opens it. The sequences are green-width
...
//	under $HOME    → ~/projects/green/main.go
...
func renderUserRow(row transcriptRow, width int) string {
	...
	for _, line := range wrapped {
		lines = append(lines, renderUserPromptStyledLine(greenTheme.ink.Bold(true).Render(line), contentWidth))
	}
...
func renderUserPromptStyledLine(styledText string, contentWidth int) string {
	if contentWidth <= 0 {
		return greenTheme.userPrompt.Render("▌")
	}
	fitted := fitStyledLine(styledText, contentWidth)
	return greenTheme.userPrompt.Render("▌") + "  " + fitted
}
...
func renderAssistantRow(row transcriptRow, width int) string {
	...
	if !row.final {
		...
		for _, line := range narr {
			styled := styleAssistantMarkdownLine(line, greenTheme.ink)
	...
	for index := range lines {
		lines[index] = styleAssistantMarkdownLine(lines[index], greenTheme.ink)
	}
...
func renderReasoningBlock(text string, expanded bool, width int, running bool, elapsed time.Duration, maxBodyLines int) string {
	...
	for _, line := range body {
		lines = append(lines, fitStyledLine("  "+styleAssistantMarkdownLine(line, greenTheme.sayText), width))
	}
	return strings.Join(lines, "\n")
}
...
func reasoningHiddenMarker(hidden int) string {
	return greenTheme.faint.Render(fmt.Sprintf("… %d earlier lines · Ctrl+O for all", hidden))
}
...
func reasoningHeaderLine(text string, expanded bool, running bool, elapsed time.Duration) string {
	return greenTheme.faint.Render(reasoningHeaderText(text, expanded, running, elapsed))
}
...
func doneLine(row transcriptRow) string {
	return greenTheme.faint.Render("worked for " + formatElapsedSeconds(row.turnElapsed))
}
...
func renderSystemNote(text string, width int) string {
	...
	marker, style := greenTheme.faint.Render("·"), greenTheme.muted
	if isCancellationNotice(trimmed) {
		marker, style = greenTheme.amber.Render("⊘"), greenTheme.amber
	}
...
func renderCommandCardRow(text string, width int) string {
	...
	for _, line := range raw[1:] {
		...
		case isCommandCardActionsLine(trimmed):
			lines = append(lines, greenTheme.accent.Render("actions: ")+greenTheme.ink.Render(strings.TrimSpace(strings.TrimPrefix(trimmed, "actions:"))))
		case isCommandCardHintLine(trimmed):
			lines = append(lines, greenTheme.faint.Render(line))
		...
		default:
			...
			lines = append(lines, greenTheme.accent.Bold(true).Render(line))
		}
	}
	return styledBlockFillTitle(width, title, lines, greenTheme.accent, lipgloss.NewStyle())
}
...
func renderPlanCardRow(text string, width int) string {
	...
	for _, line := range raw[1:] {
		...
		default:
			...
			lines = append(lines, greenTheme.muted.Bold(true).Render(line))
		}
	}
	return styledBlockFillTitleStyled(width, title, lines, greenTheme.faintest, lipgloss.NewStyle(), planCardTitleStyle(title))
}
...
func styleCommandCardContentRow(line string) string {
	...
	if strings.HasPrefix(body, "/") {
		if name, desc, ok := strings.Cut(body, " - "); ok {
			return indent + greenTheme.ink.Bold(true).Render(name) + greenTheme.muted.Render(" — "+desc)
		}
		return indent + greenTheme.ink.Bold(true).Render(body)
	}
	// "- bullet" list item: keep it readable (not dim grey).
	if strings.HasPrefix(body, "- ") {
		return indent + greenTheme.ink.Render(body)
	}
...
func styledCommandCardStatus(trimmed string) string {
	...
	switch state {
	case "warning":
		return greenTheme.amber.Render(trimmed)
	case "blocked":
		return greenTheme.red.Render(trimmed)
...
func renderMCPManagerCard(text string, width int) string {
	...
	for index, line := range raw {
		...
		case index == 0:
			lines = append(lines, greenTheme.accent.Bold(true).Render(line))
		case index == 1:
			lines = append(lines, greenTheme.ink.Bold(true).Render(line))
		case isMCPManagerHeading(trimmed):
			lines = append(lines, greenTheme.accent.Bold(true).Render(line))
		case strings.Contains(trimmed, "green mcp "):
			lines = append(lines, greenTheme.ink.Render(line))
		case strings.HasPrefix(trimmed, "›") || strings.HasPrefix(trimmed, "- "):
			lines = append(lines, greenTheme.ink.Render(line))
		default:
			lines = append(lines, greenTheme.muted.Render(line))
		}
	}
	return styledBlock(width, lines, greenTheme.accent)
}
...
func renderCompactRunningCard(text string, width int) string {
	...
	for index, line := range raw {
		switch index {
		case 0:
			lines = append(lines, greenTheme.amber.Bold(true).Render(line))
		case 1:
			lines = append(lines, greenTheme.muted.Render(line))
		case 2:
			lines = append(lines, greenTheme.amber.Bold(true).Render(line))
	...
	return styledBlock(width, lines, greenTheme.amber)
}
...
func renderCompactCompleteCard(text string, width int) string {
	...
	for index, line := range raw {
		switch index {
		case 0:
			lines = append(lines, greenTheme.green.Bold(true).Render(line))
		case 1:
			lines = append(lines, greenTheme.ink.Render(line))
		default:
			lines = append(lines, greenTheme.muted.Render(line))
	...
	return styledBlock(width, lines, greenTheme.green)
}
...
func renderDoctorResultCard(text string, width int) string {
	...
	for index, line := range raw {
		...
		case strings.HasPrefix(trimmed, "- [pass]"):
			lines = append(lines, greenTheme.green.Render(line))
		case strings.HasPrefix(trimmed, "- [warn]"):
			lines = append(lines, greenTheme.amber.Render(line))
		case strings.HasPrefix(trimmed, "- [fail]"):
			lines = append(lines, greenTheme.red.Render(line))
...
func doctorResultBorderStyle(text string) lipgloss.Style {
	for _, line := range strings.Split(strings.ReplaceAll(text, "\r\n", "\n"), "\n") {
		...
		case "status: blocked":
			return greenTheme.red
		case "status: warning":
			return greenTheme.amber
		}
	}
	return greenTheme.accent
}
...
func renderErrorRow(row transcriptRow, width int) string {
	note := noteBox(row.text, width, greenTheme.cardErr, greenTheme.red)
	...
	if hint := strings.TrimSpace(row.hint); hint != "" {
		note += "\n" + fitStyledLine(greenTheme.faint.Render("→ "+hint), width)
	}
	return note
}
...
func renderAskUserRow(row transcriptRow, width int) string {
	line := fitStyledLine(greenTheme.accent.Render("ask green")+"  "+greenTheme.ink.Render(strings.TrimPrefix(row.text, "ask_user: ")), width)
...
func renderPermissionRow(row transcriptRow, width int) string {
	event := row.permission
	if event == nil {
		return greenTheme.amber.Render("permission") + "  " + greenTheme.ink.Render(row.text)
	}
	...
	dot := greenTheme.faintest.Render(" · ")
	...
	case agent.PermissionActionAllow:
		...
		line := greenTheme.green.Render(label) + dot + greenTheme.green.Render(displayName)
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += dot + greenTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		return fitStyledLine(line, width)
	case agent.PermissionActionDeny:
		line := greenTheme.red.Render("denied") + dot + greenTheme.red.Render(displayName)
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += dot + greenTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		if reason := permissionDisplayReason(event.Reason); reason != "" {
			line += greenTheme.faint.Render(" — " + truncateRunes(reason, maxInt(16, width-lipgloss.Width(displayName)-16)))
		}
		...
		return out
	case agent.PermissionActionCancel:
		line := greenTheme.red.Render("cancelled") + dot + greenTheme.red.Render(displayName)
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += dot + greenTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		if reason := permissionDisplayReason(event.Reason); reason != "" {
		...
		return out
	default:
		line := greenTheme.amber.Render("permission") + "  " + greenTheme.ink.Render(displayName) + "  " + greenTheme.amber.Render("prompt")
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += "  " + greenTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		out := fitStyledLine(line, width)
		if detail := strings.TrimSpace(row.detail); detail != "" {
			out += "\n" + wrapDetailBlock(detail, width)
...
func wrapDetailBlock(detail string, width int) string {
	...
	for index := range lines {
		lines[index] = "  " + greenTheme.muted.Render(lines[index])
	}
	return strings.Join(lines, "\n")
}
...
func renderFocusedPermissionPrompt(request agent.PermissionRequest, cursor int, width int) (string, []int) {
	...
	fill := greenTheme.onPerm

	top := greenTheme.permBadge.Render(" PERMISSION ")

	body := fill(greenTheme.amber).Bold(true).Render(name)
	if request.ToolName == tools.RequestPermissionsToolName {
		body = fill(greenTheme.amber).Bold(true).Render("Grant requested permissions?")
	} else if request.SideEffect != "" {
		body += fill(greenTheme.ink).Render("  " + request.SideEffect)
	}
	lines := []string{top, body}
	if reason := permissionDisplayReason(request.Reason); reason != "" {
		lines = append(lines, fill(greenTheme.muted).Render(reason))
	}
	...
	if scope := strings.TrimSpace(request.Scope); scope != "" {
		lines = append(lines, fill(greenTheme.muted).Render(permissionScopeLine(request, scope)))
	}
	...
	for index, option := range options {
		...
		hotkey := fill(greenTheme.faint).Render(" [" + option.hotkey + "]")
		optionLabel := permissionOptionLabel(option, request)
		if index == cursor {
			marker := fill(greenTheme.accent).Render("▸ ")
			label := greenTheme.badge.Render(" " + optionLabel + " ")
			lines = append(lines, marker+label+hotkey)
		} else {
			label := fill(greenTheme.ink).Render(optionLabel)
	...
	lines = append(lines, fill(greenTheme.faint).Render(footer))

	return styledBlockFill(width, lines, greenTheme.permBorder, greenTheme.permBg), offsets
}
...
func permissionOptionLabel(option permissionOption, request agent.PermissionRequest) string {
	...
	case permissionDecisionCancel:
		return "No, and tell green what to do differently"
	default:
		return option.label
	}
}
...
func renderAskUserQuestionnaire(prompt pendingAskUserPrompt, input string, width int) string {
	...
	if len(questions) == 0 {
		return styledBlockFill(width, []string{greenTheme.badge.Render(" ASK ")}, greenTheme.lineStrong, lipgloss.NewStyle())
	}
	...
	if header := strings.TrimSpace(prompt.request.Header); header != "" {
		lines = append(lines, fill(greenTheme.ink).Render(header))
	}
	...
	if multi {
		...
		for index, question := range questions {
			...
			switch {
			case index == active:
				tabs = append(tabs, greenTheme.badge.Render(" "+title+" "))
			case prompt.states[index].answered:
				tabs = append(tabs, fill(greenTheme.muted).Render(" ✓ "+title+" "))
			default:
				tabs = append(tabs, fill(greenTheme.faint).Render(" "+title+" "))
			}
		}
		if active == confirm {
			tabs = append(tabs, greenTheme.badge.Render(" Confirm "))
		} else {
			tabs = append(tabs, fill(greenTheme.faint).Render(" Confirm "))
		}
	...
	if active == confirm {
		lines = append(lines, "")
		lines = append(lines, fill(greenTheme.ink).Render("Review and submit:"))
		for index, question := range questions {
			answer := strings.TrimSpace(prompt.states[index].answer)
			rendered := fill(greenTheme.ink).Render(answer)
			if answer == "" {
				rendered = fill(greenTheme.faint).Render("(no answer)")
			}
			lines = append(lines, "  "+fill(greenTheme.muted).Render(askUserTabTitle(question, index)+": ")+rendered)
		}
		lines = append(lines, "")
		lines = append(lines, fill(greenTheme.faint).Render("⇆ tab · enter submit · esc dismiss"))
		return styledBlockFill(width, lines, greenTheme.lineStrong, lipgloss.NewStyle())
	}

	question := questions[active]
	state := prompt.states[active]
	lines = append(lines, "")
	lines = append(lines, fill(greenTheme.ink).Render(question.Question))
	...
	if len(question.Options) > 0 && !state.typing {
		...
		for index, option := range question.Options {
			...
			if index == cursor {
				lines = append(lines, fill(greenTheme.accent).Render("▸ ")+greenTheme.badge.Render(" "+label+" "))
			} else {
				lines = append(lines, "  "+fill(greenTheme.ink).Render(label))
			}
			if index < len(question.OptionDescriptions) && strings.TrimSpace(question.OptionDescriptions[index]) != "" {
				lines = append(lines, "     "+fill(greenTheme.faint).Render(question.OptionDescriptions[index]))
			}
		}
		typeOwn := fmt.Sprintf("%d. %s", len(question.Options)+1, askUserTypeMyOwnLabel)
		if cursor >= len(question.Options) {
			lines = append(lines, fill(greenTheme.accent).Render("▸ ")+greenTheme.badge.Render(" "+typeOwn+" "))
		} else {
			lines = append(lines, "  "+fill(greenTheme.muted).Render(typeOwn))
		}
		lines = append(lines, "")
	...
	if question.MultiSelect && len(question.Options) > 0 {
		lines = append(lines, fill(greenTheme.muted).Render("suggested: "+strings.Join(question.Options, ", ")))
	}
	lines = append(lines, greenTheme.userPrompt.Render("❯ ")+fill(greenTheme.ink).Render(input)+fill(greenTheme.accent).Render("▌"))
...
func (m model) renderRunningToolCard(row transcriptRow, width int, rc rowContext, opts cardRenderOptions) string {
	glyph := greenTheme.faintest.Render("…")
	if m.pending && row.runID != 0 && row.runID == m.activeRunID {
		glyph = greenTheme.accent.Render(m.spinnerGlyph())
	}
	...
	head := toolCardHead(toolRowName(row), hint, arg, "", "", "", true, greenTheme.ink, rc.auto[rcKey(row.runID, row.id)], width, opts)
	return toolCard(head, glyph, nil, "", greenTheme.cardRun, width)
}
...
func renderToolResultCard(row transcriptRow, width int, rc rowContext, opts cardRenderOptions) string {
	...
	glyph := greenTheme.green.Render("•")
	nameStyle := greenTheme.green
	borderStyle := greenTheme.line
	if opts.fileSelected {
		...
		borderStyle = greenTheme.accent
	}
	if failed {
		glyph = greenTheme.red.Render("•")
		nameStyle = greenTheme.red
		borderStyle = greenTheme.cardErr
	}
...
func toolCardHead(name string, target string, arg string, headTag string, detail string, actionDetail string, running bool, nameStyle lipgloss.Style, auto bool, width int, opts cardRenderOptions) string {
	...
	if target = singleLineToolHeadText(toolCardHeadTarget(name, target, detail)); target != "" {
		...
		styled := greenTheme.toolTarget.Render(middleTruncate(shown, maxInt(16, width/2)))
		if looksLikePath(target) {
	...
	if arg = singleLineToolHeadText(arg); arg != "" && widthTier(width) == tierFull {
		head += "  " + greenTheme.toolArg.Render(truncateRunes(arg, maxInt(12, width/3)))
	}
	if headTag != "" {
		if strings.Contains(headTag, "\x1b") {
			head += "  " + headTag
		} else {
			head += "  " + greenTheme.faint.Render(headTag)
		}
	}
...
func capCardLines(lines []string, cap int) []string {
	...
	lines = lines[:cap]
	return append(lines, greenTheme.faint.Render(fmt.Sprintf("… %d more lines", hidden)))
}
...
func diffCountTag(adds int, dels int) string {
	...
	return greenTheme.faint.Render("(") +
		greenTheme.diffAdd.Render(fmt.Sprintf("+%d", adds)) +
		greenTheme.faint.Render(" ") +
		greenTheme.diffDel.Render(fmt.Sprintf("-%d", dels)) +
		greenTheme.faint.Render(")")
}
...
func diffCardBody(detail string, width int, opts cardRenderOptions) cardBody {
	...
	for i := 0; i < len(rawLines); i++ {
		...
		case !inHunk, strings.HasPrefix(line, `\`):
			...
			lines = append(lines, greenTheme.diffMeta.Render(truncateRunes(line, innerWidth)))
		case strings.HasPrefix(line, "+"):
		...
		default:
			text := truncateRunes(strings.TrimPrefix(line, " "), textBudget)
			row := "   " + greenTheme.muted.Render(text)
			if gutter {
				row = greenTheme.faintest.Render(fmt.Sprintf("%4d", newLine)) + row
			}
...
func highlightedAddedDiffLines(rawLines []string, meta diffMetadata, textBudget int) []string {
	...
	highlighted, ok := highlightCodeForPath(content, meta.path, textBudget, greenTheme.addLine.GetBackground())
...
func diffBodyLine(number int, sign string, text string, added bool, textBudget int, gutter bool) string {
	...
	if gutter {
		...
		if added {
			numCol = greenTheme.addLineNum.Render(num)
		} else {
			numCol = greenTheme.delLineNum.Render(num)
		}
	}
	if added {
		return numCol + greenTheme.addSign.Render(" "+sign+" ") + greenTheme.addLine.Render(text)
	}
	return numCol + greenTheme.delSign.Render(" "+sign+" ") + greenTheme.delLine.Render(text)
}

func diffBodyStyledLine(number int, sign string, styledText string, added bool, textBudget int, gutter bool) string {
	lineStyle, signStyle, numStyle := greenTheme.delLine, greenTheme.delSign, greenTheme.delLineNum
	if added {
		lineStyle, signStyle, numStyle = greenTheme.addLine, greenTheme.addSign, greenTheme.addLineNum
	}
...
func diffBodyLineSpanned(number int, sign string, text []rune, added bool, spanStart, spanEnd, textBudget int, gutter bool) string {
	...
	lineStyle, wordStyle, signStyle, numStyle := greenTheme.delLine, greenTheme.delLineWord, greenTheme.delSign, greenTheme.delLineNum
	if added {
		lineStyle, wordStyle, signStyle, numStyle = greenTheme.addLine, greenTheme.addLineWord, greenTheme.addSign, greenTheme.addLineNum
	}
...
func exploreCardBody(name string, hint string, arg string, detail string, width int, opts cardRenderOptions, failed bool) cardBody {
	...
	if !opts.expanded && opts.bodyCap > 0 {
		footer := ""
		if strings.TrimSpace(detail) != "" {
			footer = greenTheme.faint.Render("▸ details")
		}
	...
	if opts.expanded && opts.bodyCap > 0 && footer == "" {
		footer = greenTheme.faint.Render("▾ collapse")
	}
...
func exploreCardLine(name string, hint string, arg string, detail string, width int, opts cardRenderOptions, marker string) string {
	...
	line := greenTheme.faint.Render("  "+marker+" ") + greenTheme.green.Render(action)
	if target != "" {
		shown := target
		...
		styled := greenTheme.toolTarget.Render(middleTruncate(shown, maxInt(8, width-lipgloss.Width(action)-6)))
		if isPath {
			styled = hyperlink(fileURL(opts.cwd, target), styled)
		}
		line += " " + styled
...
func localControlChildLine(text string, width int) string {
	...
	return greenTheme.faint.Render(prefix) + greenTheme.muted.Render(truncateDisplayWidth(text, budget))
}
...
func bashCardBody(command string, detail string, width int, opts cardRenderOptions) cardBody {
	...
	for _, line := range strings.Split(detail, "\n") {
		...
		case strings.HasPrefix(line, "exit_code: "):
			...
			if code != "0" {
				footer = greenTheme.red.Render("exit " + code)
			}
		default:
			style := greenTheme.muted
			if section == "stderr" {
				style = greenTheme.delText
			}
...
func execCommandCardBody(command string, detail string, width int, opts cardRenderOptions) cardBody {
	...
	for _, line := range strings.Split(detail, "\n") {
		...
		case line == "interrupted: true":
			interrupted = true
			footer = greenTheme.green.Render("interrupted")
		case strings.HasPrefix(line, "exit_code: "):
			code := strings.TrimPrefix(line, "exit_code: ")
			if code == "0" {
		...
		case strings.HasPrefix(line, "session_id: "):
			footer = greenTheme.faint.Render("session " + strings.TrimSpace(strings.TrimPrefix(line, "session_id: ")))
		case strings.HasPrefix(line, "Use write_stdin "):
			continue
		default:
			style := greenTheme.muted
			if section == "" && strings.HasPrefix(line, "Command is still running.") {
				style = greenTheme.faint
			}
			output = append(output, commandOutputLine{text: line, style: style})
		}
	}
...
func renderCommandOutputLines(output []commandOutputLine, width int, opts cardRenderOptions) []string {
	...
	if len(output) == 1 {
		...
		return capCardLines([]string{greenTheme.faint.Render(prefix) + output[0].style.Render(text)}, opts.bodyCap)
	}
	...
	for _, item := range output {
		...
		lines = append(lines, greenTheme.faint.Render(prefix)+item.style.Render(text))
	}
...
func renderSessionsCards(payload string, width int) string {
	...
	for _, record := range strings.Split(payload, "\n") {
		...
		if len(fields) < 4 {
			blocks = append(blocks, fitStyledLine(greenTheme.faint.Render(record), width))
			continue
		}
		id, age, title, meta := fields[0], fields[1], fields[2], fields[3]
		innerWidth := width - 4
		top := joinHeaderLine(greenTheme.onPanel(greenTheme.accent).Render(id), greenTheme.onPanel(greenTheme.faint).Render(age), innerWidth)
		metaParts := strings.Split(meta, " · ")
		for index := range metaParts {
			metaParts[index] = greenTheme.onPanel(greenTheme.faint).Render(metaParts[index])
		}
		lines := []string{
			top,
			greenTheme.onPanel(greenTheme.ink).Render(title),
			strings.Join(metaParts, greenTheme.onPanel(greenTheme.faintest).Render(" · ")),
		}
		blocks = append(blocks, styledBlockFill(width, lines, greenTheme.line, greenTheme.panel))
	}
...
func grepCardBody(detail string, width int, opts cardRenderOptions) cardBody {
	...
	for _, line := range raw {
		if match := grepMatchPattern.FindStringSubmatch(line); match != nil {
			matches++
			location := greenTheme.grepLoc.Render(match[1])
			...
			lines = append(lines, location+"  "+greenTheme.muted.Render(truncateDisplayWidth(match[2], budget)))
			continue
		}
		lines = append(lines, greenTheme.muted.Render(line))
	}
	footer := ""
	if matches > 0 {
		footer = greenTheme.faint.Render(fmt.Sprintf("%d matches", matches))
	}
	return cardBody{lines: capCardLines(lines, opts.bodyCap), footer: footer}
}
