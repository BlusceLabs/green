...
import (
	...
	"github.com/BlusceLabs/green/internal/credstore"
	...
	for _, check := range report.Checks {
		...
		case "sandbox.backend":
			if remedy := doctorCheckDetailString(check, "remedy"); remedy != "" {
				...
				lines = append(lines, "native sandbox: run green sandbox policy --effective to inspect backend status")
			}
		case "lsp.servers":
...
func (m model) doctorConnectivityRunningText() string {
	return strings.Join([]string{
		"Checking provider",
		"green is probing the active endpoint. Keep typing; messages will queue until the check finishes.",
...
func (m model) handleModelCommand(args string) (model, string) {
	...
	if !ok {
		return m, "Model\nunknown green model " + strconv.Quote(args)
	}
...
func (m model) switchProviderModel(providerName, modelID string) (model, string, bool, tea.Cmd) {
	...
	if strings.TrimSpace(target.APIKey) == "" && strings.TrimSpace(target.AuthHeaderValue) == "" &&
		...
		return m, "Model\nprovider " + strconv.Quote(providerName) + " has no usable credential — run setup or `green auth login " + providerName + "`.", false, nil
	}
...
// the token lives under the catalog id, and `green auth logout codex` would
