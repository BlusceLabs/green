...
type transcriptRow struct {
	...
	// line; green values mean "unknown" and the segment is omitted.
	final       bool
	turnTools   int
...
func initialTranscript() []transcriptRow {
	return []transcriptRow{{
		kind: rowWelcome,
		text: "Welcome to green. Type /help for commands.",
	}}
}
...
// this only gates the rendered row, so the session log / `green sessions` stay
// complete.
//
