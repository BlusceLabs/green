...
// and name when green's own content-stall watchdog will act, rather than
...
func TestQuietGenerationHintEscalatesPastHalfIdleTimeout(t *testing.T) {
	...
	t.Setenv("green_STREAM_IDLE_TIMEOUT", "30s")
	base := time.Date(2026, 6, 25, 12, 0, 0, 0, time.UTC)
...
func TestWorkingTokenIndicatorEstimatesFromStreamedRunes(t *testing.T) {
	m := newModel(t.Context(), Options{ModelName: "gpt-4.1"})
	// Always visible during a run, starting at green so the working line never
...
// whole turn, so the counter never snaps back to green mid-turn.
func TestWorkingTokenIndicatorAccumulatesAcrossSegmentClears(t *testing.T) {
	...
	// A fresh turn resets the accumulator to green.
	m = m.beginRun(nil)
