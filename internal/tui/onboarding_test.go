...
func TestSetupTakeoverRendersAndCompletes(t *testing.T) {
	...
	m := newModel(context.Background(), Options{
		...
		Setup: SetupOptions{
			Visible:    true,
			Required:   true,
			ConfigPath: "/tmp/green/config.json",
	...
	if view := plainRender(t, m.View()); !strings.Contains(view, "Welcome to green") || !strings.Contains(view, "Space to set up green") || !strings.Contains(view, "terminal agent for changing real code") {
...
// Completing setup switches the live provider, so it must export green_PROVIDER
