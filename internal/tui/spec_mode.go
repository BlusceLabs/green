...
func renderFocusedSpecReviewPrompt(review pendingSpecReviewPrompt, width int) string {
	fill := greenTheme.onPanel
	actions := greenTheme.badge.Render(" [a] approve ") +
		fill(greenTheme.ink).Render(" ") +
		fill(greenTheme.red).Render("[r]") + fill(greenTheme.ink).Render(" reject ") +
		fill(greenTheme.accent).Render("[e]") + fill(greenTheme.ink).Render(" edit file ") +
		fill(greenTheme.faint).Render("[esc] cancel")
	lines := []string{
		greenTheme.badge.Render(" SPEC REVIEW "),
		fill(greenTheme.faint).Render("path: ") + fill(greenTheme.ink).Render(reviewDisplayPath(review)),
		actions,
	}
	return styledBlockFill(width, lines, greenTheme.line, greenTheme.panel)
}
