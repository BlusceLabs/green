...
func TestSpecCommandCreatesDraftReview(t *testing.T) {
	...
	if !strings.Contains(next.pendingSpecReview.RelativePath, ".green/specs/") {
...
func TestSpecReviewBlocksShiftTabModeCycle(t *testing.T) {
	...
	m.pendingSpecReview = &pendingSpecReviewPrompt{SpecID: "spec", SpecFilePath: ".green/specs/spec.md"}
...
func newSpecModeTestModel(root string, provider greenruntime.Provider, store *sessions.Store) model {
...
func submitSpecScript(callID string, title string, plan string) []greenruntime.StreamEvent {
	args, _ := json.Marshal(map[string]string{
		"title": title,
		"plan":  plan,
	})
	return []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventToolCallStart, ToolCallID: callID, ToolName: specmode.SubmitToolName},
		{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: callID, ArgumentsFragment: string(args)},
		{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: callID},
		{Type: greenruntime.StreamEventDone},
	}
}

func providerRequestIncludesTool(request greenruntime.CompletionRequest, name string) bool {
...
func providerRequestsContain(requests []greenruntime.CompletionRequest, text string) bool {
...
func TestSpecLaunchesSeedElapsedClock(t *testing.T) {
	...
	next.turnStartedAt = time.Time{} // green it to prove the impl path re-seeds independently
