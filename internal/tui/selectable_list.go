...
func renderSelectableList(options selectableListOptions) string {
	...
	for index, item := range visible {
		...
		label := surface(greenTheme.ink).Render(item.Label)
		pad := surface(greenTheme.ink).Render(strings.Repeat(" ", maxInt(0, labelWidth-lipgloss.Width(item.Label))))
		...
		if strings.TrimSpace(item.Description) != "" {
			...
			if desc != "" {
				line += surface(greenTheme.faint).Render("  " + desc)
			}
		}
	...
	if hidden := len(options.Items) - len(visible); hidden > 0 {
		lines = append(lines, fitStyledLine(greenTheme.faint.Render(fmt.Sprintf("  %d more", hidden)), width))
	}
