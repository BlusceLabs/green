...
func (provider *fakeProvider) StreamCompletion(
	...
	ch := make(chan greenruntime.StreamEvent, len(provider.events))
...
func TestPromptSubmitInjectsLiveSessionModelContext(t *testing.T) {
	...
	provider := &fakeProvider{events: []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventText, Content: "I am using the active session model."},
...
func TestPromptSubmitStoresReasoningSeparatelyFromAnswer(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	provider := &fakeProvider{events: []greenruntime.StreamEvent{
		{Type: greenruntime.StreamEventReasoning, Content: "private "},
		{Type: greenruntime.StreamEventReasoning, Content: "thought"},
...
func TestParseCommand(t *testing.T) {
	cases := []struct {
		...
		{input: "hello green", kind: commandPrompt, text: "hello green"},
	}
...
func TestInitialRenderShowsLimeChatSurface(t *testing.T) {
	...
	assertContains(t, view, `/workspace/green`)
	...
	if strings.Contains(view, "Welcome to green") {
...
func TestContextCommandShowsSessionState(t *testing.T) {
	...
	registry.Register(tools.NewReadFileTool("."))
	m := newModel(context.Background(), Options{
		Cwd:            `D:\codings\Opensource\green`,
		ProviderName:   "openai",
		ModelName:      "gpt-4.1",
		Registry:       registry,
		PermissionMode: agent.PermissionModeAsk,
...
func TestModelCommandPersistsSelectedModelToUserConfig(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "green.json")
...
func TestResumePickerSelectionHydratesSession(t *testing.T) {
	...
	if !transcriptContains(next.transcript, "Resumed green session") || !transcriptContains(next.transcript, target.SessionID) {
...
func TestResumeCommandWithUnknownIDReportsMissingSession(t *testing.T) {
	...
	m.input.SetValue("/resume green_123")

	updated, _ := m.Update(testKey(tea.KeyEnter))
	next := updated.(model)

	if !transcriptContains(next.transcript, "green session not found: green_123") {
...
func TestPromptSubmitAppendsUserAndAssistantRows(t *testing.T) {
	provider := &fakeProvider{events: []greenruntime.StreamEvent{
		...
		{Type: greenruntime.StreamEventText, Content: " back"},
		{Type: greenruntime.StreamEventDone},
	}}
	m := newModel(context.Background(), Options{
		Provider:     provider,
...
func TestAgentEventRenderingMappingCoversRuntimeContract(t *testing.T) {
	surfaces := map[greenruntime.AgentEventType]string{
		greenruntime.AgentEventText:       "assistant transcript row",
		greenruntime.AgentEventToolCall:   "tool call transcript row",
		greenruntime.AgentEventToolResult: "tool result transcript row",
		greenruntime.AgentEventThinking:   "deferred: no transcript row until runtime emits thinking deltas",
		greenruntime.AgentEventUsage:      "usage tracker footer segment",
		greenruntime.AgentEventPlanUpdate: "system transcript row from /plan",
		greenruntime.AgentEventError:      "error transcript row",
		greenruntime.AgentEventTurnEnd:    "control boundary, no transcript row",
	}
	for _, eventType := range []greenruntime.AgentEventType{
		greenruntime.AgentEventText,
		greenruntime.AgentEventToolCall,
		greenruntime.AgentEventToolResult,
		greenruntime.AgentEventThinking,
		greenruntime.AgentEventUsage,
		greenruntime.AgentEventPlanUpdate,
		greenruntime.AgentEventError,
		greenruntime.AgentEventTurnEnd,
	} {
	...
	renderedRows := map[greenruntime.AgentEventType]struct {
		row   transcriptRow
		wants []string
	}{
		greenruntime.AgentEventText: {
	...
	m.width = 96
	m, usageRows := m.recordUsageEvent("gpt-4.1", greenruntime.Usage{InputTokens: 100, OutputTokens: 20})
	if len(usageRows) != 0 {
...
func TestSelectionHighlightUsesGutterShiftedCoordinate(t *testing.T) {
	...
	if !strings.Contains(styled, greenTheme.selection.Render("world")) {
...
func TestTranscriptSelectionPaintsHighlightOnceNotTwice(t *testing.T) {
	...
	sentinel := greenTheme.selection.Render("\x00")
