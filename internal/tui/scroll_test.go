...
func TestAltScreenTranscriptScrollKeepsFooterFixed(t *testing.T) {
	...
	if !strings.Contains(bottom, "describe a task for green") || !strings.Contains(bottom, "openai") {
	...
	if !strings.Contains(scrolled, "describe a task for green") || !strings.Contains(scrolled, "openai") {
