...
// curated greenTheme styles ordered cold (faint) at the cosine troughs to
...
func ripplePalette() []lipgloss.Style {
	...
	bright := greenTheme.accent.GetForeground()
	dim := greenTheme.faint.GetForeground()
	if bright == nil || dim == nil {
		return []lipgloss.Style{greenTheme.faint, greenTheme.muted, greenTheme.accent}
	}
