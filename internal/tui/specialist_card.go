...
func (m model) renderSpecialistCard(info specialistInfo, width int) string {
	...
	// to green so very long names never underflow.
	...
	case specialistRunning:
		icon := m.spinnerGlyph()
		header = greenTheme.accent.Render(fmt.Sprintf("%s%s · %s · %s", icon, info.name, description, elapsedStr))
	case specialistCompleted:
		header = greenTheme.green.Render(fmt.Sprintf("✓ %s · %s · %s", info.name, description, elapsedStr))
	case specialistError:
		header = greenTheme.red.Render(fmt.Sprintf("✗ %s · %s · %s", info.name, description, elapsedStr))
	default:
		header = greenTheme.accent.Render(fmt.Sprintf("• %s · %s · %s", info.name, description, elapsedStr))
	}
	...
	// the segment when it is green rather than advertise a misleading "0 tokens" (M18).
	...
	var body string
	if info.status == specialistError {
		body = greenTheme.red.Render(bodyText)
	} else {
		body = greenTheme.muted.Render(bodyText)
	}
	...
	body += greenTheme.faint.Render("   · enter to open")
	...
	if info.status == specialistRunning && info.currentTool != "" {
		...
		lines = append(lines, greenTheme.muted.Render(progressLine))
	}
	...
	if info.status == specialistError && strings.TrimSpace(info.errorMsg) != "" {
		...
		lines = append(lines, greenTheme.red.Render("  "+errMsg))
	}
...
func specialistBorderStyle(status specialistStatus) lipgloss.Style {
	switch status {
	case specialistRunning:
		return greenTheme.cardRun
	case specialistError:
		return greenTheme.cardErr
	default:
		return greenTheme.line
	}
}
...
func renderSpecialistSummary(specialists []specialistInfo, spinnerView string) string {
	...
	return greenTheme.accent.Render(spinnerView) + greenTheme.muted.Render(summary[tailStart:])
}
