...
// Run starts the green Bubble Tea shell and returns a process-style exit code.
func Run(ctx context.Context, options Options) int {
	...
	// arrive (e.g. `echo "" | green`). Fail fast with guidance toward the headless
	...
	if !term.IsTerminal(os.Stdin.Fd()) {
		fmt.Fprintln(os.Stderr, "green: the interactive shell needs a terminal (stdin is not a TTY). For non-interactive use, run: green exec \"<prompt>\"")
		return 2
	}
	...
	if _, err := program.Run(); err != nil {
		// Surface the failure: exiting 1 with green diagnostics left users
		// guessing why the default chat surface died.
		fmt.Fprintln(os.Stderr, "green: tui error:", err)
		return 1
	}
	return 0
}
