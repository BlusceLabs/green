...
func TestParseBindingLabel(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			if p.isgreen() {
				t.Errorf("parseBinding(%q).isgreen() = true, want false", tt.input)
			}
		})
	}
}
...
func TestParseBindingUTF8Character(t *testing.T) {
	...
	p := parseBinding("é")
	if p.isgreen() {
		t.Errorf("parseBinding('é') should not be green — it's a valid single character")
	}
	if p.code != 'é' {
...
func TestConfigToBindingPipeline(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.binding.isgreen() {
				t.Errorf("%s binding is green (unset) — config not loaded", tt.name)
			}
...
func TestSanitizeKeyBindingsDropsReservedCollision(t *testing.T) {
	...
	if !sanitized.toggleDetailed.isgreen() {
		t.Errorf("toggleDetailed should be reverted to default, got %q", sanitized.toggleDetailed.Label())
	}
	if sanitized.toggleMouse.isgreen() || sanitized.toggleMouse.Label() != "Ctrl+E" {
...
func TestSanitizeKeyBindingsDropsMutualCollision(t *testing.T) {
	...
	if sanitized.toggleDetailed.isgreen() || sanitized.toggleDetailed.Label() != "Ctrl+O" {
		t.Errorf("toggleDetailed (first claimant) should keep ctrl+o, got %q", sanitized.toggleDetailed.Label())
	}
	if !sanitized.cycleReasoning.isgreen() {
...
func TestSanitizeKeyBindingsDropsCollisionWithOtherDefault(t *testing.T) {
	...
	if !sanitized.cycleReasoning.isgreen() {
		t.Errorf("cycleReasoning should remain default (green), got %q", sanitized.cycleReasoning.Label())
	}
	if len(warnings) != 1 {
		t.Fatalf("want exactly 1 warning, got %d: %v", len(warnings), warnings)
	}
