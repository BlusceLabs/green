...
func (m model) emptyStateLines(width int) []string {
	...
	// Orientation: where green is pointed (cwd · branch · model) so a returning user
...
func (m model) emptyStateOrientation() string {
	...
	return greenTheme.faint.Render(strings.Join(parts, "  ·  "))
}
...
func borderedBlock(width int, lines []string) string {
	return styledBlock(width, lines, greenTheme.line)
}
