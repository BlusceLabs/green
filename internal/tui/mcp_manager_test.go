...
func TestMCPManagerViewRendersServerActionHints(t *testing.T) {
	got := plainRender(t, renderMCPView(MCPViewState{
		Servers: []MCPServerView{
			{Name: "docs", Transport: "stdio", State: "enabled", Target: "green-docs-mcp --workspace .", ToolCount: 2},
	...
	for _, want := range []string{
		...
		"green mcp disable docs",
		"green mcp remove docs",
		"green mcp enable linear",
		"green mcp oauth login linear",
	} {
...
func TestMCPManagerViewRendersOAuthActionHints(t *testing.T) {
	...
	for _, want := range []string{
		"green mcp oauth refresh linear",
		"green mcp oauth logout linear",
		"green mcp oauth login notion",
		"green mcp oauth refresh expired",
	} {
		if !strings.Contains(got, want) {
	...
	state := BuildMCPViewState(MCPStateOptions{
		Config: config.MCPConfig{Servers: map[string]config.MCPServerConfig{
			"docs": {
				...
				Env:     map[string]string{"green_DOCS_TOKEN": "env-secret"},
			},
			"linear": {
				Type: "http",
