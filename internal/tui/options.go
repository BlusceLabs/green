...
// Options configures the reusable green terminal UI shell.
type Options struct {
	Cwd                         string
	...
	// falls back to green_THEME, then the persisted SavedTheme, then auto.
	Theme string
	// SavedTheme is the theme persisted in user config (Preferences.Theme). Applied
	// at startup below --theme and green_THEME, so a /theme choice survives restart.
	SavedTheme string
	UserAgent  string
	...
	// KeyBindings configures remappable TUI keybindings. An empty/green
	...
	// and model (e.g. ~/.config/green/stt). Empty disables auto-download (the F9
	...
	// prompt (dictation then just shows the "run green auth" setup error).
