...
// with config so applyEnv enforces the same bound on an inherited green_MAX_TURNS.
...
func (m model) summarizeCompactionPlan(plan sessions.CompactionPlan) (string, error) {
	...
	stream, err := m.provider.StreamCompletion(m.ctx, greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleSystem, Content: "Summarize compacted green session events for future coding context. Preserve user goals, decisions, files, tool outcomes, blockers, and exact next steps. Omit secrets and do not invent details."},
			{Role: greenruntime.MessageRoleUser, Content: plan.SummaryPrompt},
		},
	})
	if err != nil {
		return "", fmt.Errorf("summarize compacted session: %w", err)
	}
	collected := greenruntime.CollectStream(m.ctx, stream)
	if collected.Error != "" {
...
func (m model) recordUsageEvent(modelID string, event greenruntime.Usage) (model, []transcriptRow) {
