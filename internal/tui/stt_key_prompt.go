...
import (
	...
	"github.com/BlusceLabs/green/internal/dictation"
)
...
func (m model) sttKeyPromptOverlay(width int) string {
	...
	if p.input != "" {
		value = greenTheme.ink.Render(maskedProviderWizardKey(p.input)) + greenTheme.accent.Render("▌")
	}
	input := greenTheme.userPrompt.Render("api key > ") + value
	intro := p.label + " isn't set up yet. Paste or type your " + p.label + " API key:"
	footer := "⏎ save · Esc cancel · stored in green's credential store"
	if p.optional {
		...
		footer = "⏎ replace · Esc keep saved key · stored in green's credential store"
	}
	lines := []string{
		greenTheme.ink.Render(intro),
		"",
		input,
		"",
		greenTheme.faint.Render(footer),
	}
	return centerRenderedBlock(styledBlockFillTitle(overlayWidth, p.label+" API key", lines, greenTheme.lineStrong, lipgloss.NewStyle()), width)
}
