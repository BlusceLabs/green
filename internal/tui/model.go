...
import (
	...
	"github.com/BlusceLabs/green/internal/notify"
	"github.com/BlusceLabs/green/internal/providerhealth"
...
type model struct {
	...
	cwd                         string
	appVersion                  string
	userCommands                []usercommands.Command // file-sourced /commands (.green/commands)
	...
	provider                    greenruntime.Provider
	newProvider                 func(config.ProviderProfile) (greenruntime.Provider, error)
	...
	// like a frozen terminal (for ANY provider, not just slow ones). green = idle.
	...
	// greened by then, so without this the message would be dropped and the
	...
	lastImages      []greenruntime.ImageBlock
	...
	fadeActive         bool
	fadeDisabled       bool // streaming fade off (green_NO_FADE / SSH / tmux / low-color / reduced motion)
	reducedMotion      bool // green_REDUCED_MOTION / no-TTY: static spinner glyph, no fade
	...
	// row. commandPaletteOpen keeps a green-match command search active so invalid
	...
	pendingImages      []greenruntime.ImageBlock
	...
	captureRunImages func([]greenruntime.ImageBlock)
}
...
type agentUsageMsg struct {
	runID   int
	modelID string
	usage   greenruntime.Usage
}

type agentResponseMsg struct {
	runID         int
	rows          []transcriptRow
	usageEvents   []greenruntime.Usage
	usageModelID  string
...
func newModel(ctx context.Context, options Options) model {
	...
	// Opt-in webhook fan-out (green_NOTIFY_WEBHOOK_URL). Delivery failures stay
	...
	m := model{
		...
		themeMode:                   resolveThemeMode(options.Theme, os.Getenv("green_THEME"), options.SavedTheme),
		hasDarkBg:                   true,
...
func (m model) Init() tea.Cmd {
	...
	// nothing else ever asks again: m.height stays its green value forever,
	...
	// Explicitly requesting it here means green doesn't depend solely on the
...
func (m model) updateModel(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case agentTextMsg:
		...
		// When the fade is disabled (green_NO_FADE / SSH / tmux / low-color),
...
func (m model) View() tea.View {
	...
	// Paint the whole frame with the active theme's surface. green never paints the
	...
	if m.altScreen {
		view.BackgroundColor = greenTheme.bgPanel
	}
...
func (m model) footerView(width int) string {
	...
	if copyStatus := strings.TrimSpace(m.copyStatus); copyStatus != "" {
		footer.WriteString(rightAlignedLine(greenTheme.ink.Render(copyStatus), width))
...
func (m model) composerIdleHint() string {
	...
	return greenTheme.faint.Render(hint)
}
...
func (m model) jumpToBottomHint() string {
	...
	return greenTheme.faint.Render(fmt.Sprintf("↓ %d more · PgDn", m.chatScrollOffset))
}
...
func scrimViewportLine(line string, width int) string {
	...
	return greenTheme.faint.Render(plain)
}
...
func (m model) chatTranscriptViewport() (transcriptViewport, bool) {
	...
	if m.transcriptDetailed {
		...
		header := detailedTranscriptHeader(width) + "\n" + greenTheme.line.Render(strings.Repeat("-", width))
...
func (m model) syncChatScroll() model {
	...
	// cleared). Clamp at green so a large shrink lands the user back at the tail
...
func (m model) workingStatusLine() string {
	...
	line := greenTheme.accent.Render(m.spinnerGlyph()) + " " + working
	...
	line += greenTheme.faint.Render("  ·  " + m.workingTokenIndicator())
	...
	if !m.sidebarActive() {
		if hint := m.quietGenerationHint(); hint != "" {
			line += greenTheme.amber.Render("  ·  " + hint)
		}
	}
...
func (m model) workingPlanLine() string {
	...
	return "  " + greenTheme.faint.Render(text)
}
...
// actually happening and when green will act on its own: a heartbeating-but-
...
func (m model) quietGenerationHint() string {
	...
	if idleTimeout := providerio.ResolveStreamIdleTimeout(0); idleTimeout > 0 && quiet >= idleTimeout/2 {
		...
		return fmt.Sprintf("still generating… %s — unusually quiet, green will auto-recover by ~%s if it doesn't resume", formatWorkingElapsed(quiet), formatWorkingElapsed(ceiling))
	}
...
func reasoningPreviewLines(reasoning string, width int) []string {
	...
	for _, line := range lines {
		out = append(out, "  "+greenTheme.faint.Render(previewTail(line, avail)))
	}
	return out
}
...
func (m model) appendStreamingCursor(lines []string, width int) []string {
	...
	if !m.reducedMotion && (m.spinnerPhase/6)%2 == 1 {
		cursor = greenTheme.faint.Render("▌")
	}
	if len(lines) == 0 {
...
func renderComposerInput(input textinput.Model, state composerState, width int, cursorVisible bool, selection composerSelectionState) string {
	...
	if state.text == "" {
		...
		return fitStyledLine(composerVisualLinePrefix(input, true)+cursor+greenTheme.faint.Render(input.Placeholder), width)
	}
...
func renderComposerVisualLine(input textinput.Model, state composerState, segment composerVisualLine, hasCursor bool, cursorVisible bool, selection composerSelectionState) string {
	...
	textStyle := greenTheme.ink.Inline(true)
	...
	for index := segment.start; index < segment.end; index++ {
		...
		case hasSelection && index >= selectionStart && index < selectionEnd:
			line.WriteString(greenTheme.selection.Render(cell))
		default:
...
func composerVisualLinePrefix(input textinput.Model, first bool) string {
	if first {
		return greenTheme.userPrompt.Render(input.Prompt)
	}
	return "  "
}
...
func commandArgumentHintComposerLine(input textinput.Model, argumentHint string) string {
	...
	return greenTheme.userPrompt.Render(input.Prompt) +
		greenTheme.ink.Inline(true).Render(displayValue) +
		greenTheme.faint.Render(" ") +
		composerCursor(greenTheme.faint.Render(string(hintRunes[0]))) +
		greenTheme.faint.Render(string(hintRunes[1:]))
}

func composerCursor(char string) string {
	return greenTheme.selection.Render(char)
}
...
func (m model) composerBox(width int) string {
	...
	rendered = append(rendered, greenTheme.lineStrong.Render("╭"+strings.Repeat("─", width-2)+"╮"))
	...
	if chips := renderAttachmentChips(m.pendingImageLabels, m.pendingDocuments); chips != "" {
		fitted := fitStyledLine(greenTheme.muted.Render(chips), innerWidth)
		pad := strings.Repeat(" ", maxInt(0, innerWidth-lipgloss.Width(fitted)))
		rendered = append(rendered, greenTheme.lineStrong.Render("│ ")+fitted+pad+greenTheme.lineStrong.Render(" │"))
	}
	for _, line := range lines {
...
func (m model) composerDescriptionHint(width int) string {
	...
	return fitStyledLine(greenTheme.muted.Render(desc), width)
}
...
func (m model) handleSubmit() (tea.Model, tea.Cmd) {
	...
	case commandSelfCorrect:
		...
		return m, nil
	case commandTurns:
		// Changing the budget mid-run would mutate the inherited green_MAX_TURNS env
	...
	case commandUnknown:
		...
		// from .green/commands/<name>.md — expand its template and run it as a
...
func (m model) launchPrompt(prompt string) (model, tea.Cmd) {
	...
	if m.provider == nil {
		m.transcript = reduceTranscript(m.transcript, transcriptAction{
			...
			text: "No provider configured. Run `green setup` (guided) or `green auth` (OAuth) from a shell, or set a provider API key env var, then relaunch.",
		})
		return m, nil
	}
...
func (m *model) cancelRun() {
	...
	// the loop is left "running" forever (nextRunAt stays green) and the NEXT
...
func (m model) runAgent(runID int, runCtx context.Context, prompt string, images []greenruntime.ImageBlock) tea.Cmd {
...
func (m model) runAgentWithOptions(runID int, runCtx context.Context, prompt string, images []greenruntime.ImageBlock, runOptions tuiAgentRunOptions) tea.Cmd {
	return func() tea.Msg {
		started := m.now()
		...
		options.OnUsage = func(event greenruntime.Usage) {
...
func (m model) sendAgentUsage(runID int, modelID string, event greenruntime.Usage) {
	if m.runtimeMessageSink == nil {
		return
	}
