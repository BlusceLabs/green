...
func (m model) sidebarFilesHeader(width int) string {
	...
	return sidebarHeaderWithCount("FILES", fmt.Sprintf("%d", n), greenTheme.muted, width)
}
...
func (m model) sidebarFileLines(width int) ([]string, []fileHit) {
	...
	if live != "" {
		lines = append(lines, " "+greenTheme.accent.Render("●")+" "+
			greenTheme.ink.Render(truncatePathLeft(live, room)))
	}
	...
	for _, f := range visible {
		if shown >= maxSidebarFiles {
			lines = append(lines, "   "+greenTheme.faint.Render(fmt.Sprintf("+%d more", len(visible)-shown)))
			break
		}
		shown++
...
func (m model) renderFileRow(f touchedFile, room int) string {
	badge := greenTheme.muted.Render("M")
	switch {
	case f.failed:
		badge = greenTheme.red.Render("✗")
	case f.created:
		badge = greenTheme.green.Render("A")
	}
	...
	pathStyle := greenTheme.muted
	lead := " "
	if m.selectedFile == f.path {
		lead = greenTheme.accent.Render("▸")
		pathStyle = greenTheme.ink
	}
	line := lead + badge + " " + pathStyle.Render(truncatePathLeft(f.path, pathRoom))
	if stat != "" {
		line += greenTheme.faintest.Render(stat)
	}
	return line
}
