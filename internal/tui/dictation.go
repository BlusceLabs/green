...
func (m model) handleDictationTranscribed(msg dictationTranscribedMsg) (tea.Model, tea.Cmd) {
	...
	if msg.err != nil && !errors.Is(msg.err, context.Canceled) {
		...
		// retry, instead of hitting a dead-end "run green auth" line.
...
func (m model) dictationStatusChip() string {
	...
	case dictStarting, dictRecording:
		...
		wave := greenTheme.amber.Render("● " + renderWaveBars(m.dictation.waveBars) + " REC")
		return wave + greenTheme.muted.Render(" · "+stop+", Esc to cancel")
	case dictTranscribing:
		return greenTheme.accent.Render("●") + " " + greenTheme.muted.Render("transcribing…")
	}
	return ""
}
...
func (m model) voiceModeIndicator() string {
	...
	return greenTheme.accent.Render("🎙 voice") + greenTheme.muted.Render(" · "+m.dictation.currentModelLabel())
}
