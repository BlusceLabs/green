...
func managerTestModel(t *testing.T) model {
	...
	t.Setenv("green_OAUTH_TOKENS_PATH", filepath.Join(home, "oauth-tokens.json"))
...
// the entry `green auth logout` would actually delete (PR #560 review, P3).
func TestProviderManagerDeleteHintNamesActualOAuthLogin(t *testing.T) {
	...
	if !strings.Contains(status, "green auth logout chatgpt") {
...
func TestProviderManagerReadsStoredKeyBesideConfig(t *testing.T) {
	...
	m.newProvider = func(profile config.ProviderProfile) (greenruntime.Provider, error) {
		built = profile
		return &fakeProvider{}, nil
	}
	m = managerKey(t, m, testKey(tea.KeyDown)) // select "backup"
