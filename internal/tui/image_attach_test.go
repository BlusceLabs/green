...
func TestSubmitThreadsImagesThenClears(t *testing.T) {
	...
	captured := make(chan []greenruntime.ImageBlock, 1)
	...
	m.captureRunImages = func(imgs []greenruntime.ImageBlock) { captured <- imgs }
...
func TestSubmitPrependsDocumentTextThenClears(t *testing.T) {
	...
	if last.Role != greenruntime.MessageRoleUser {
...
func TestRetryResendsAttachments(t *testing.T) {
	...
	m.lastImages = []greenruntime.ImageBlock{{MediaType: "image/png", Data: []byte{0x89, 'P', 'N', 'G'}}}
