...
func generateSessionTitle(ctx context.Context, provider greenruntime.Provider, digest string) (string, error) {
	if provider == nil {
	...
	request := greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleSystem, Content: sessionTitleSystemPrompt},
			{Role: greenruntime.MessageRoleUser, Content: "Conversation:\n\n" + digest + "\n\nTitle:"},
		},
	}
