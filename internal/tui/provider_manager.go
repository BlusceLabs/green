...
// exports green_PROVIDER, warms discovery). On success the manager closes and
...
// doesn't force a browser round-trip; green auth logout removes it.
...
func providerManagerCleanupCmd(configPath string, profile config.ProviderProfile) tea.Cmd {
	...
	return func() tea.Msg {
		...
		if login, ok := oauthLoginName(config.ProviderProfile{Name: name, CatalogID: catalogID}); ok {
			notes = append(notes, "OAuth login kept — remove with `green auth logout "+login+"`.")
		}
...
// live session follows a rename so green_PROVIDER and the status line never
...
func (m model) saveManagerEdit() (model, tea.Cmd) {
	...
	// is running on: the exported green_PROVIDER must resolve for spawned children.
...
func (wizard *providerWizardState) renderManageStep(width int) []string {
	...
	if wizard.manageStatus != "" {
		lines = append(lines, fitStyledLine(greenTheme.accent.Render(wizard.manageStatus), width), "")
	}
	if len(rows) == 0 {
		lines = append(lines, greenTheme.faint.Render("  No providers saved — press a to add one."))
		return lines
	}
	...
	for offset, row := range rows[start : start+maxVisible] {
		...
		if strings.EqualFold(strings.TrimSpace(row.profile.Name), strings.TrimSpace(wizard.manageActiveName)) {
			active = surface(greenTheme.accent).Render(" ● active")
		}
		name := padProviderManagerCell(row.profile.Name, nameWidth)
		meta := providerManagerRowMeta(row.profile)
		left := marker + surface(greenTheme.ink).Render(name) + "  " + surface(greenTheme.faint).Render(meta)
		right := surface(greenTheme.faint).Render(providerManagerCredDisplay(row)) + active
	...
	if row, ok := wizard.currentManagerRow(); ok {
		lines = append(lines, greenTheme.line.Render(strings.Repeat("─", width)))
		...
		if wizard.manageDeleting {
			lines = append(lines, fitStyledLine(greenTheme.red.Render("Delete "+row.profile.Name+"? This also removes its stored API key.  Enter/y confirm · Esc/n cancel"), width))
		}
	}
	return lines
}
...
func (wizard *providerWizardState) renderEditMenuStep(width int) []string {
	lines := []string{greenTheme.accent.Render("Edit " + wizard.editOriginal.Name)}
	...
	for index, entry := range providerEditFields {
		...
		left := marker + surface(greenTheme.ink).Render(padProviderManagerCell(entry.label, 12))
		if display != "" {
			left += surface(greenTheme.faint).Render(display)
		}
		lines = append(lines, fillPaletteLine(fitStyledLine(left, width), width, surface))
	}
	entry := providerEditFields[wizard.editCursor]
	lines = append(lines, "", fitStyledLine(greenTheme.faint.Render(entry.hint), width))
	return lines
}
...
func (wizard *providerWizardState) renderEditValueStep(width int) []string {
	...
	prompt := greenTheme.userPrompt.Render(entry.label + " > ")
	if value == "" {
		value = greenTheme.faint.Render("(empty)")
	} else {
		value = greenTheme.ink.Render(value)
	}
	return []string{
		greenTheme.accent.Render("Edit " + wizard.editOriginal.Name + " — " + entry.label),
		fitStyledLine(prompt+value, width),
		"",
		fitStyledLine(greenTheme.faint.Render(entry.hint), width),
	}
}
