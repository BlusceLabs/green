...
// at green so content starts at the chat edge and tool/code blocks can use the
...
func (m model) sidebarAgentHeader(width int) string {
	...
	return sidebarHeaderWithCount("AGENTS", fmt.Sprintf("%d", n), greenTheme.muted, width)
}
...
type swarmAgent struct {
	...
	finishedAt time.Time // when first seen finished (green until the spinner tick stamps it)
}
...
func (m model) sidebarAgentRows(width int) ([]string, []sidebarAgentHit) {
	...
	for _, a := range specialists {
		...
		case specialistRunning:
			...
			icon = greenTheme.accent.Render(m.spinnerGlyph())
		case specialistError:
			icon = greenTheme.red.Render("✗")
		default: // completed
			icon = greenTheme.green.Render("✓")
		}
		name := strings.TrimSpace(a.name)
		if name == "" {
			name = "agent"
		}
		nameStyle := greenTheme.ink
		...
		if a.status != specialistRunning && m.agentExitFading(a.completedAt) {
			...
			icon = greenTheme.faint.Render(glyph)
			nameStyle = greenTheme.faint
		}
		...
		if detail != "" {
			lines = append(lines, "   "+greenTheme.faint.Render("↳ "+truncateStep(detail, maxInt(2, room-2))))
		}
	}
	...
	for _, a := range swarm {
		...
		if a.finishing {
			icon := greenTheme.green.Render("✓")
			nameStyle := greenTheme.muted
			if m.agentExitFading(a.finishedAt) {
				icon = greenTheme.faint.Render("✓")
				nameStyle = greenTheme.faint
			}
		...
		if st := strings.TrimSpace(a.state); st != "" && st != "running" {
			suffix = " " + greenTheme.faint.Render(st)
			nameRoom = maxInt(4, room-len(st)-1)
		}
		lines = append(lines, " "+greenTheme.accent.Render("•")+" "+style.Render(truncateStep(a.name, nameRoom))+suffix)
	}
	return lines, hits
}
...
func swarmPulseStyles() []lipgloss.Style {
	fg := greenTheme.blue.GetForeground()
	dim := greenTheme.muted.GetForeground()
	if fg == nil || dim == nil {
		return nil
	}
...
func (m model) renderContextSidebar(width, height int) []string {
	...
	if lineOffset, ok := m.hoveredSidebarLineOffset(width); ok && lineOffset >= 0 && lineOffset < len(lines) {
		lines[lineOffset] = greenTheme.hover.Render(ansi.Strip(lines[lineOffset]))
	}
...
func sidebarHeader(label string, _ int) string {
	return greenTheme.muted.Bold(true).Render(strings.ToUpper(label))
}
...
func sidebarHeaderWithCount(label, count string, countStyle lipgloss.Style, width int) string {
	left := greenTheme.muted.Bold(true).Render(strings.ToUpper(label))
...
func sidebarPlaceholder(text string, width int) string {
	return " " + greenTheme.faint.Render(truncateRunes(text, maxInt(1, width-1)))
}
...
func (m model) sidebarPlanHeader(width int) string {
	...
	countStyle := greenTheme.accent
	if done == total {
		countStyle = greenTheme.green
	}
...
func (m model) sidebarPlanLines(width int) []string {
	...
	for _, step := range state.steps {
		...
		case "completed":
			icon = greenTheme.green.Render("✓")
			body = greenTheme.muted.Render(truncateStep(step.content, room))
		case "in_progress":
			icon = greenTheme.accent.Render("•")
			body = greenTheme.ink.Render(truncateStep(step.content, room))
		case "failed":
			icon = greenTheme.red.Render("✗")
			body = greenTheme.muted.Render(truncateStep(step.content, room))
		default: // pending
			icon = greenTheme.faint.Render("○")
			body = greenTheme.faint.Render(truncateStep(step.content, room))
		}
...
func (m model) sidebarActivityLines(width, budget int) []string {
	...
	for i := len(m.transcript) - 1; i >= 0 && len(work) < limit && scanned < maxSidebarActivityScan; i-- {
		...
		work = append(work, " "+glyph+" "+greenTheme.muted.Render(truncateStep(m.activitySummary(row), room)))
	}
	live := ""
	if m.activeRunID != 0 {
		if hint := m.quietGenerationHint(); hint != "" {
			live = " " + greenTheme.accent.Render("•") + " " + greenTheme.faint.Render(truncateStep(hint, room))
		}
	}
...
func (m model) sidebarTokenLine(width int) string {
	...
	if pct, _, _, style, ok := m.contextFillPercent(); ok {
		chip = greenTheme.faint.Render(" · ") + style.Render(fmt.Sprintf("%d%%", pct))
	}
	budget := maxInt(1, width-1-lipgloss.Width(chip))
	return " " + greenTheme.faint.Render(truncateRunes(label, budget)) + chip
}
...
func joinColumns(chat []string, sidebar []string, chatW, sidebarW int) []string {
	...
	divider := " " + greenTheme.line.Render("│") + " "
