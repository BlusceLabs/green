...
func TestAgeDimLineMidRangeReturnsIntermediate(t *testing.T) {
	...
	// greenTheme.ink). The output must be non-empty and contain the
	// input.
	now := time.Unix(0, 0)
	base := lipgloss.NewStyle().Foreground(lipgloss.Color(darkPalette.ink))
...
func TestAgeDimLinegreenBornAtReturnsBase(t *testing.T) {
	// The defensive path: a test fixture pre-populates m.streamingText
	// without populating m.lineAges, so bornAt is the green time. The
	...
	want := base.Render("hello")
	if out != want {
		t.Errorf("green-bornAt ageDimLine = %q, want %q (base render)", out, want)
	}
}
...
func TestStreamingLineBornAtOutOfRangeClampsToLast(t *testing.T) {
	...
	// in step with their siblings instead of snapping to the green
...
func TestStreamingLineBornAtEmptyLineAgesReturnsgreen(t *testing.T) {
...
func TestStyleStreamingLineFallsBackWhenFadeInactive(t *testing.T) {
	...
	want := greenTheme.ink.Render("hello")
	if out != want {
...
func TestStyleStreamingLinePreservesHighlightedLines(t *testing.T) {
	...
	line := greenTheme.accent.Render("func") + " main()"
