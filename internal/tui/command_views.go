...
func (m *model) mcpViewState() MCPViewState {
	...
	// Older tests may construct a green-value model; keep that path useful, while
...
func (m model) permissionsTextWithStore(store grantLister) string {
	...
	snapshots := greencommands.SandboxGrantSnapshots(grants)
...
func (m model) providerText() string {
		...
		return renderCommandOutput(commandOutput{
			...
			Sections: []commandSection{
				...
				{Title: "Next actions", Lines: []string{
					"green providers catalog",
					"green providers setup openai --set-active",
					"green providers add openai --api-key-env OPENAI_API_KEY --set-active",
				}},
			},
		})
	}

	snapshot := greencommands.ProviderSnapshotFromProfile(m.providerProfile, true)
	...
	// as "api key: not set" with a warning, unlike `green providers list` which
...
func providerNextActionLines(profile config.ProviderProfile, snapshot greencommands.ProviderSnapshot, activeName string) []string {
	...
	if providerCredentialRequired(profile, snapshot.ProviderKind) && !providerProfileHasCredential(profile) {
		if envName := providerCredentialEnvName(profile, snapshot.ProviderKind); envName != "" {
			lines = append(lines,
				...
				"green providers add "+setupID+" --api-key-env "+envName+" --set-active",
			)
		} else {
	...
	return append(lines,
		"green providers check "+providerName+" --connectivity",
		"green providers catalog",
		"green providers setup "+setupID+" --set-active",
	)
}
...
func (m model) skillsText() string {
	return renderCommandOutput(commandOutput{
		...
		Hints: []string{
			"install one: create <skills-dir>/<name>/SKILL.md (see `green skills`)",
		},
	})
}
