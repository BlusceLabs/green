...
func TestEnterWithNoSuggestionStillSubmits(t *testing.T) {
	m := newModel(context.Background(), Options{})
	m.input.SetValue("hello green") // plain prompt, no suggestions
	...
	if !transcriptContains(next.transcript, "hello green") {
...
func TestSuggestionsSuppressedDuringSpecReview(t *testing.T) {
	...
	m.pendingSpecReview = &pendingSpecReviewPrompt{SpecID: "spec-1", SpecFilePath: ".green/specs/spec-1.md"}
...
func TestSuggestionOverlayStaysVisibleWhenTranscriptScrolled(t *testing.T) {
	...
	for index, line := range lines {
		...
		case strings.Contains(line, "no model"):
			// The composer rule shows the model; the mode ("auto") is now on
