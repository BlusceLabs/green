...
func TestSandboxCheckReportsWindowsNativeSetupStates(t *testing.T) {
	...
	t.Setenv("green_WINDOWS_SANDBOX_HOME", t.TempDir())
	...
	if setupStatus != "missing-or-out-of-date" || !strings.Contains(setupRemedy, "green sandbox setup") {
	...
	// on PATH for the Windows backend to be usable — the running green binary acts
	...
	// require a setup marker) the check passes. The actionable "run `green sandbox
