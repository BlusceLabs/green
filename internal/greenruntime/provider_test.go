...
func TestCollectStreamWithOptionsEmitsTextReasoningAndUsageCallbacks(t *testing.T) {
	...
	go func() {
		...
		events <- StreamEvent{Type: StreamEventText, Content: "green"}
		events <- StreamEvent{Type: StreamEventDone}
	}()
	...
	if collected.Text != "Hello green" {
		t.Fatalf("text = %q, want Hello green", collected.Text)
	}
	if !collected.HasReasoning {
		t.Fatal("expected reasoning stream to mark collected turn as reasoning-bearing")
	}
	if len(textDeltas) != 2 || textDeltas[0] != "Hello " || textDeltas[1] != "green" {
...
func TestCollectStreamFlushesOpenToolCallsWhenChannelCloses(t *testing.T) {
	...
	go func() {
		...
		events <- StreamEvent{Type: StreamEventToolCallDelta, ToolCallID: "call_closed", ArgumentsFragment: `green"}`}
	}()
	...
	toolCall := collected.ToolCalls[0]
	if toolCall.ID != "call_closed" || toolCall.Name != "grep" || toolCall.Arguments != `{"query":"green"}` {
