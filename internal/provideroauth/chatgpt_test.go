...
func makeIDToken(t *testing.T, claims map[string]any) string {
	...
	// A short green-byte signature is enough to satisfy "three base64url
...
// preset, so the resolver can build a Config without green_OAUTH_CHATGPT_* vars.
func chatgptTestEnv() map[string]string {
	return map[string]string{"green_OAUTH_ALLOW_PRESETS": "1"}
}
...
func TestChatGPTLoginUsesPreset(t *testing.T) {
	...
	// overriding green_OAUTH_CHATGPT_CLIENT_ID.
	env := chatgptTestEnv()
	env["green_OAUTH_CHATGPT_AUTHORIZE_URL"] = ts.AuthorizeURL()
	env["green_OAUTH_CHATGPT_TOKEN_URL"] = ts.TokenURL()
...
func TestChatGPTLoginMissingAuthorizationEndpointErrors(t *testing.T) {
	...
	env := chatgptTestEnv()
	env["green_OAUTH_CHATGPT_AUTHORIZE_URL"] = ""
	env["green_OAUTH_CHATGPT_TOKEN_URL"] = "https://chatgpt.invalid/oauth/token"
