...
func TestFormatSummaryIncludesWarnings(t *testing.T) {
	...
	result := Result{
		...
		ColdStartCommand:    []string{"/repo/green", "--version"},
		FirstOutputCommand:  []string{"/repo/green", "--version"},
		Iterations:          2,
		WarmupIterations:    1,
...
func TestPerfBenchHelperProcess(t *testing.T) {
	if os.Getenv("green_PERF_HELPER") != "1" {
		return
	}
	fmt.Println("green 0.1.0")
	os.Exit(0)
}
