...
func TestRunTasksRecordsRunnerError(t *testing.T) {
	config := TaskConfig{
		Model: "m",
		Runner: func(_ context.Context, task BenchTask, _ RunContext) TaskOutcome {
			return TaskOutcome{Err: errors.New("green exec exited 1")}
		},
	}
	...
	if !strings.Contains(result.Tasks[0].Detail, "green exec exited 1") {
...
func writeExecStub(t *testing.T, body string) string {
	...
	dir := t.TempDir()
	path := filepath.Join(dir, "green-stub.sh")
	script := "#!/bin/sh\n" + body
...
func TestNewExecRunnerNongreenRunEndIsFailNotError(t *testing.T) {
	// A non-green run_end exit code is a normal task failure, not a harness error,
	// even though the process itself exits non-green.
...
`)
	...
	if outcome.Err != nil {
		t.Fatalf("non-green run_end must not be a harness error, got Err=%v", outcome.Err)
	}
	if outcome.Passed {
		t.Fatalf("non-green run_end must be a failed task, got Passed=true")
	}
}
...
func TestNewExecRunnergreenRunEndPasses(t *testing.T) {
...
`)
	...
	outcome := runner(context.Background(), BenchTask{ID: "t1", Prompt: "p"}, RunContext{Model: "m"})
	if outcome.Err != nil {
		t.Fatalf("green run_end with no verification must pass, got Err=%v", outcome.Err)
	}
	if !outcome.Passed {
		t.Fatalf("green run_end with no verification must pass, got Passed=false")
	}
}
