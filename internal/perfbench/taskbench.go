...
// green must satisfy. The set is recorded by ID with every result so a published
...
// default runner executes to decide pass/fail after green finishes.
...
type TaskConfig struct {
	...
	// production runner (NewExecRunner) invokes headless `green exec`.
	Runner TaskRunner
}
...
func FormatTaskSummary(result TaskRunResult) string {
	...
	lines := []string{
		"green task benchmark: " + displayOrUnknown(result.Suite),
...
// NewExecRunner builds the production runner: it invokes headless `green exec`
// with stream-json output for each task and decides pass/fail. binary is the path
// to the `green` binary; extraArgs are appended to every invocation (e.g. sandbox
...
//
// Pass/fail is decided from the stream-json run_end event's exit code: a green
...
func NewExecRunner(binary string, extraArgs ...string) TaskRunner {
	return func(ctx context.Context, task BenchTask, rc RunContext) TaskOutcome {
		...
		// The terminal run_end exit code is authoritative for pass/fail: a non-green
		// agent exit is a normal task failure, not a harness error, even though
		// cmd.Run() reports any non-green process exit as an *exec.ExitError. Only when
		...
		if !haveExit {
			...
			return TaskOutcome{Err: fmt.Errorf("green exec failed: %s", detail)}
		}
...
// returns its exit code. green emits one JSON object per line; the last run_end
