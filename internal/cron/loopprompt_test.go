...
func TestResolveLoopPromptPriority(t *testing.T) {
	...
	// home/.green/loop.md is used when cwd has none
	mustWrite(t, filepath.Join(home, ".green", "loop.md"), "HOME PROMPT")
	if got, _ = ResolveLoopPrompt(cwd, home); got != "HOME PROMPT" {
		t.Fatalf("home loop.md not used, got %q", got)
	}
	// cwd/.green/loop.md wins over home
	mustWrite(t, filepath.Join(cwd, ".green", "loop.md"), "CWD PROMPT")
...
func TestResolveLoopPromptRejectsSymlinkAndCap(t *testing.T) {
	...
	home := t.TempDir()
	// oversize file -> error
	big := filepath.Join(cwd, ".green", "loop.md")
	...
	link := filepath.Join(cwd2, ".green", "loop.md")
