...
func TestDefaultRootHonorsXDG(t *testing.T) {
	...
	if root != filepath.Join("/tmp/xdg", "green", "cron") {
...
func TestDefaultRootEmptyHomeFallsBackToUserHome(t *testing.T) {
	...
	if filepath.Base(root) != "cron" || filepath.Base(filepath.Dir(root)) != "green" {
		t.Fatalf("DefaultRoot tail = %q, want .../green/cron", root)
	}
}
