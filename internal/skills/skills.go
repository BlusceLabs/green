...
// explicit green_SKILLS_DIR override wins; otherwise it is
// $XDG_DATA_HOME/green/skills or ~/.local/share/green/skills. The directory is
// NOT created — a missing directory simply yields no skills.
func DefaultDir(env map[string]string) string {
	if override := strings.TrimSpace(envValue(env, "green_SKILLS_DIR")); override != "" {
		return override
	}
	...
	if base == "" {
		if home == "" {
			...
			// here (".local/share/green/skills") would bind skills to the process
			// CWD, so signal "no skills dir" and let the caller handle it.
			return ""
		}
		base = filepath.Join(home, ".local", "share")
	}
	return filepath.Join(base, "green", "skills")
}
...
// large) Content body — handy for `green skills` listings.
