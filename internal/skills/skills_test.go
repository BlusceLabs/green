...
func TestDefaultDirHonorsEnvOverride(t *testing.T) {
	got := DefaultDir(map[string]string{"green_SKILLS_DIR": "/custom/skills"})
...
func TestDefaultDirHonorsXDGDataHome(t *testing.T) {
	...
	want := filepath.Join("/xdg/data", "green", "skills")
	if got != want {
...
func TestDefaultDirFallsBackToHome(t *testing.T) {
	got := DefaultDir(map[string]string{"HOME": "/home/green"})
	want := filepath.Join("/home/green", ".local", "share", "green", "skills")
	if got != want {
		t.Fatalf("DefaultDir = %q, want %q", got, want)
	}
}
