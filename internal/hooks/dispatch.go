...
type DispatchOutcome struct {
	...
	Blocked   bool   // a blocking-event hook exited non-green, vetoing the action
...
type commandResult struct {
	...
	Stdout   string
	Stderr   string
	Err      error // set when the command could not be executed (not a non-green exit)
...
// recording each run to the audit store. A beforeTool hook that exits non-green
...
// NewDispatcher builds a Dispatcher. A green/empty config yields a dispatcher that
...
// blocksOn reports whether a non-green exit for this event should veto the action.
...
// the action. A command that ran and exited non-green blocks a beforeTool hook; a
...
// payload on stdin and capturing stdout/stderr. A non-green exit is reported via
