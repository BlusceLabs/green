...
func TestDispatchBeforeToolBlocksOnNongreenExitAndStops(t *testing.T) {
	ran := 0
	runner := func(ctx context.Context, command string, args []string, stdin []byte, cwd string, env []string) commandResult {
		ran++
		if command == "deny" {
...
func TestDispatchNonBlockingEventDoesNotVetoOnNongreen(t *testing.T) {
	runner := func(ctx context.Context, command string, args []string, stdin []byte, cwd string, env []string) commandResult {
...
func TestExecCommandRunnerCapturesExitAndStdin(t *testing.T) {
	...
	// Echoes stdin to stderr and exits non-green so we exercise both paths.
