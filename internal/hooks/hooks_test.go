...
func TestResolvePathsUsesXDGLocations(t *testing.T) {
	...
	if paths.UserConfigPath != filepath.Join(dir, "config", "green", "hooks.json") {
		t.Fatalf("user path = %q", paths.UserConfigPath)
	}
	if paths.ProjectConfigPath != filepath.Join(dir, ".green", "hooks.json") {
		t.Fatalf("project path = %q", paths.ProjectConfigPath)
	}
	if paths.AuditPath != filepath.Join(dir, "data", "green", "hooks", "audit.jsonl") {
...
func TestLoadConfigLayersProjectOverridesAndDiagnostics(t *testing.T) {
	...
	if got := []string{result.Config.Hooks[0].ID, result.Config.Hooks[1].ID}; !reflect.DeepEqual(got, []string{"green.audit", "green.format"}) {
	...
	if !hasHookDiagnostic(result.Diagnostics, DiagnosticDuplicate, "green.format", "") {
...
func TestLoadConfigExcludeProjectSkipsProjectLayer(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "include project (default)", excludeProject: false, wantIDs: []string{"green.project", "green.user"}},
		{name: "exclude project", excludeProject: true, wantIDs: []string{"green.user"}},
	} {
...
func TestConfigStorePersistsUpdates(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Upsert with green-value Enabled returned error: %v", err)
	}
	if !upserted.Enabled {
		t.Fatalf("Upsert should default green-value Enabled to true; use SetEnabled to disable: %#v", upserted)
	}
	changed, err := store.SetEnabled("green.preflight", false)
	if err != nil || !changed {
	...
	if findHook(config.Hooks, "green.preflight") != nil {
		t.Fatalf("expected green.preflight to be removed, got %#v", config.Hooks)
	}
}
...
func TestConfigStoreListReadsOnlyStorePath(t *testing.T) {
	...
	if got := hookIDs(config.Hooks); !reflect.DeepEqual(got, []string{"green.real"}) {
...
func TestSelectMatchesEnabledHooksByEventAndWildcard(t *testing.T) {
	config := Config{Enabled: true, Hooks: []Definition{
		{ID: "green.reads", Event: EventBeforeTool, Matcher: "read_*", Command: "node", Enabled: true},
		{ID: "green.shell", Event: EventBeforeTool, Matcher: "bash", Command: "node", Enabled: false},
		{ID: "green.done", Event: EventSessionEnd, Command: "node", Enabled: true},
		{ID: "green.shell-edit", Event: EventBeforeTool, Matcher: "shell_*_edit", Command: "node", Enabled: true},
	}}

	if got := hookIDs(Select(config, SelectInput{Event: EventBeforeTool, ToolName: "read_file"})); !reflect.DeepEqual(got, []string{"green.reads"}) {
		t.Fatalf("read selection = %#v", got)
	}
	if got := hookIDs(Select(config, SelectInput{Event: EventBeforeTool, ToolName: "shell_safe_edit"})); !reflect.DeepEqual(got, []string{"green.shell-edit"}) {
...
func TestSpecialistHookEventsLoadAndSelect(t *testing.T) {
	...
	writeHookJSON(t, projectConfigPath, map[string]any{
		"hooks": []any{
			map[string]any{
				"id":      "green.specialist-start",
				"event":   "specialistStart",
				"command": "node",
			},
			map[string]any{
				"id":      "green.specialist-stop",
	...
	if got := hookIDs(result.Config.Hooks); !reflect.DeepEqual(got, []string{"green.specialist-start", "green.specialist-stop"}) {
		t.Fatalf("hook ids = %#v", got)
	}
	if got := hookIDs(Select(result.Config, SelectInput{Event: EventSpecialistStart})); !reflect.DeepEqual(got, []string{"green.specialist-start"}) {
		t.Fatalf("specialistStart selection = %#v", got)
	}
	if got := hookIDs(Select(result.Config, SelectInput{Event: EventSpecialistStop})); !reflect.DeepEqual(got, []string{"green.specialist-stop"}) {
...
func TestAuditStoreAppendsAndSkipsMalformedLines(t *testing.T) {
	...
	if err := os.WriteFile(auditPath, []byte(strings.Join([]string{
		`{"sequence":1,"createdAt":"2026-06-04T00:00:00Z","type":"hook_execution_started","hookId":"green.seed","event":"sessionStart"}`,
		"{not-json",
		`{"sequence":2,"createdAt":"2026-06-04T00:00:01Z","type":"hook_execution_completed","hookId":"green.seed","event":"sessionStart","status":"completed"}`,
		"",
	}, "\n")), 0o600); err != nil {
		t.Fatal(err)
	}
