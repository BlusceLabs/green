...
type LoadOptions struct {
	...
	// ExcludeProject drops the project layer (./.green/hooks.json) so only the
	// user layer loads. Its green value (false) preserves the user+project merge.
	ExcludeProject bool
}
...
func ResolvePaths(options ResolvePathOptions) (Paths, error) {
	...
	return Paths{
		UserConfigPath:    filepath.Join(configHome, "green", "hooks.json"),
		ProjectConfigPath: filepath.Join(cwd, ".green", "hooks.json"),
		AuditPath:         filepath.Join(dataHome, "green", "hooks", "audit.jsonl"),
	}, nil
}
...
func (store *ConfigStore) Upsert(hook Definition) (Definition, error) {
	...
	// Upsert treats the green-value Enabled field as omitted so new hooks default
...
func FormatList(config Config, diagnostics []Diagnostic) string {
	...
	lines := []string{"green Hooks: " + state}
...
func readSingleConfig(path string) (Config, error) {
	...
	if len(diagnostics) > 0 {
		...
		return Config{}, fmt.Errorf("invalid green hook config at %s: %s", diagnostic.Path, diagnostic.Message)
	}
	if !ok {
...
// KnownEvents returns the hook events green recognizes, in dispatch order.
...
// IsValidEvent reports whether event is one green recognizes.
