...
func TestPowerShellInstallerScriptMatchesWindowsReleaseContracts(t *testing.T) {
	...
	containsAll(t, script, []string{
		`[string]$Repository = $(if ($env:green_REPO)`,
		`Join-Path $env:LOCALAPPDATA "green\bin"`,
		`$archiveName = "green-v$releaseVersion-windows-$arch.zip"`,
		`$checksumName = "$archiveName.sha256"`,
		`Get-FileHash -Path $archivePath -Algorithm SHA256`,
		`Expand-Archive -Path $archivePath -DestinationPath $extractDir -Force`,
		`Find-greenExtractedFile -Root $extractDir -FileName $fileName`,
		...
		`Find-greenOptionalExtractedDirectory -Root $extractDir -DirectoryName "helpers"`,
...
func TestUnixInstallerInstallsFromPrefixedReleaseArchiveWithoutNetwork(t *testing.T) {
	...
	if !strings.Contains(stdout, "Installed "+filepath.Join(fixture.installDir, "green")) {
		t.Fatalf("install.sh stdout missing install path:\n%s", stdout)
	}

	installed := readFile(t, filepath.Join(fixture.installDir, "green"))
	if !strings.Contains(string(installed), "mock-green") {
		t.Fatalf("installed binary = %q, want mock-green script", string(installed))
	}
	...
	if _, err := os.Stat(filepath.Join(fixture.installDir, "green-linux-sandbox")); err != nil {
...
func TestUnixInstallerRejectsChecksumMismatchWithoutNetwork(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(fixture.installDir, "green")); !os.IsNotExist(err) {
...
func newUnixInstallFixture(t *testing.T) unixInstallFixture {
	...
	packageName := fmt.Sprintf("green-v0.1.0-%s-%s", releasePlatform, releaseArch)
	...
	writeFile(t, filepath.Join(packageDir, "green"), []byte("#!/usr/bin/env sh\necho mock-green\n"), 0o755)
	writeFile(t, filepath.Join(packageDir, "green-linux-sandbox"), []byte("#!/usr/bin/env sh\n"), 0o755)
	writeFile(t, filepath.Join(packageDir, "green-seccomp"), []byte("#!/usr/bin/env sh\n"), 0o755)
...
func runUnixInstaller(t *testing.T, fixture unixInstallFixture) (string, string, error) {
	...
	command.Env = append(os.Environ(),
		...
		"green_GITHUB_BASE_URL=https://example.test",
		"green_REPO=BlusceLabs/green",
	)
