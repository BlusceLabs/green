...
//
//	green_STT_TEST_BINARY=/path/to/sherpa-onnx-offline \
//	green_STT_TEST_MODEL=/path/to/model-dir \
//	green_STT_TEST_WAV=/path/to/16k-mono.wav \
//	go test ./internal/dictation/ -run TestLocalTranscribeRealBinary -v
func TestLocalTranscribeRealBinary(t *testing.T) {
	binary := os.Getenv("green_STT_TEST_BINARY")
	model := os.Getenv("green_STT_TEST_MODEL")
	wav := os.Getenv("green_STT_TEST_WAV")
	if binary == "" || model == "" || wav == "" {
		t.Skip("set green_STT_TEST_BINARY, green_STT_TEST_MODEL, green_STT_TEST_WAV to run the real-engine smoke test")
	}
