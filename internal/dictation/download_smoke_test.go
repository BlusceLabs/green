...
// It downloads ~130MB, so it is opt-in via green_STT_DOWNLOAD_TEST=1.
//
//	green_STT_DOWNLOAD_TEST=1 go test ./internal/dictation/ -run RealDownload -v -timeout 10m
func TestEnsureLocalEngineRealDownload(t *testing.T) {
	if os.Getenv("green_STT_DOWNLOAD_TEST") == "" {
		t.Skip("set green_STT_DOWNLOAD_TEST=1 to run the real ~130MB download smoke test")
	}
	dest := t.TempDir()
