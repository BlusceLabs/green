...
//
//	green_STT_STREAM_SERVER=/path/to/sherpa-onnx-online-websocket-server \
//	green_STT_STREAM_MODEL=/path/to/streaming-model-dir \
//	green_STT_STREAM_WAV=/path/to/16k-mono.wav \
//	go test ./internal/dictation/ -run RealServer -v -timeout 5m
func TestLocalStreamingRealServer(t *testing.T) {
	server := os.Getenv("green_STT_STREAM_SERVER")
	model := os.Getenv("green_STT_STREAM_MODEL")
	wav := os.Getenv("green_STT_STREAM_WAV")
	if server == "" || model == "" || wav == "" {
		t.Skip("set green_STT_STREAM_SERVER, green_STT_STREAM_MODEL, green_STT_STREAM_WAV to run the streaming smoke test")
	}
