...
// Distribution command handlers: `green skill {add,info,remove}`,
// `green plugin {add,remove}`, and `green tools {make,list}`. These wire the
...
func runSkillAdd(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if source == "" {
		return writeExecUsageError(stderr, "usage: green skill add <git-url|path> [--force] [--json]")
	}
	if dir == "" {
...
func runSkillInfo(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: green skill info <name> [--json]")
	}
...
func runSkillRemove(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: green skill remove <name>")
	}
...
func runPluginAdd(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if source == "" {
		return writeExecUsageError(stderr, "usage: green plugin add <git-url|path> [--force] [--json]")
	}
	if dir == "" {
...
func runPluginRemove(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if id == "" {
		return writeExecUsageError(stderr, "usage: green plugin remove <id>")
	}
...
func runTools(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "tools subcommand required. Use `green tools make <name>` or `green tools list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runToolsMake(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: green tools make <name> [--runtime shell|node|python] [--description text]")
	}
	if dir == "" {
	...
	lines := []string{
		...
		"  2. Run `green tools list` to confirm it loads.",
...
func formatToolboxList(items []toolboxItem, diagnostics []plugins.Diagnostic, dir string) string {
	lines := []string{}
	if len(items) == 0 {
		lines = append(lines, fmt.Sprintf("No green plugin-tools found in %s.", dir))
	} else {
		lines = append(lines, "green Tools:")
...
func writeSkillAddHelp(w io.Writer) error {
  ...
  green skill add <git-url|path> [flags]
...
func writePluginAddHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green plugin add <git-url|path> [flags]
