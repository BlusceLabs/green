...
func (provider *capturingImageProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	provider.mu.Lock()
	for index := len(request.Messages) - 1; index >= 0; index-- {
		if request.Messages[index].Role == greenruntime.MessageRoleUser {
			provider.images = append([]greenruntime.ImageBlock(nil), request.Messages[index].Images...)
			break
		}
	}
	provider.mu.Unlock()
	...
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: "ok"}
