...
func runChanges(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	switch command {
	case "inspect", "status":
		summary, err := deps.inspectChanges(context.Background(), greengit.InspectOptions{
			Cwd:          workspaceRoot,
		...
		if options.json {
			if err := writePrettyJSON(stdout, greengit.SnapshotFromSummary(safeSummary)); err != nil {
				return exitCrash
			}
			return exitSuccess
		}
	...
	case "commit":
		...
		result, err := deps.commitChanges(context.Background(), greengit.CommitOptions{
			Cwd:          workspaceRoot,
			Message:      message,
...
func parseChangesArgs(args []string, command string) (changesCommandOptions, bool, error) {
	...
	if command != "commit" && options.message != "" {
		return options, false, execUsageError{"--message is only valid with `green changes commit`"}
	}
	if command != "commit" && (options.hasMessage || options.dryRun || options.auto) {
		return options, false, execUsageError{"--message, --dry-run, and --auto are only valid with `green changes commit`"}
	}
	...
	if command != "inspect" && options.baseRef != "" {
		return options, false, execUsageError{"--base is only valid with `green changes inspect`"}
	}
	...
	if command != "pr" && (options.title != "" || options.body != "" || options.fill || options.draft) {
		return options, false, execUsageError{"--title, --body, --fill, and --draft are only valid with `green changes pr`"}
	}
...
func redactChangeSummary(summary greengit.ChangeSummary) greengit.ChangeSummary {
...
func redactCommitResult(result greengit.CommitResult) greengit.CommitResult {
...
func formatChangeSummary(summary greengit.ChangeSummary) string {
	lines := []string{
		"green changes",
		"root: " + summary.Root,
...
func formatCommitResult(result greengit.CommitResult) string {
	lines := []string{
		"green changes commit",
		"root: " + result.Root,
...
func writeWorktreesHelp(w io.Writer) error {
  ...
  green worktrees prepare [flags] [name]

Prepares an isolated git worktree for a green task.

Flags:
      --name <name>       Worktree name; defaults to a timestamped task name
      --dir <path>        Base directory for green worktrees
...
func writeChangesHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green changes inspect [flags]
  green changes commit [flags]
  green changes push [flags]
  green changes pr [flags]
...
Flags:
  ...
  -m, --message <text>    Commit message for `+"`green changes commit`"+`
...
func runChangesPush(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	result, err := deps.pushChanges(context.Background(), greengit.PushOptions{
		Cwd:                    workspaceRoot,
...
func runChangesPR(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	pushResult, err := deps.pushChanges(context.Background(), greengit.PushOptions{
		Cwd:                    workspaceRoot,
		Remote:                 options.remote,
		Force:                  options.force,
		AllowPushDefaultBranch: options.yes,
	...
	prResult, err := deps.createPR(context.Background(), greengit.PROptions{
		Cwd:   workspaceRoot,
		Fill:  options.fill,
		Draft: options.draft,
		Title: options.title,
...
func generateAutoCommitMessage(ctx context.Context, provider greenruntime.Provider, model string, summary greengit.ChangeSummary) (string, error) {
	...
	request := greenruntime.CompletionRequest{
		Messages: []greenruntime.Message{
			{Role: greenruntime.MessageRoleUser, Content: promptBuilder.String()},
		},
	}
	...
	collected := greenruntime.CollectStream(ctx, stream)
	if collected.Error != "" {
