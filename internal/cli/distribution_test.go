...
func TestRunSkillAddRejectsInvalid(t *testing.T) {
	...
	if exit == 0 {
		t.Fatalf("expected a non-green exit for an invalid skill source")
	}
}
...
func TestRunPluginAddListRemove(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "green.demo") {
	...
	stdout.Reset()
	stderr.Reset()
	if exit := runWithDeps([]string{"plugin", "remove", "green.demo"}, &stdout, &stderr, deps); exit != 0 {
		t.Fatalf("plugin remove exit = %d, stderr = %s", exit, stderr.String())
	}
	if _, err := os.Stat(filepath.Join(pluginsDir, "green.demo")); err == nil {
...
func TestRunPluginAddRejectsInvalidManifest(t *testing.T) {
	...
	if exit == 0 {
		t.Fatalf("expected a non-green exit for an invalid manifest")
	}
}
...
func TestRunToolsMakeRejectsBadName(t *testing.T) {
	...
	if exit == 0 {
		t.Fatalf("expected a non-green exit for an invalid tool name")
	}
}
...
func TestRunToolsMakeRequiresName(t *testing.T) {
	...
	if exit == 0 {
		t.Fatalf("expected a non-green exit when no name is given")
	}
}
