...
func seedStoredProviderKey(t *testing.T, providerName, key string) (userConfigPath string) {
	t.Helper()
	t.Setenv("green_CRED_STORAGE", "encrypted-file") // never touch the real OS keychain in tests
	dir := t.TempDir()
...
// fillAppDeps also wraps probeProviderHealth, the code path green providers
// check --connectivity, green doctor --connectivity, and the TUI doctor panel
...
// green_PROVIDER so children spawned at any point in the run are pinned to the
// parent's provider from launch — not only after an in-session switch. Without
// this, a provider switch persisted by another green process mid-session moves
