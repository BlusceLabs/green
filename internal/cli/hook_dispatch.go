...
// Do NOT hoist it to the callers behind an excludeProject bool whose green value
...
// projectHooksFileExists reports whether a ./.green/hooks.json is present under
...
func projectHooksFileExists(workspaceRoot string) bool {
	...
	info, err := os.Stat(filepath.Join(workspaceRoot, ".green", "hooks.json"))
...
func emitTrustNotice(stderr io.Writer, skips ...trustSkip) {
	...
	if storeErrored {
		_, _ = fmt.Fprintln(stderr, "green: the workspace-trust store could not be read; ignoring project hooks/plugins/MCP servers (fail-closed). Run 'green trust' to enable.")
		return
	}
	_, _ = fmt.Fprintln(stderr, "green: ignoring project hooks/plugins/MCP servers in an untrusted workspace. Run 'green trust' to enable.")
}
