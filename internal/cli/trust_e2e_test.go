...
func (p toolThenTextProvider) StreamCompletion(_ context.Context, req greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	toolAlreadyCalled := false
	for _, m := range req.Messages {
		if m.Role == greenruntime.MessageRoleTool {
			toolAlreadyCalled = true
			break
		}
	}
	ch := make(chan greenruntime.StreamEvent, 8)
	go func() {
		defer close(ch)
		...
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: p.toolName}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "c1", ArgumentsFragment: `{"pattern":"*"}`}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "c1"}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventDone}
	}()
	return ch, nil
}

// writeMarkerHook writes a ./.green/hooks.json under dir whose enabled beforeTool
...
func writeMarkerHook(t *testing.T, dir, markerPath string) {
	...
	if err := os.MkdirAll(filepath.Join(dir, ".green"), 0o755); err != nil {
		t.Fatal(err)
	}
	body := `{"enabled":true,"hooks":[{"id":"m","event":"beforeTool","command":"/bin/sh","args":["-c","touch '` + markerPath + `'"],"enabled":true}]}`
	if err := os.WriteFile(filepath.Join(dir, ".green", "hooks.json"), []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
}
...
// trust verdict and the real ./.green/config.json that projectMCPConfigExists reads.
...
// The fake provider never submits a spec, so the run exits non-green; that is
...
func TestExecSpecSurfacesMCPTrustNotice(t *testing.T) {
	...
	if !strings.Contains(errBuf.String(), "MCP servers") || !strings.Contains(errBuf.String(), "green trust") {
...
// checkout carries the committed .green/hooks.json; the gate must load it only when
...
func TestExecSpecWorktreeInheritsTrustEndToEnd(t *testing.T) {
	...
	// The spec-draft flow itself exits non-green here (the fake provider does not
