...
func backendLifecycleSnapshot(deps appDeps) (greencommands.BackendLifecycleSnapshot, error) {
	cwd, err := deps.getwd()
	if err != nil {
		return greencommands.BackendLifecycleSnapshot{}, fmt.Errorf("failed to resolve workspace: %w", err)
	}
	...
	if err != nil {
		return greencommands.BackendLifecycleSnapshot{}, err
	}
	...
	return greencommands.NewBackendLifecycleSnapshot(servers, hookResult.Config.Hooks, pluginResult.Plugins), nil
}

func backendDoctorReport(deps appDeps) (greencommands.BackendDoctorReport, error) {
	cwd, err := deps.getwd()
	if err != nil {
		return greencommands.BackendDoctorReport{}, fmt.Errorf("failed to resolve workspace: %w", err)
	}
	...
	cfg, err := deps.resolveMCPConfig(cwd, false)
	if err != nil {
		return greencommands.BackendDoctorReport{}, err
	}
	hookResult, err := deps.loadHooks(hooks.LoadOptions{Cwd: cwd})
	if err != nil {
		return greencommands.BackendDoctorReport{}, err
	...
	return greencommands.NewBackendDoctorReport(greencommands.BackendDoctorInput{
		MCP:     cfg,
		Hooks:   hookResult,
...
func formatBackendLifecycleSnapshot(snapshot greencommands.BackendLifecycleSnapshot) string {
	lines := []string{"green Backends:"}
...
func formatBackendDoctorReport(report greencommands.BackendDoctorReport) string {
	lines := []string{
		"green backend doctor",
...
func writeBackendsHelp(w io.Writer) error {
  ...
  green backends [flags]
  green backends doctor [flags]
