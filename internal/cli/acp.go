...
import (
	...
	"github.com/BlusceLabs/green/internal/acp"
...
const acpUsage = `green acp — serve the Agent Client Protocol (ACP) over stdio

Editors that speak ACP (Zed, JetBrains, Neovim, ...) spawn this command and drive
green as a backend over JSON-RPC 2.0 on stdin/stdout. green keeps your provider,
...
setting at "green acp".`

// runACP serves ACP over stdio so an editor can drive green's agent core. It
// speaks JSON-RPC 2.0 (newline-delimited JSON) on stdin/stdout; stderr stays free
// for human-readable diagnostics. The session lifecycle maps onto green's own
// session store, and provider/model/keys remain owned by green.
func runACP(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	acp.NewAgent(conn, acp.Deps{
		...
		AgentInfo:            acp.Implementation{Name: "green", Version: version},
	})
...
// editor must not be able to point green's file/shell tools at the whole disk.
