...
func TestRunSpecialistListShowAndPath(t *testing.T) {
	...
	userDir := filepath.Join(configRoot, "green", "specialists")
...
Find the likely failure area.`)
	...
	for _, want := range []string{"green Specialists", "worker [builtin]", "triage [user]", "code-review"} {
	...
	if !strings.Contains(stdout.String(), "green Specialist: triage") || !strings.Contains(stdout.String(), "Find the likely failure area.") {
	...
	if !strings.Contains(stdout.String(), userDir) || !strings.Contains(stdout.String(), filepath.Join(cwd, ".green", "specialists")) {
...
func TestRunSpecialistShowAndPathJSON(t *testing.T) {
	...
	if pathPayload.UserDir == "" || pathPayload.ProjectDir != filepath.Join(cwd, ".green", "specialists") {
...
func TestRunSpecialistCreateProjectAndRejectDuplicate(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(cwd, ".green", "specialists", "project-helper.md")); err != nil {
