...
// the plugin skill search roots. The green value (no plugins) is inert — the
...
// projectPluginsDirExists reports whether a ./.green/plugins directory is present
...
func projectPluginsDirExists(workspaceRoot string) bool {
	...
	info, err := os.Stat(filepath.Join(workspaceRoot, ".green", "plugins"))
...
func writePluginActivationWarning(stderr io.Writer, message string) {
	...
	_, _ = fmt.Fprintf(stderr, "[green] WARNING: plugin activation: %s\n", message)
}
