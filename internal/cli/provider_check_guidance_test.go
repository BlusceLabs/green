...
func TestRunProvidersCheckMissingKeyPrintsNextAction(t *testing.T) {
	...
	if !strings.Contains(output, "next: set GROQ_API_KEY and rerun green providers check groq") {
...
func TestRunProvidersCheckConnectivitySkippedPrintsNextAction(t *testing.T) {
	...
	if !strings.Contains(output, "next: run green providers check work --connectivity") {
...
func TestRunProvidersCheckConnectivityFailurePrintsGuidance(t *testing.T) {
	...
	if !strings.Contains(output, "next: verify the API key, base URL, and model, then rerun green providers check local --connectivity") {
...
func TestRunProvidersCheckConnectivityOKPrintsExecNextAction(t *testing.T) {
	...
	output := stdout.String()
	if !strings.Contains(output, `next: run green exec "hello" --model gpt-4.1`) {
