...
import (
	...
	"github.com/BlusceLabs/green/internal/workspacetrust"
...
func TestRunPrintsVersion(t *testing.T) {
	...
	if got := stdout.String(); got != "green dev\n" {
...
func TestRunNoArgsLaunchesSetupTUIWithNilProviderWhenNoProviderConfigured(t *testing.T) {
	...
	projectConfigPath := filepath.Join(cwd, ".green", "config.json")
	...
	userConfigPath := filepath.Join(t.TempDir(), "green", "config.json")
...
func TestRunNoArgsFailsWhenResolveErrorIsNotProviderRelated(t *testing.T) {
	...
	if exitCode == 0 {
		t.Fatal("exit code = 0, want non-green for fatal config error")
	}
...
// and the real ./.green/config.json.
func TestRunInteractiveSurfacesMCPTrustNotice(t *testing.T) {
	setTrustConfigRoot(t)
	repo := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repo, ".green"), 0o700); err != nil {
		t.Fatal(err)
	}
	body := `{"mcp":{"servers":{"proj":{"type":"stdio","command":"proj-cmd"}}}}`
	if err := os.WriteFile(filepath.Join(repo, ".green", "config.json"), []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	untrusted := run()
	if !strings.Contains(untrusted, "MCP servers") || !strings.Contains(untrusted, "green trust") {
...
func TestRunSkipPermissionsUnsafeLaunchesTUIInUnsafeMode(t *testing.T) {
	...
	// "green --skip-permissions-unsafe" must launch the interactive TUI in unsafe
...
func TestRunSetupProviderWritesActiveConfig(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "green setup complete") || !strings.Contains(stdout.String(), "next: green") {
...
func TestRunUpdateCheckTextAndJSON(t *testing.T) {
	result := update.Result{
		CurrentVersion: "dev",
		LatestVersion:  "0.2.0",
		ReleaseURL:     "https://github.com/BlusceLabs/green/releases/tag/v0.2.0",
		TagName:        "v0.2.0",
		ReleaseAsset: update.AssetCheck{
			Platform:      "linux",
			Arch:          "x64",
			ArchiveName:   "green-v0.2.0-linux-x64.tar.gz",
			ArchiveURL:    "https://example.test/green-v0.2.0-linux-x64.tar.gz",
			ChecksumName:  "green-v0.2.0-linux-x64.tar.gz.sha256",
			ChecksumURL:   "https://example.test/green-v0.2.0-linux-x64.tar.gz.sha256",
			ArchiveFound:  true,
			ChecksumFound: true,
			Verified:      true,
		},
	...
	if !strings.Contains(stdout.String(), "Release asset: green-v0.2.0-linux-x64.tar.gz") || !strings.Contains(stdout.String(), "Checksum asset: green-v0.2.0-linux-x64.tar.gz.sha256") {
	...
	exitCode := runWithDeps([]string{"update", "--check"}, &stdout, &stderr, appDeps{
		checkUpdate: func(context.Context, update.Options) (update.Result, error) {
			return update.Result{
				CurrentVersion:  "dev",
				LatestVersion:   "dev",
				ReleaseURL:      "https://github.com/BlusceLabs/green/releases/tag/dev",
				TagName:         "dev",
...
func TestRunUpdateApplyTextAndJSON(t *testing.T) {
	applyResult := update.ApplyResult{
		...
		BinaryPath:    "/usr/local/bin/green",
...
func TestRunUpgradeDefaultsToApply(t *testing.T) {
	...
	deps := appDeps{
		...
		checkUpdate: func(context.Context, update.Options) (update.Result, error) {
			t.Fatal("checkUpdate should not run for `green upgrade`")
			return update.Result{}, nil
		},
	}
	...
	if !applyCalled {
		t.Fatal("expected `green upgrade` to call applyUpdate")
	}
}
...
func (cliFakeProvider) StreamCompletion(context.Context, greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
...
func TestRunThemeFlagPopulatesTUIOptions(t *testing.T) {
	...
	// over green_THEME). Previously Options.Theme was read but never set by the CLI.
...
// A custom endpoint saved without a model previously bricked bare `green` and
// `green setup` — the exact commands that could have fixed it (the resolve
...
func TestRunNoArgsEntersSetupWhenActiveProviderMissesModel(t *testing.T) {
	...
	exitCode := runWithDeps([]string{}, &stdout, &stderr, appDeps{
		...
		userConfigPath: func() (string, error) { return filepath.Join(t.TempDir(), "green", "config.json"), nil },
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return noopMCPRuntime{}, nil
		},
		runTUI: func(_ context.Context, options tui.Options) int {
...
// `green login`/`green logout` don't exist (it's `green auth login`); first-run
...
func TestRunUnknownLoginSuggestsAuthLogin(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), `"green auth login"`) {
		t.Fatalf("stderr = %q, want a green auth login suggestion", stderr.String())
	}
