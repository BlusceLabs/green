...
func TestRunExecUsesInitSessionIDAndSessionTitle(t *testing.T) {
	...
	store := sessions.NewStore(sessions.StoreOptions{RootDir: filepath.Join(dataHome, "green", "sessions")})
...
func (provider recordingExecProvider) StreamCompletion(context.Context, greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	*provider.called = true
	return nil, errors.New("provider should not run")
}

func TestRunPromptFlagRoutesToExecRunner(t *testing.T) {
	execExitCode, execStdout, execStderr := runExecWithEcho(t, []string{"exec", "hello green"})
...
func TestRunExecUsesProjectConfigAndOpenAICompatibleProvider(t *testing.T) {
	clearProviderEnv(t)
	root := t.TempDir()
	configDir := filepath.Join(root, ".green")
...
func (canceledExecProvider) StreamCompletion(context.Context, greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	return nil, context.Canceled
}

type echoExecProvider struct{}

func (echoExecProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	prompt := ""
	for index := len(request.Messages) - 1; index >= 0; index-- {
		if request.Messages[index].Role == greenruntime.MessageRoleUser {
	...
	case <-ctx.Done():
		close(ch)
		return ch, ctx.Err()
	case ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: prompt}:
	}
...
func (provider *escalatingExecProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	turn := provider.turns
	provider.turns++
	ch := make(chan greenruntime.StreamEvent, 4)
	if provider.escalateOnce && turn == 0 {
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call_escalate", ToolName: "escalate_model"}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call_escalate", ArgumentsFragment: "{}"}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call_escalate"}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventDone}
		close(ch)
		return ch, nil
	}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: "done"}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventDone}
	close(ch)
	return ch, nil
}
...
func TestRunExecWiresModelSwitcherUnderFlag(t *testing.T) {
	...
	// would then handle green turns and the first would handle both).
...
func (provider *escalateThenAnswerProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	turn := provider.turns
	provider.turns++
	ch := make(chan greenruntime.StreamEvent, 6)
	if turn == 0 {
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call_escalate", ToolName: "escalate_model"}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call_escalate", ArgumentsFragment: "{}"}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call_escalate"}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventUsage, Usage: greenruntime.Usage{InputTokens: 3, OutputTokens: 4}}
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventDone}
		close(ch)
		return ch, nil
	}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: "done"}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventUsage, Usage: greenruntime.Usage{InputTokens: 5, OutputTokens: 7}}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventDone}
	close(ch)
	return ch, nil
}
...
func (provider *usageEmittingEscalatingProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	ch := make(chan greenruntime.StreamEvent, 6)
...
func (usageEmittingEchoProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
...
func (provider *usageThenAnswerProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	turn := provider.turns
	provider.turns++
	ch := make(chan greenruntime.StreamEvent, 6)
	if turn == 0 {
