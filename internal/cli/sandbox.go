...
func runSandbox(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "sandbox subcommand required. Use `green sandbox policy` or `green sandbox grants list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runSandboxPolicy(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	policy := applyConfiguredSandboxPolicy(greenSandbox.DefaultPolicy(), resolved.Sandbox)
	backend := deps.selectSandboxBackend(greenSandbox.BackendOptions{})
	...
	if options.effective {
		...
		// exists) must not crash `green sandbox policy --effective` — fall
		// back to the workspace root and surface the error visibly instead.
		writeRoots := []string{workspaceRoot}
		var writeRootsErr error
		if scope, scopeErr := greenSandbox.NewScope(workspaceRoot, resolved.Sandbox.AdditionalWriteRoots); scopeErr != nil {
			...
			if fallback, fallbackErr := greenSandbox.NewScope(workspaceRoot, nil); fallbackErr == nil {
	...
	if options.json {
		payload := struct {
			Policy     greenSandbox.Policy      `json:"policy"`
			Backend    greenSandbox.Backend     `json:"backend"`
			Plan       greenSandbox.BackendPlan `json:"plan"`
...
func runSandboxSetup(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if backend.Name != greenSandbox.BackendWindowsRestrictedToken || !backend.Available || backend.Executable == "" {
	...
	scope, err := greenSandbox.NewScope(workspaceRoot, resolved.Sandbox.AdditionalWriteRoots)
	if err != nil {
		return writeExecUsageError(stderr, err.Error())
	}
	profile := greenSandbox.PermissionProfileFromPolicy(workspaceRoot, policy, scope)
	// Resolve the setup helper the same way the runner is resolved: a standalone
	// .exe when shipped (release), else self-dispatch via the running green binary
	// (dev / plain build). This mirrors the backend's command-runner resolution
	// so `green sandbox setup` works in every layout, not just release.
	setupHelper := greenSandbox.ResolveWindowsSandboxSetupHelper(nil)
	if !setupHelper.Available() {
		return writeAppError(stderr, "Windows sandbox setup helper is not available", exitProvider)
	}
	setupArgs, err := greenSandbox.BuildWindowsSandboxSetupArgs(greenSandbox.WindowsSandboxSetupArgsOptions{
...
type sandboxSetupResult struct {
	...
	Backend  greenSandbox.BackendName `json:"backend"`
...
func resolveSandboxGuards(policy greenSandbox.Policy) sandboxGuards {
	return sandboxGuards{
		...
		Network:            policy.Network == greenSandbox.NetworkDeny,
		Workspace:          policy.EnforceWorkspace,
	}
}

func runSandboxPolicyEffective(options sandboxCommandOptions, workspaceRoot string, policy greenSandbox.Policy, backend greenSandbox.Backend, plan greenSandbox.BackendPlan, grantsPath string, writeRoots []string, writeRootsErr error, stdout io.Writer) int {
...
func formatEffectiveSandboxPolicy(workspaceRoot string, policy greenSandbox.Policy, backend greenSandbox.Backend, plan greenSandbox.BackendPlan, guards sandboxGuards, grantsPath string, writeRoots []string, writeRootsErr error) string {
	lines := []string{
		"green effective sandbox policy",
		"root: " + workspaceRoot,
...
func runSandboxGrants(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "sandbox grants subcommand required. Use `green sandbox grants list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
	...
	case "list":
		...
		if options.json {
			if err := writePrettyJSON(stdout, struct {
				Grants          []greenSandbox.Grant              `json:"grants"`
				CommandPrefixes []greenSandbox.CommandPrefixGrant `json:"commandPrefixes,omitempty"`
			}{Grants: grants, CommandPrefixes: prefixes}); err != nil {
				return exitCrash
			}
			return exitSuccess
		}
		if _, err := fmt.Fprintln(stdout, greenSandbox.FormatGrantListWithCommandPrefixes(grants, prefixes)); err != nil {
			return exitCrash
		}
		return exitSuccess
	case "allow", "deny":
...
func runSandboxGrantSet(command string, args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: green sandbox grants "+command+" <tool> [--path file] [--reason text] [--json]")
	}
	decision := greenSandbox.GrantAllow
	if command == "deny" {
		decision = greenSandbox.GrantDeny
	}
	store, err := deps.newSandboxStore()
	if err != nil {
		return writeAppError(stderr, err.Error(), exitCrash)
	}
	input := greenSandbox.GrantInput{
	...
	if options.path != "" {
		...
		input.Scope = abs
		input.ScopeKind = greenSandbox.ScopeFile
	}
	...
	if options.json {
		if err := writePrettyJSON(stdout, struct {
			Grant greenSandbox.Grant `json:"grant"`
		}{Grant: grant}); err != nil {
			return exitCrash
		}
		return exitSuccess
...
func runSandboxGrantRevoke(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: green sandbox grants revoke <tool> [--path file] [--json]")
	}
	store, err := deps.newSandboxStore()
	if err != nil {
		return writeAppError(stderr, err.Error(), exitCrash)
...
func runSandboxGrantClear(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if !options.confirm {
		return writeExecUsageError(stderr, "green sandbox grants clear requires --confirm")
	}
	store, err := deps.newSandboxStore()
	if err != nil {
		return writeAppError(stderr, err.Error(), exitCrash)
...
func formatSandboxPolicy(workspaceRoot string, policy greenSandbox.Policy, backend greenSandbox.Backend, plan greenSandbox.BackendPlan, grantsPath string) string {
	lines := []string{
...
func writeSandboxGrantSetHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green sandbox grants allow <tool> [flags]
  green sandbox grants deny <tool> [flags]
...
func writeSandboxGrantRevokeHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green sandbox grants revoke <tool> [flags]
...
func writeSandboxGrantClearHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green sandbox grants clear --confirm [flags]
