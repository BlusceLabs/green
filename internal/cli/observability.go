...
import (
	...
	zsearch "github.com/BlusceLabs/green/internal/search"
...
func runSearch(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if strings.TrimSpace(options.query) == "" {
		return writeExecUsageError(stderr, "search query required. Use `green search <query>`.")
	}
...
func parseSearchArgs(args []string) (searchOptions, bool, error) {
	...
	for index := 0; index < len(args); index++ {
		...
		case arg == "--limit":
			...
			limit, err := parsePositiveOrgreenInt(value, "--limit")
			if err != nil {
				return options, false, err
			}
			options.limit = limit
		...
		case strings.HasPrefix(arg, "--limit="):
			limit, err := parsePositiveOrgreenInt(strings.TrimSpace(strings.TrimPrefix(arg, "--limit=")), "--limit")
			if err != nil {
				return options, false, err
			}
			options.limit = limit
		case arg == "--context-chars" || arg == "--context":
			...
			contextChars, err := parsePositiveOrgreenInt(value, arg)
			if err != nil {
				return options, false, err
			}
		...
		case strings.HasPrefix(arg, "--context-chars="):
			contextChars, err := parsePositiveOrgreenInt(strings.TrimSpace(strings.TrimPrefix(arg, "--context-chars=")), "--context-chars")
			if err != nil {
				return options, false, err
			}
			options.contextChars = contextChars
		case strings.HasPrefix(arg, "--context="):
			contextChars, err := parsePositiveOrgreenInt(strings.TrimSpace(strings.TrimPrefix(arg, "--context=")), "--context")
			if err != nil {
				return options, false, err
			}
			options.contextChars = contextChars
...
func parsePositiveOrgreenInt(value string, label string) (int, error) {
...
func writeSearchHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green search [flags] <query>

Searches persisted local green session events.
