...
import (
	...
	"github.com/BlusceLabs/green/internal/observability"
	"github.com/BlusceLabs/green/internal/plugins"
	...
	"github.com/BlusceLabs/green/internal/provideronboarding"
	...
	"github.com/BlusceLabs/green/internal/selfverify"
	"github.com/BlusceLabs/green/internal/sessions"
	"github.com/BlusceLabs/green/internal/skills"
	"github.com/BlusceLabs/green/internal/specialist"
	"github.com/BlusceLabs/green/internal/swarm"
	"github.com/BlusceLabs/green/internal/tools"
	"github.com/BlusceLabs/green/internal/tui"
...
type appDeps struct {
	...
	inspectChanges         func(context.Context, greengit.InspectOptions) (greengit.ChangeSummary, error)
	commitChanges          func(context.Context, greengit.CommitOptions) (greengit.CommitResult, error)
	pushChanges            func(context.Context, greengit.PushOptions) (greengit.PushResult, error)
	createPR               func(context.Context, greengit.PROptions) (greengit.PRResult, error)
...
func defaultAppDeps() appDeps {
	return appDeps{
		...
		inspectChanges:   greengit.Inspect,
		commitChanges:    greengit.Commit,
		pushChanges:      greengit.Push,
		createPR:         greengit.CreatePR,
		runTUI:           tui.Run,
...
// ($XDG_CONFIG_HOME/green/plugins) used as the install target for `plugin add`
...
func runWithDeps(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) (exitCode int) {
	...
	// is shipped (dev / plain `go build`), the sandbox launches the running green
	...
	// green_THEME). Re-split --add-dir afterward so it may appear on either side of --theme.
	...
	case "--skip-permissions-unsafe":
		...
		// dropping it, so `green --skip-permissions-unsafe "fix bug"` doesn't appear to
		// hang in the TUI with the prompt discarded. (AUDIT-L3)
		for _, arg := range rest {
			if strings.TrimSpace(arg) != "" {
				return writeAppError(stderr, "--skip-permissions-unsafe launches the interactive TUI and takes no prompt or subcommand; for a one-shot unsafe run use `green exec --skip-permissions-unsafe -p \"...\"`", 1)
			}
		}
	...
	case "-v", "--version", "version":
		for _, a := range args[1:] {
			if a == "-h" || a == "--help" {
				if _, err := fmt.Fprintln(stdout, "Usage: green version\n\nPrint the green CLI version. Takes no flags."); err != nil {
					return 1
				}
				return 0
			}
		}
		if _, err := fmt.Fprintf(stdout, "green %s\n", version); err != nil {
			return 1
		}
		return 0
	case "-p", "--prompt":
		...
		// dash (e.g. `green -p "-foo"`) is taken verbatim instead of being mistaken for
	...
	default:
		...
		// First-run users reach for `green login`/`green logout` (reported in the
		// wild); point them at the real command instead of a bare usage pointer.
		switch strings.ToLower(args[0]) {
		case "login", "logout":
			if _, err := fmt.Fprintf(stderr, "did you mean %q?\n", "green auth "+strings.ToLower(args[0])); err != nil {
				return 1
			}
		}
		if _, err := fmt.Fprintln(stderr, "Run green --help for usage."); err != nil {
			return 1
		}
		return 2
	}
...
func fillAppDeps(deps appDeps) appDeps {
	...
	deps.newProvider = func(profile config.ProviderProfile) (greenruntime.Provider, error) {
...
func runInteractiveTUIWithSetup(stderr io.Writer, deps appDeps, permissionMode agent.PermissionMode, addDirs []string, theme string, forceSetup bool) int {
	...
	if err != nil {
		...
		// no catalog default) — previously the second shape bricked bare `green`
		// and `green setup`, the exact commands that could have fixed it. Any
		// other error (malformed JSON, directory conflict, etc.) is still fatal,
		// and headless commands (green config / green exec) still fail with the
		// actionable message.
	...
	if needsSetup && !forceSetup {
		...
		// any time with `green provider use <name>`. Onboarding only runs when no
	...
	// untrusted workspace must not spawn its ./.green/config.json stdio MCP servers.
	...
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "[green] warning: failed to initialize MCP OAuth tokens: %s\n", err)
		mcpTokenStore = nil
		err = nil
	}
	...
	// root is overridden — e.g. in tests or a custom green config dir — so the two
	// don't diverge. userConfigPath points at .../green/config.json, so its dir is the
	// green config dir.
	sttDownloadRoot := ""
	if userConfigPath != "" {
...
func buildProvider(resolved config.ResolvedConfig, deps appDeps) (greenruntime.Provider, error) {
	...
	// provider switch persisted by ANOTHER green process mid-session would silently
...
func localArtifactsDirFromConfig(workspaceRoot string, cfg config.LocalControlConfig) string {
	...
	if dir == "" {
		dir = filepath.Join(".green", "artifacts")
	}
...
func registerSpecialistTools(registry *tools.Registry, workspaceRoot string, maxTeamSize int) (*agentToolRuntime, error) {
	...
	sw, err := swarm.New(swarm.Options{
		BaseDir:     filepath.Join(workspaceRoot, ".green", "swarm"),
...
func closeMCPRuntime(stderr io.Writer, runtime mcpToolRuntime) {
	...
	if err := runtime.Close(); err != nil {
		_, _ = fmt.Fprintf(stderr, "[green] mcp_close_error: %s\n", err)
	}
}
...
func closeSpecialistRuntime(stderr io.Writer, runtime *agentToolRuntime) {
	...
	if runtime.specialist != nil {
		if err := runtime.specialist.Close(); err != nil {
			_, _ = fmt.Fprintf(stderr, "[green] specialist_cleanup_error: %s\n", err)
		}
	}
}

func writeAppError(stderr io.Writer, message string, exitCode int) int {
	if _, err := fmt.Fprintf(stderr, "[green] %s\n", message); err != nil {
		return 1
	}
	return exitCode
}
...
func writeHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `green terminal coding agent
...
Commands:
  ...
  models     List green model registry entries
  ...
  search     Search persisted local green session events
  find       Alias for search
  sessions   Inspect local green session lineage
  spec       Review and approve saved spec-mode drafts
  specialist Manage local green specialist profiles
  plugins    Inspect, install, and remove local green plugins
  backends   Inspect MCP, hook, and plugin backend lifecycle state
  skills     Inspect, install, and remove local green skills
  tools      Scaffold and list local green plugin-tools
  hooks      Inspect green hook configuration
  ...
  update     Check for green CLI updates
  ...
  serve      Run green protocol servers
...
// argument list (green --add-dir <path> [--add-dir <path>] [subcommand …]).
...
func writePromptRequired(stderr io.Writer) int {
	if _, err := fmt.Fprintln(stderr, "[green] Prompt required. Use `green exec \"prompt\"` or `green exec --file prompt.txt`."); err != nil {
		return 1
	}
	return 2
}
