...
func TestMCPGateUntrustedExcludesProjectServer(t *testing.T) {
	...
	// This repo has no ./.green/config.json, so there is no project MCP config to
...
// TestMCPToolsListSurfacesTrustNotice proves `green mcp tools list` no longer drops the
...
func TestMCPToolsListSurfacesTrustNotice(t *testing.T) {
	...
	if errUntrusted := run(); !strings.Contains(errUntrusted, "MCP servers") || !strings.Contains(errUntrusted, "green trust") {
...
// only a ./.green/config.json that parses AND declares at least one server is true.
func TestProjectMCPConfigExists(t *testing.T) {
	writeCfg := func(t *testing.T, dir, body string) {
		t.Helper()
		if err := os.MkdirAll(filepath.Join(dir, ".green"), 0o700); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(dir, ".green", "config.json"), []byte(body), 0o600); err != nil {
			t.Fatal(err)
		}
	}
...
// untrusted workspace actually declares project MCP servers in ./.green/config.json,
...
// TestMCPCheckSurfacesTrustNotice proves the `green mcp check` notice fix: in an untrusted
// workspace whose only definition of a server lives in ./.green/config.json, the gate
...
func TestMCPCheckSurfacesTrustNotice(t *testing.T) {
	...
	if got := errBuf.String(); !strings.Contains(got, "not configured") ||
		!strings.Contains(got, "MCP servers") || !strings.Contains(got, "green trust") {
	...
	if got := errBuf2.String(); strings.Contains(got, "ignoring project") || strings.Contains(got, "green trust") {
