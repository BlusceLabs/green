...
// sandboxCheckReport is the combined snapshot `green sandbox check` emits: the
...
// tool. It is the production consumer of the greencommands sandbox-snapshot
...
type sandboxCheckReport struct {
	...
	Plan     greencommands.SandboxPlanSnapshot       `json:"plan"`
	Decision greencommands.SandboxDecisionSnapshot   `json:"decision"`
	Grant    greencommands.SandboxGrantMatchSnapshot `json:"grant"`
}
...
func runSandboxCheck(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if strings.TrimSpace(options.tool) == "" {
		return writeExecUsageError(stderr, "tool name is required: green sandbox check <tool> [flags]")
	}
	...
	engine := greenSandbox.NewEngine(greenSandbox.EngineOptions{
	...
	defer stop()
	decision := engine.Evaluate(ctx, greenSandbox.Request{
		WorkspaceRoot:  workspaceRoot,
		ToolName:       options.tool,
		SideEffect:     sideEffect,
		PermissionMode: greenSandbox.PermissionModeAsk,
		Args:           requestArgs,
	...
	report := sandboxCheckReport{
		...
		Plan:     greencommands.SandboxPlanSnapshotFromPlan(plan),
		Decision: greencommands.SandboxDecisionSnapshotFromDecision(decision),
		Grant:    greencommands.SandboxGrantMatchSnapshotFromLookup(options.tool, lookup),
	}
...
func parseSandboxCheckSideEffect(value string) (greenSandbox.SideEffect, error) {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case "", "read":
		return greenSandbox.SideEffectRead, nil
	case "write":
		return greenSandbox.SideEffectWrite, nil
	case "shell":
		return greenSandbox.SideEffectShell, nil
	case "network":
		return greenSandbox.SideEffectNetwork, nil
	case "none":
		return greenSandbox.SideEffectNone, nil
	default:
...
func writeSandboxCheckHelp(w io.Writer) error {
  ...
  green sandbox check <tool> [flags]
