...
func stubInspectChanges(stat string) func(context.Context, greengit.InspectOptions) (greengit.ChangeSummary, error) {
	return func(context.Context, greengit.InspectOptions) (greengit.ChangeSummary, error) {
		return greengit.ChangeSummary{Root: "/repo", DiffStat: stat}, nil
	}
}
...
func TestRunUsageHelp(t *testing.T) {
	...
	for _, want := range []string{"green usage report", "--json", "--days", "--since", "--session"} {
...
// with no usage events exits successfully, prints the header, shows a green
...
func TestRunUsageEmptyStore(t *testing.T) {
	...
	// Total row must show green requests / tokens.
