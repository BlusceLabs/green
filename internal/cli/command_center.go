...
type configSummary = greencommands.ConfigSnapshot
type providerSummary = greencommands.ProviderSnapshot
type modelSummary = greencommands.ModelSnapshot
type providerCatalogSummary = greencommands.ProviderCatalogSnapshot
...
func summarizeConfig(resolved config.ResolvedConfig) configSummary {
	summary := greencommands.ConfigSnapshotFromResolved(resolved)
...
func listModelSummaries(registry modelregistry.Registry, options commandCenterOptions) ([]modelSummary, error) {
	summaries, err := greencommands.ModelSnapshots(registry, greencommands.ModelSnapshotOptions{
...
func listProviderCatalogSummaries(options commandCenterOptions) ([]providerCatalogSummary, error) {
	summaries, err := greencommands.ProviderCatalogSnapshots(greencommands.ProviderCatalogSnapshotOptions{
...
func formatProviderCatalogLine(provider providerCatalogSummary) string {
	...
	if provider.RuntimeSupported {
		lines = append(lines, "    setup: green providers setup "+displayCLIValue(provider.ID, "unknown")+" --set-active")
	} else {
...
func writeModelsHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green models list [flags]

Lists green model registry entries.
...
func writeProvidersHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  green providers current [flags]
  green providers list [flags]
  green providers catalog [flags]
  green providers add <catalog-id> [flags]
  green providers check [name] [flags]
  green providers use <name> [flags]
  green providers remove <name> [flags]
  green providers rename <old> <new> [flags]
  green providers setup <catalog-id> [flags]
  green providers detect [flags]
  green providers models [name] [flags]
