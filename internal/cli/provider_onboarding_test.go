...
func TestRunProvidersUseSetsActiveProvider(t *testing.T) {
	...
	for _, want := range []string{"Active provider set to fast", "green providers check fast"} {
...
func TestRunProvidersSetupPrintsCommandPlan(t *testing.T) {
	...
	for _, want := range []string{
		...
		"green providers add groq --name fast --model llama-3.1-70b --base-url https://gateway.example/v1 --api-key-env FAST_API_KEY",
		"green providers check fast --connectivity",
...
func TestRunProvidersSetupJSONIncludesCommands(t *testing.T) {
	...
	if payload.CatalogID != "groq" ||
		payload.Name != "fast" ||
		payload.AddCommand != "green providers add groq --name fast --api-key-env GROQ_API_KEY --set-active" ||
		payload.CheckCommand != "green providers check fast --connectivity" ||
		payload.UseCommand != "" ||
...
func TestRunProvidersSetupHelpListsOnboardingCommands(t *testing.T) {
	...
	output := stdout.String()
	for _, want := range []string{"green providers use <name>", "green providers setup <catalog-id>"} {
		if !strings.Contains(output, want) {
