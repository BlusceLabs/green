...
func TestRunExecStreamJSONOutputsRunEndAndRecordsSession(t *testing.T) {
	...
	store := sessions.NewStore(sessions.StoreOptions{
		RootDir: filepath.Join(dataHome, "green", "sessions"),
	})
...
func TestRunExecJSONInterruptedEmitsTerminalEvents(t *testing.T) {
	...
	exitCode := runWithDeps([]string{"exec", "--output-format", "json", "hello"}, &stdout, &stderr, appDeps{
		...
		newProvider:   func(config.ProviderProfile) (greenruntime.Provider, error) { return canceledExecProvider{}, nil },
	})
...
func (reasoningExecProvider) StreamCompletion(context.Context, greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	ch := make(chan greenruntime.StreamEvent, 3)
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventReasoning, Content: "Thinking. "}
...
func (provider toolCallingExecProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	for _, message := range request.Messages {
		if message.Role == greenruntime.MessageRoleTool {
			ch := make(chan greenruntime.StreamEvent, 2)
			ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: provider.answer}
			ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventDone}
			close(ch)
			return ch, nil
		}
	...
	case <-ctx.Done():
		close(ch)
		return ch, ctx.Err()
	case ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallStart, ToolCallID: provider.toolCallID, ToolName: provider.toolName}:
	}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: provider.toolCallID, ArgumentsFragment: provider.arguments}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: provider.toolCallID}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventDone}
	close(ch)
	return ch, nil
}

func (provider toolThenErrorExecProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	for _, message := range request.Messages {
		if message.Role == greenruntime.MessageRoleTool {
