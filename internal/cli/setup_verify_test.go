...
func TestFormatSetupCompleteIncludesTryThisExample(t *testing.T) {
	out := formatSetupComplete(tui.SetupResult{
		ConfigPath: "/home/u/.config/green/config.json",
	...
	if !strings.Contains(out, "green setup complete") {
	...
	if !strings.Contains(out, `green exec "`) || !strings.Contains(out, "--model gpt-4.1") {
