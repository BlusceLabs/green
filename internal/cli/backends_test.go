...
import (
	...
	"github.com/BlusceLabs/green/internal/greencommands"
)
...
func TestRunBackendsJSONUsesLifecycleSnapshotWithoutConnectingMCP(t *testing.T) {
	...
	deps := appDeps{
		...
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return nil, errors.New("green backends must not connect to MCP servers")
		},
	}
	...
	var snapshot greencommands.BackendLifecycleSnapshot
	...
	if len(snapshot.Hooks) != 1 || snapshot.Hooks[0].ID != "green.preflight" || strings.Contains(strings.Join(snapshot.Hooks[0].Args, " "), secret) {
		t.Fatalf("unexpected hook snapshots: %#v", snapshot.Hooks)
	}
	if len(snapshot.Plugins) != 1 || snapshot.Plugins[0].ID != "green.docs" || snapshot.Plugins[0].ToolCount != 1 || snapshot.Plugins[0].PromptCount != 1 || snapshot.Plugins[0].HookCount != 1 {
...
func TestRunBackendsTextAndHelp(t *testing.T) {
	...
	for _, want := range []string{"green Backends:", "MCP servers: 0", "Hooks: 0", "Plugins: 0"} {
	...
	for _, want := range []string{"Usage:", "green backends", "--json"} {
		if !strings.Contains(stdout.String(), want) {
...
func TestRunBackendsDoctorJSONAndTextWithoutConnectingMCP(t *testing.T) {
	...
	deps := appDeps{
		...
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return nil, errors.New("green backends doctor must not connect to MCP servers")
		},
	}
	...
	var payload greencommands.BackendDoctorReport
	...
	if payload.Status != greencommands.BackendDoctorStatusFail {
		t.Fatalf("payload.Status = %q, want %q", payload.Status, greencommands.BackendDoctorStatusFail)
	}
	assertBackendDoctorPayloadCheck(t, payload, "backend.mcp.invalid", "broken")
	assertBackendDoctorPayloadCheck(t, payload, "backend.hooks.diagnostic", "green.bad")
	...
	for _, want := range []string{"green backend doctor", "[fail] backend.mcp.invalid", "green mcp add broken", "[fail] backend.hooks.diagnostic"} {
		if !strings.Contains(stdout.String(), want) {
...
func TestRunBackendsDoctorHelp(t *testing.T) {
	...
	for _, want := range []string{"Usage:", "green backends doctor", "--json"} {
		if !strings.Contains(stdout.String(), want) {
...
func TestRunBackendsDoctorDoesNotConnectOrExecuteConfiguredBackends(t *testing.T) {
	...
	helperArgs := func(path string) []string {
		return []string{"-test.run=TestBackendDoctorHelperProcess", "--", "--green-backend-doctor-sentinel", path}
	}
	...
	hookConfigPath := filepath.Join(cwd, ".green", "hooks.json")
	...
	pluginDir := filepath.Join(cwd, ".green", "plugins", "sentinel")
	...
	deps := appDeps{
		...
		loadPlugins: func(options plugins.LoadOptions) (plugins.LoadResult, error) {
			return plugins.Load(plugins.LoadOptions{
				Roots: []plugins.Root{{Source: plugins.SourceProject, Path: filepath.Join(cwd, ".green", "plugins")}},
				Cwd:   options.Cwd,
			})
		},
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return nil, errors.New("green backends doctor must not register MCP tools")
		},
	}
...
func assertBackendDoctorPayloadCheck(t *testing.T, report greencommands.BackendDoctorReport, id string, target string) {
	t.Helper()
...
func TestBackendDoctorHelperProcess(t *testing.T) {
	for index, arg := range os.Args {
		if arg != "--green-backend-doctor-sentinel" || index+1 >= len(os.Args) {
			continue
		}
