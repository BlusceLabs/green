...
func TestRunPluginsListsJSONAndText(t *testing.T) {
	...
	if payload.Plugins[0].ID != "green.docs" || payload.Plugins[0].Name != "Docs" || payload.Plugins[0].Version != "1.0.0" {
	...
	for _, want := range []string{"green Plugins:", "green.docs", "Docs", "1.0.0", "1 prompts"} {
...
func TestRunHooksListsRedactedJSONAndText(t *testing.T) {
	...
	if payload.Hooks.Hooks[0].ID != "green.preflight" {
	...
	if !strings.Contains(stdout.String(), "green Hooks") || !strings.Contains(stdout.String(), "green.preflight") {
...
func TestRunMCPPermissionsHelpDoesNotOpenStore(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			if !strings.Contains(stdout.String(), "green mcp permissions") {
