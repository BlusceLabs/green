...
func providerSetupAddCommand(options providerSetupOptions, profile config.ProviderProfile) string {
	parts := []string{"green", "providers", "add", profile.CatalogID}
	if strings.TrimSpace(options.name) != "" {
...
// provider needs no new browser round-trip; `green auth logout <name>` removes it.
func runProvidersRemove(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	options, help, err := parseProviderNamesArgs(args, 1, "usage: green providers remove <name>")
	if err != nil {
		return writeExecUsageError(stderr, err.Error())
	}
	if help {
	...
	if active := strings.TrimSpace(cfg.ActiveProvider); active != "" {
		...
		if _, err := fmt.Fprintln(stdout, "No providers remain — run green setup to add one."); err != nil {
			return exitCrash
		}
	}
	return exitSuccess
...
func runProvidersRename(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	options, help, err := parseProviderNamesArgs(args, 2, "usage: green providers rename <old> <new>")
	if err != nil {
		return writeExecUsageError(stderr, err.Error())
	}
	if help {
