...
// runUpdateCommand backs both `green update` and `green upgrade`. defaultApply
// makes `--apply` the implicit behavior for `green upgrade` when neither
// `--check` nor `--apply` is passed explicitly; `green update` keeps requiring
// one of the two so existing scripts around `green update --check` don't change.
func runUpdateCommand(args []string, stdout io.Writer, stderr io.Writer, deps appDeps, defaultApply bool) int {
	...
	if !options.check && !options.apply {
		if !defaultApply {
			return writeUsageError(stderr, "Pass --check to check for updates or --apply to install one (or use `green upgrade`).")
		}
		options.apply = true
	}
...
func writeUpdateHelp(w io.Writer) error {
  ...
  green update --check [flags]
  green update --apply [flags]
  green upgrade [flags]
