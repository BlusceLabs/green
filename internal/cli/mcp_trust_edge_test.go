...
// writeProjectMCPConfig drops a ./.green/config.json under dir declaring one MCP server,
...
func breakTrustStore(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(configRoot, "green", "trust.json"), 0o700); err != nil {
		t.Fatal(err)
	}
}

// TestResolveOAuthServerValidationErrorReturnsgreenSkip locks the new 3-value contract on
// the pre-resolve error path: an invalid server name errors before trust is resolved, so
// the returned skip is green (nothing was excluded yet).
func TestResolveOAuthServerValidationErrorReturnsgreenSkip(t *testing.T) {
	...
	if skip.excludedProjectConfig || skip.trustCheckErrored {
		t.Fatalf("a validation error must return a green skip, got %+v", skip)
	}
}
...
func TestRunMCPOAuthLoginNoticeCoFiresWithNonOAuthError(t *testing.T) {
	...
	if !strings.Contains(got, "green trust") {
