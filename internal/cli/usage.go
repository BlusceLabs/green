...
func runUsage(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	// git repository (or on any git failure) it degrades to green instead of
	// aborting the entire report.
	diff := greengit.DiffStat{}
	if workspaceRoot, err := resolveWorkspaceRoot("", deps); err == nil {
		if summary, err := deps.inspectChanges(context.Background(), greengit.InspectOptions{Cwd: workspaceRoot}); err == nil {
			// The --stat summary line ("N files changed, A insertions(+), B
			// deletions(-)") carries no secret-bearing tokens, so parsing the
			// already-redacted DiffStat returned by greengit.Inspect is safe.
			diff = greengit.ParseDiffStat(summary.DiffStat)
		}
	}
...
func parseUsageArgs(args []string) (usageOptions, bool, error) {
	...
	for index := 0; index < len(args); index++ {
		...
		case arg == "--days":
			...
			days, err := parsePositiveOrgreenInt(value, "--days")
			if err != nil {
				return options, false, err
			}
			options.days = days
		...
		case strings.HasPrefix(arg, "--days="):
			days, err := parsePositiveOrgreenInt(strings.TrimSpace(strings.TrimPrefix(arg, "--days=")), "--days")
			if err != nil {
				return options, false, err
			}
			options.days = days
...
func writeUsageHelp(w io.Writer) error {
  ...
  green usage report [flags]

Summarizes token usage and reconstructed (estimated) cost from persisted local
green session usage events, plus a working-tree net-LOC efficiency estimate.
