...
func writeAgentEvalHelp(w io.Writer) error {
  ...
  green eval --suite <path> [flags]
  green eval run --suite <path> [flags]
  green eval bench --suite <path> [flags] [--model <id>] [--agent-command <argv...>]
...
func agentEvalBenchWorkRoot(options agentEvalOptions) (string, string, error) {
	...
	created, err := os.MkdirTemp("", "green-eval-")
	if err != nil {
