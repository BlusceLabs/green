...
func runProvidersAdd(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, map[string]any{
			...
			"provider":       greencommands.ProviderSnapshotFromProfile(profile, cfg.ActiveProvider == profile.Name),
		}); err != nil {
			return exitCrash
		}
		return exitSuccess
...
func runProvidersCheck(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	snapshot := greencommands.ProviderSnapshotFromProfile(profile, profile.Name == resolved.ActiveProvider)
	if options.json {
...
func providerCheckNextActions(profile config.ProviderProfile, connectivity bool, health providerhealth.Result) []string {
	...
	if !connectivity {
		return []string{fmt.Sprintf("run green providers check %s --connectivity", name)}
	}
	switch health.Status {
	case providerhealth.StatusFail:
		return []string{fmt.Sprintf("verify the API key, base URL, and model, then rerun green providers check %s --connectivity", name)}
	case providerhealth.StatusWarn:
		return []string{fmt.Sprintf("review the warning, then rerun green providers check %s --connectivity", name)}
	default:
		model := strings.TrimSpace(profile.Model)
		if model == "" {
			return []string{"provider is ready"}
		}
		return []string{fmt.Sprintf("run green exec %q --model %s", "hello", model)}
	}
}
...
func providerCheckMissingKeyNextAction(profile config.ProviderProfile) string {
	...
	return fmt.Sprintf("set %s and rerun green providers check %s", apiKeyEnv, providerCheckName(profile))
}
...
func validateProviderRuntimeReady(profile config.ProviderProfile) error {
	// A stored OAuth login (e.g. `green auth chatgpt`) is a credential too: a
