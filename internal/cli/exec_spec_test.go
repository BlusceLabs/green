...
func TestRunExecUseSpecCreatesDraftSession(t *testing.T) {
	...
	if draft.SpecID == "" || !strings.Contains(filepath.ToSlash(draft.SpecFilePath), ".green/specs/") {
...
func TestParseExecUseSpecRejectsResume(t *testing.T) {
	_, _, err := parseExecArgs([]string{"--use-spec", "--resume", "green_1", "plan"})
...
func (provider *submitSpecExecProvider) StreamCompletion(ctx context.Context, request greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	...
	case <-ctx.Done():
		close(ch)
		return ch, ctx.Err()
	case ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: specmode.SubmitToolName}:
	}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: string(arguments)}
	ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventToolCallEnd, ToolCallID: "call-1"}
