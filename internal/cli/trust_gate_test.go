...
// at <root>/.config/green while the tests wrote to <root>/green, so the store-error
...
// writeProjectHooks writes a project ./.green/hooks.json under repo with the given
// hooks and top-level enabled flag.
func writeProjectHooks(t *testing.T, repo string, enabled bool, defs ...hooks.Definition) {
	t.Helper()
	path := filepath.Join(repo, ".green", "hooks.json")
...
func writeUserHooks(t *testing.T, configRoot string, enabled bool, defs ...hooks.Definition) {
	t.Helper()
	path := filepath.Join(configRoot, "green", "hooks.json")
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
...
func TestPluginGateUntrustedExcludesProject(t *testing.T) {
	setTrustConfigRoot(t)
	repo := t.TempDir()
	// A ./.green/plugins dir so the skip report flags an excluded project config.
	if err := os.MkdirAll(filepath.Join(repo, ".green", "plugins"), 0o700); err != nil {
	...
	if !activation.excludedProjectConfig {
		t.Fatalf("skip report should flag the excluded ./.green/plugins dir")
	}
...
func TestEmitTrustNoticeOneLineWhenSkipped(t *testing.T) {
	t.Run("both surfaces skipped yields one line", func(t *testing.T) {
		...
		if !bytes.Contains(buf.Bytes(), []byte("green trust")) {
			t.Fatalf("notice should point at 'green trust', got %q", buf.String())
		}
	})
