...
import (
	...
	"github.com/BlusceLabs/green/internal/imageinput"
...
type execOptions struct {
	...
	// mutating tool call green runs the workspace verification plan and feeds
	...
	// present to reply (e.g. greenclaw chat). There, a final message that hands
...
func runExec(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	// modelRegistry is the green Registry when the catalog fails to build; the
	...
	if !config.HasProviderProfile(resolved.Provider) {
		return writeExecProviderError(stdout, stderr, options.outputFormat, "provider_error", "No provider configured. Run `green setup` (guided), `green auth` (OAuth providers), set a provider API key env var (e.g. OPENAI_API_KEY / ANTHROPIC_API_KEY / GEMINI_API_KEY), or add .green/config.json.")
	}
	...
	// Opt-in webhook fan-out (green_NOTIFY_WEBHOOK_URL). Headless runs can safely
...
// or with a green/negative threshold, tool_search is never registered, so the
...
func resolveExecPrompt(options execOptions, workspaceRoot string, stdin io.Reader) (string, []greenruntime.ImageBlock, error) {
	...
	if prompt == "" {
		return "", nil, execUsageError{"Prompt required. Use `green exec \"prompt\"` or `green exec --file prompt.txt`."}
	}
...
func resolveExecImages(paths []string, workspaceRoot string) ([]greenruntime.ImageBlock, error) {
	if len(paths) == 0 {
		return nil, nil
	}
	images := make([]greenruntime.ImageBlock, 0, len(paths))
