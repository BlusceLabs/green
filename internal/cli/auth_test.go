...
// pinning the file backend so an inherited green_OAUTH_STORAGE=keyring can't
...
func withAuthStore(t *testing.T) string {
	...
	t.Setenv("green_OAUTH_TOKENS_PATH", path)
	t.Setenv("green_OAUTH_STORAGE", "file")
	return path
}
...
func TestRunAuthRejectsInvalidStorageMode(t *testing.T) {
	...
	t.Setenv("green_OAUTH_STORAGE", "encryptd")
	var stdout, stderr bytes.Buffer
	if code := runWithDeps([]string{"auth", "status"}, &stdout, &stderr, appDeps{}); code == exitSuccess {
		t.Fatalf("invalid green_OAUTH_STORAGE should fail, got success; stdout=%q", stdout.String())
	}
	if !strings.Contains(stderr.String(), "green_OAUTH_STORAGE") {
...
func TestRunAuthRefreshNoToken(t *testing.T) {
	withAuthStore(t)
	t.Setenv("green_OAUTH_DEMO_CLIENT_ID", "client") // so config resolves; refresh still fails (no token)
	var stdout, stderr bytes.Buffer
...
func TestRunAuthHelp(t *testing.T) {
	...
	for _, want := range []string{"green auth", "login", "logout", "status", "refresh", "--device"} {
...
// TestRunAuthLoginChatGPTRoutesToDedicatedFlow verifies `green auth login
...
func TestEnsureLoginProviderProfileAddsProviderWithoutStealingActive(t *testing.T) {
	...
	if !strings.Contains(line, "green providers use chatgpt") {
