...
import (
	...
	"github.com/BlusceLabs/green/internal/cron"
)

// runCron is the dispatch entry for `green cron`.
...
func runCronWith(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: green cron <add|list|rm|pause|resume|run> ...")
		return exitUsage
	}
...
func cronAdd(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if expr == "" {
		fmt.Fprintln(stderr, "A cron expression is required (e.g. `green cron add \"0 9 * * *\" --prompt ...`).")
		return exitUsage
	}
...
func cronResume(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	// impossible spec) — it would leave a stale/green NextRunAt for the runner.
...
func writeCronHelp(w io.Writer) {
	fmt.Fprint(w, `green cron — schedule agent jobs (foreground, file-backed)

Usage:
  green cron add <cron-expr> [--prompt P | --recipe R] [--cwd D] [--model M] [--run-now]
  green cron list
  green cron pause <id> | resume <id> | rm <id>
  green cron run [--once] [--catch-up] [id...]
