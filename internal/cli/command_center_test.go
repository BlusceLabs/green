...
func TestRunProvidersCatalogJSONIncludesDescriptors(t *testing.T) {
	...
	var payload struct {
		Providers []greencommands.ProviderCatalogSnapshot `json:"providers"`
	}
...
func TestRunProvidersAddWritesCatalogProfile(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	configPath := filepath.Join(t.TempDir(), "green", "config.json")
...
func TestRunProvidersAddWritesCustomHeaders(t *testing.T) {
	...
	exitCode := runWithDeps([]string{
		...
		"--auth-scheme", "Token",
		"--header", "HTTP-Referer=https://green.dev",
		"--header", "X-Title=green",
	...
	if profile.CustomHeaders["HTTP-Referer"] != "https://green.dev" || profile.CustomHeaders["X-Title"] != "green" {
...
func TestRunProvidersCheckConnectivityJSON(t *testing.T) {
	...
	deps.newProvider = func(config.ProviderProfile) (greenruntime.Provider, error) {
...
func TestRunProvidersPositionalHelp(t *testing.T) {
	...
	output := stdout.String()
	for _, want := range []string{"Usage:", "green providers", "list", "current", "catalog"} {
		if !strings.Contains(output, want) {
...
func findProviderCatalogSnapshot(t *testing.T, snapshots []greencommands.ProviderCatalogSnapshot, id string) greencommands.ProviderCatalogSnapshot {
	t.Helper()
	...
	return greencommands.ProviderCatalogSnapshot{}
}

type commandCenterProvider struct{}

func (commandCenterProvider) StreamCompletion(context.Context, greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
...
// is a stored OAuth login (the shape `green auth chatgpt` now writes) must render
...
func TestProvidersListMarksOAuthLoginProviders(t *testing.T) {
	...
	t.Setenv("green_OAUTH_TOKENS_PATH", tokensPath)
