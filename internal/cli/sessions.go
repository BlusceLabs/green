...
func runSessionsList(store *sessions.Store, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, redaction.RedactValue(greencommands.SessionSnapshots(items), redaction.Options{})); err != nil {
			return exitCrash
		}
		return exitSuccess
	}
	if _, err := fmt.Fprintln(stdout, formatSessionSnapshotsList(greencommands.SessionSnapshots(items))); err != nil {
		return exitCrash
	}
	return exitSuccess
}
...
func runSessionsLineage(store *sessions.Store, sessionID string, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, redaction.RedactValue(greencommands.SessionSnapshots(lineage), redaction.Options{})); err != nil {
			return exitCrash
		}
		return exitSuccess
	}
	ids := make([]string, 0, len(lineage))
	for _, session := range greencommands.SessionSnapshots(lineage) {
...
func runSessionsTree(store *sessions.Store, sessionID string, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, redaction.RedactValue(greencommands.SessionTreeSnapshotFromNode(tree), redaction.Options{})); err != nil {
			return exitCrash
		}
		return exitSuccess
	}
	if _, err := fmt.Fprint(stdout, formatSessionSnapshotTree(greencommands.SessionTreeSnapshotFromNode(tree))); err != nil {
		return exitCrash
	}
	return exitSuccess
}
...
func runSessionsRewind(store *sessions.Store, sessionID string, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if session == nil {
		return writeExecUsageError(stderr, "green session not found: "+redact(sessionID))
	}
...
func writeSessionCommandError(stderr io.Writer, err error) int {
	message := strings.TrimPrefix(err.Error(), "green session")
	if message != err.Error() {
		message = "green session" + message
	}
	if strings.Contains(message, "not found") || strings.Contains(message, "invalid green session id") {
...
func formatSessionSnapshotsList(items []greencommands.SessionSnapshot) string {
	if len(items) == 0 {
		return "No green sessions found."
	}
	lines := []string{fmt.Sprintf("green sessions (%d):", len(items))}
...
func formatSessionSnapshotTree(node greencommands.SessionTreeSnapshot) string {
	lines := []string{"green session tree:"}
	appendSessionSnapshotTree(&lines, node, "")
	return strings.Join(lines, "\n") + "\n"
}

func appendSessionSnapshotTree(lines *[]string, node greencommands.SessionTreeSnapshot, prefix string) {
...
func formatSessionSnapshotLine(session greencommands.SessionSnapshot) string {
...
func writeSessionsHelp(w io.Writer) error {
  ...
  green sessions <command> [flags]

Commands:
  list                  List local green sessions
