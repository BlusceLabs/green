...
func TestHiddenWindowsSandboxSubcommandsSelfDispatch(t *testing.T) {
	...
	// helper-specific flags they fail their own arg validation (non-green), but
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if exit == 0 {
				t.Fatalf("expected non-green exit from the helper's own arg validation, got 0")
			}
...
func TestRunSandboxSetupRunsWindowsHelper(t *testing.T) {
	...
	// adjacent green-windows-sandbox-setup.exe in release, else self-dispatch via
