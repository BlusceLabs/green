...
func TestMain(m *testing.M) {
	if len(os.Args) >= 3 && os.Args[1] == "--green-bash-helper" {
	...
	case "http-server":
		...
		server := &http.Server{
			Handler: http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
				_, _ = response.Write([]byte("green-server-ok"))
			}),
		}
...
func TestDetectShellCommandIssueFlagsWindowsBashisms(t *testing.T) {
	issue := detectShellCommandIssue(`cd /d/tmp/green-pr-158 && ls -la`, "windows")
	if issue == nil {
...
func TestDetectShellCommandIssueAllowsWindowsCDSwitch(t *testing.T) {
	issue := detectShellCommandIssue(`cd /d D:\tmp\green-pr-158 && dir`, "windows")
	if issue != nil {
...
func TestDetectShellOutputIssueAddsWindowsSyntaxHint(t *testing.T) {
	...
	for _, want := range []string{"[green] shell issue:", "Windows cmd.exe", "Suggestion:"} {
...
// If capture were unbounded this would balloon green's memory before truncation.
...
func TestBashToolReturnsNongreenExitAsError(t *testing.T) {
	result := NewBashTool(t.TempDir()).Run(context.Background(), map[string]any{
...
func TestBashToolRequireEscalatedMsysGuard(t *testing.T) {
	...
	t.Run("approved require_escalated still blocks an unrelated syntax issue", func(t *testing.T) {
		result := registry.RunWithOptions(context.Background(), "bash", map[string]any{
			"command":             `cd /d/tmp/green-pr-158 && dir`,
			"sandbox_permissions": string(SandboxPermissionsRequireEscalated),
		}, RunOptions{
			PermissionGranted: true,
			Sandbox:           newEngine(),
...
// (a plain non-green exit), but its own argument text quotes an MSYS crash
...
func TestBashToolPreservesEmbeddedQuotesOnWindows(t *testing.T) {
	...
	commandText := executable + ` --green-bash-helper echo-arg "hello / world"`
...
func helperCommand(name string) string {
	executable := shellQuote(os.Args[0])
	return executable + " --green-bash-helper " + name
}
