...
func TestListDirectoryToolListsRecursivelyAndIgnoresJunk(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, "README.md"), "# green")
...
func TestGlobToolFindsMatchesWithLimit(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, filepath.Join(root, "a.go"), "package green")
...
func TestGrepSkipsAlwaysExcludedDirectories(t *testing.T) {
	...
	mustWrite(".green/state.json", "needle here")
	...
	for _, forbidden := range []string{".git", ".green", "node_modules", "vendor", ".worktrees"} {
...
func TestGlobSkipsWorkspaceExcludedDirectoriesAndBinaryFiles(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, ".green", "state.json"), "{}")
	...
	for _, forbidden := range []string{".green", "vendor", ".worktrees", "image.png"} {
		if strings.Contains(res.Output, forbidden) {
