...
func (tool webFetchTool) RunWithSandbox(ctx context.Context, args map[string]any, _ *greenSandbox.Engine) Result {
	return tool.run(ctx, args)
}
...
func (tool webFetchTool) run(ctx context.Context, args map[string]any) Result {
	...
	request.Header.Set("User-Agent", "green-web-fetch/0.1")
