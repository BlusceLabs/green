...
const (
	...
	// tens of gigabytes over several hours and got the whole green process
	// OOM-killed by the OS.
	maxExecOutputBufferBytes         = 2 * 1024 * 1024
	execSessionStopTimeout           = 3 * time.Second
	execSessionEvictedMessage        = "[green] session evicted: too many background terminals\n"
	execOutputBufferTruncatedMessage = "[green] output buffer truncated: undrained output exceeded 2MiB, oldest output dropped"
)
...
func (tool execCommandTool) RunWithSandbox(ctx context.Context, args map[string]any, engine *greenSandbox.Engine) Result {
...
func (tool execCommandTool) run(ctx context.Context, args map[string]any, engine *greenSandbox.Engine) Result {
	...
	if interactive := greenSandbox.DetectInteractiveCommand(commandText, runtimeGOOS()); interactive.Interactive {
...
func (tool execCommandTool) startSession(commandText string, absoluteCwd string, relativeCwd string, ttyRequested bool, engine *greenSandbox.Engine, sandboxPermissions SandboxPermissionOverride) (*execSession, error) {
...
func truncateExecOutputSpill(output string, maxOutputTokens int, toolName string) (string, bool) {
	...
	notice := "\n[green] output truncated\n"
	if spillPath := spillTruncatedOutput(toolName, output); spillPath != "" {
		notice = "\n[green] output truncated — full output saved to " + spillPath + " (grep or read_file it instead of re-running)\n"
	}
	head := maxBytes / 2
	tail := maxBytes - head
