...
const windowsMsysSandboxSuggestion = "MSYS/Cygwin coreutils and shells (bash, sh) from Git for Windows cannot run under green's write-restricted Windows sandbox, and the WSL bash launcher cannot reach the WSL service from it either. This also hits native commands that spawn Git Bash internally: git hooks and git/gh credential helpers can fail this way even though git and gh themselves run fine. Prefer green native tools (grep, read_file with offset/limit, list_directory, glob), cmd.exe findstr, or PowerShell Select-Object -First/-Last. If host-level execution is truly required, rerun with sandbox_permissions: \"require_escalated\" and a narrow justification."
...
func shellGuidanceForGOOS(goos string) string {
	...
	if goos == "windows" {
		return "Uses " + runtime.Syntax + " syntax on Windows; prefer cwd over cd when changing directories. To include | & > < or other metacharacters in an argument value, wrap the value in double quotes (e.g. --jq \".a | b\"); single quotes do not protect metacharacters in cmd.exe. MSYS/Cygwin coreutils on PATH (Git for Windows usr\\bin) are not sandbox-compatible; prefer native green file tools."
	}
...
func detectShellCommandIssue(command string, goos string) *shellIssue {
	...
	if windowsBashStyleCDPattern.MatchString(unquoted) {
		return &shellIssue{
			...
			Message:    "Command looks like POSIX/Bash syntax, but green runs bash tool commands through Windows cmd.exe on this host.",
	...
	for _, segment := range windowsCommandSegments(trimmed) {
		...
		if windowsMsysBinaryPathPattern.MatchString(word) {
			return windowsMsysSandboxIssue("Command invokes an MSYS/Cygwin binary path that cannot run under green's Windows sandbox.")
		}
...
func detectShellOutputIssue(output string, goos string) *shellIssue {
	...
	if msysRuntimeFailedInOutput(lower) {
		return windowsMsysSandboxIssue("An MSYS/Cygwin runtime failed under green's Windows sandbox (ACCESS_DENIED during MSYS startup).")
	}
	if wslServiceDeniedInOutput(lower) {
		return windowsMsysSandboxIssue("WSL bash could not connect to the WSL service under green's Windows sandbox (Bash/Service/CreateInstance/E_ACCESSDENIED).")
	}
...
func appendShellIssueHint(output string, issue shellIssue) string {
	...
	hint := "[green] shell issue: " + issue.Message
