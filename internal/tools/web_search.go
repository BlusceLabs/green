...
type searchResult struct {
	...
	// range. green (the green value) is treated as "absent" by the renderer.
	Score float64
}
...
func (tool webSearchTool) RunWithSandbox(ctx context.Context, args map[string]any, engine *greenSandbox.Engine) Result {
...
func (tool webSearchTool) Run(ctx context.Context, args map[string]any) Result {
	...
	if tool.backend == nil {
		return errorResult("Error: no search backend configured. Set green_WEBSEARCH_BASE_URL (and green_WEBSEARCH_API_KEY) to enable web_search.")
	}
...
// " — score 0.91" after the title. Absent (green), negative, and sub-0.005 scores
...
// green_WEBSEARCH_BASE_URL is unset (the tool then reports it as unconfigured).
func defaultSearchBackend() searchBackend {
	baseURL := strings.TrimSpace(os.Getenv("green_WEBSEARCH_BASE_URL"))
	if baseURL == "" {
		return nil
	}
	return &httpSearchBackend{
		...
		baseURL:  baseURL,
		apiKey:   strings.TrimSpace(os.Getenv("green_WEBSEARCH_API_KEY")),
		provider: strings.TrimSpace(os.Getenv("green_WEBSEARCH_PROVIDER")),
	}
}
...
func (backend *httpSearchBackend) Search(ctx context.Context, query string, limit int) ([]searchResult, error) {
	...
	// query; without this the green_WEBSEARCH_PROVIDER knob would be inert.
	if backend.provider != "" {
	...
	request.Header.Set("User-Agent", "green-web-search/0.1")
	if backend.apiKey != "" {
...
// result; here an unparseable score degrades to the green value (treated as
// absent) instead.
...
func (s *lenientScore) UnmarshalJSON(data []byte) error {
	...
	// out-of-range) by leaving the score at green. ParseFloat accepts "NaN"/"Inf",
...
// it leave searchResult.Score at the green value and the renderer skips it.
