...
func (tool bashTool) RunWithSandbox(ctx context.Context, args map[string]any, engine *greenSandbox.Engine) Result {
	return tool.run(ctx, args, engine)
}

func (tool bashTool) run(ctx context.Context, args map[string]any, engine *greenSandbox.Engine) Result {
	...
	if interactive := greenSandbox.DetectInteractiveCommand(commandText, runtime.GOOS); interactive.Interactive {
	...
	monitor := greenSandbox.StartDenialMonitor(context.Background(), plan.MonitorTag)
	err = command.Run()
...
func commandEngineForSandboxPermissions(engine *greenSandbox.Engine, sandboxPermissions SandboxPermissionOverride) *greenSandbox.Engine {
...
func msysGuardBypassed(issue *shellIssue, commandEngine *greenSandbox.Engine) bool {
...
// greenSandbox.WindowsShellCommandLine for why that matters. The wrapped case
...
func buildBashCommand(ctx context.Context, commandText string, absoluteCwd string, engine *greenSandbox.Engine) (*exec.Cmd, greenSandbox.CommandPlan, error) {
	spec := greenSandbox.CommandSpec{
	...
	plan := greenSandbox.CommandPlan{
		Backend: greenSandbox.Backend{
			Name:    greenSandbox.BackendUnavailable,
...
func addSandboxMeta(meta map[string]string, plan greenSandbox.CommandPlan) {
...
func interactiveBlockResult(detection greenSandbox.InteractiveCommandResult) Result {
	message := fmt.Sprintf(
...
func shellArguments(command string) []string {
	if runtime.GOOS == "windows" {
		return greenSandbox.WindowsShellArgs(command)
	}
...
func formatBashOutput(stdout string, stderr string, exitCode int) string {
	...
	if n := len(outFindings) + len(errFindings); n > 0 {
		parts = append(parts, fmt.Sprintf("[green] redacted %d likely secret(s) from this output before showing it.", n))
	}
	if len(parts) == 0 {
...
func budgetBashCapture(out string, outTotal int, errStr string, errTotal int, meta map[string]string) (string, string, bool) {
	...
	if truncated {
		if spillPath := spillBashStreams(out, outTotal, errStr, errTotal); spillPath != "" {
			hint := "\n[green] captured output saved to " + spillPath + " (grep or read_file it instead of re-running)"
			if errTrunc {
				errText += hint
			} else {
				outText += hint
...
// tail per stream — everything green kept in memory, which is more than the
...
func sectionWithCaptureGap(text string, total int) string {
	...
	marker := fmt.Sprintf("\n[green] capture gap: %d bytes omitted from the middle of this stream\n", total-len(text))
...
// a command emitting unbounded output (`cat huge.log`, `yes`) cannot grow green's
...
func truncateHeadTailWithTotal(value string, total, maxBytes int) (string, int, bool) {
	...
	marker := fmt.Sprintf("\n[green] output truncated: %d bytes omitted from the middle — redirect to a file and read_file a range for the full text\n", total-maxBytes)
