...
func markLikelySandboxDenial(meta map[string]string, plan greenSandbox.CommandPlan, exitCode int, sections ...string) {
...
func likelySandboxDenied(plan greenSandbox.CommandPlan, exitCode int, sections ...string) bool {
	kind, _ := sandboxDenialKind(plan, exitCode, sections...)
	return kind != ""
}

func sandboxDenialKind(plan greenSandbox.CommandPlan, exitCode int, sections ...string) (string, string) {
	if !plan.Wrapped {
		return "", ""
	}
	...
	if plan.TargetBackend == greenSandbox.BackendLinuxBwrap && exitCode == 128+31 {
		return SandboxDenialKindSandbox, "seccomp"
	}
	return "", ""
}

func networkDeniedBySandbox(plan greenSandbox.CommandPlan) bool {
	return plan.PermissionProfile.Network.Mode == greenSandbox.NetworkDeny || plan.Policy.Network == greenSandbox.NetworkDeny
}
