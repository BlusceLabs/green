...
func shellStub(name string, description string) string {
	return `#!/bin/sh
# ` + name + ` — green plugin tool entry point.
# ` + description + `
#
# green invokes this script with the tool-call arguments as a single JSON object
...
func nodeStub(name string, description string) string {
	return `// ` + name + ` — green plugin tool entry point.
// ` + description + `
//
// green invokes this script with the tool-call arguments as a single JSON object
// on stdin and expects the tool result on stdout.
...
green invokes this script with the tool-call arguments as a single JSON object on
