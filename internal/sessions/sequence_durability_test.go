...
func TestAppendDerivesSequenceFromLogAfterStaleMetadata(t *testing.T) {
	...
	s, err := store.Create(CreateInput{SessionID: "green_seq_1", Title: "t", Cwd: "/repo", ModelID: "m", Provider: "p"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAppendWithStaleHighMetadataLeavesGapNotDuplicate(t *testing.T) {
	store := seqTestStore(t)
	s, _ := store.Create(CreateInput{SessionID: "green_seq_2", Title: "t", Cwd: "/repo", ModelID: "m", Provider: "p"})
...
func TestLastEventSequence_EmptyAndMultiple(t *testing.T) {
	store := seqTestStore(t)
	s, _ := store.Create(CreateInput{SessionID: "green_tail_1", Title: "t", Cwd: "/repo", ModelID: "m", Provider: "p"})
...
func TestLastEventSequence_IgnoresTornTail(t *testing.T) {
	store := seqTestStore(t)
	s, _ := store.Create(CreateInput{SessionID: "green_tail_2", Title: "t", Cwd: "/repo", ModelID: "m", Provider: "p"})
	appendN(t, store, s.SessionID, 3)
...
func TestLastEventSequence_LargeLastEvent(t *testing.T) {
	store := seqTestStore(t)
	s, _ := store.Create(CreateInput{SessionID: "green_tail_3", Title: "t", Cwd: "/repo", ModelID: "m", Provider: "p"})
