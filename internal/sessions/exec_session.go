...
func PrepareExec(options PrepareExecOptions) (PreparedExec, error) {
	...
	if forkID != "" {
		...
		if parent == nil {
			return PreparedExec{}, ExecError{"green session not found: " + forkID}
		}
	...
	if resumeID != "" || options.ResumeLatest {
		...
		if sessionID == "" && options.ResumeLatest {
			...
			if latest == nil {
				return PreparedExec{}, ExecError{"No green sessions available to resume."}
			}
		...
		if session == nil {
			return PreparedExec{}, ExecError{"green session not found: " + sessionID}
		}
	...
	if strings.TrimSpace(options.CallingSessionID) != "" {
		...
		if parent == nil {
			return PreparedExec{}, ExecError{"green parent session not found: " + parentSessionID}
		}
...
func readExecContextEvents(store *Store, sessionID string) ([]Event, error) {
	...
	log.Printf("green sessions: failed to rehydrate compaction events for %s; falling back to raw events: %v", sessionID, err)
...
func FormatExecPrompt(prompt string, prepared PreparedExec) string {
	...
	return strings.Join([]string{
		fmt.Sprintf("%s green session %s.", label, sessionID),
