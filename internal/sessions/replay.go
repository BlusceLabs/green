...
func (store *Store) PlanRewind(sessionID string, options RewindOptions) (RewindPlan, error) {
	if !ValidSessionID(sessionID) {
		return RewindPlan{}, fmt.Errorf("invalid green session id %q", sessionID)
	}
	...
	if len(events) == 0 {
		return RewindPlan{}, fmt.Errorf("green session %s has no events to rewind", sessionID)
	}
...
func (store *Store) PlanCompaction(sessionID string, options CompactionOptions) (CompactionPlan, error) {
	if !ValidSessionID(sessionID) {
		return CompactionPlan{}, fmt.Errorf("invalid green session id %q", sessionID)
	}
	events, err := store.ReadEvents(sessionID)
	if err != nil {
...
func (store *Store) RecordCompaction(sessionID string, input RecordCompactionInput) (Event, error) {
	if !ValidSessionID(sessionID) {
		return Event{}, fmt.Errorf("invalid green session id %q", sessionID)
	}
...
func buildCompactionPrompt(events []Event, maxChars int) (string, bool) {
	if len(events) == 0 {
		return "No compactable green session events.", false
	}
	lines := []string{
		"Summarize these green session events for future context.",
