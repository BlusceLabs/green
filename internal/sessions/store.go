...
func DefaultRoot(env map[string]string) string {
	...
	return filepath.Join(base, "green", "sessions")
}
...
func (store *Store) Create(input CreateInput) (Metadata, error) {
	...
	if !ValidSessionID(sessionID) {
		return Metadata{}, fmt.Errorf("invalid green session id %q", input.SessionID)
	}
	if input.Depth < 0 {
		return Metadata{}, fmt.Errorf("invalid green session depth %d", input.Depth)
	}
	...
	if err := os.MkdirAll(store.RootDir, 0o700); err != nil {
		return Metadata{}, fmt.Errorf("create green session root: %w", err)
	}
	if err := os.Mkdir(store.sessionPath(sessionID), 0o700); err != nil {
		if errors.Is(err, os.ErrExist) {
			return Metadata{}, fmt.Errorf("green session already exists: %s", sessionID)
		}
		return Metadata{}, fmt.Errorf("create green session directory: %w", err)
	}
	...
	if err != nil {
		return Metadata{}, fmt.Errorf("create green session events file: %w", err)
	}
	if err := file.Close(); err != nil {
		return Metadata{}, fmt.Errorf("close green session events file: %w", err)
	}
	return session, nil
}
...
func (store *Store) Get(sessionID string) (*Metadata, error) {
	if !ValidSessionID(sessionID) {
		return nil, fmt.Errorf("invalid green session id %q", sessionID)
	}
...
func (store *Store) List() ([]Metadata, error) {
	if err := os.MkdirAll(store.RootDir, 0o700); err != nil {
		return nil, fmt.Errorf("create green session root: %w", err)
	}
	entries, err := os.ReadDir(store.RootDir)
	if err != nil {
		return nil, fmt.Errorf("read green session root: %w", err)
	}
	sessions := []Metadata{}
...
func (store *Store) Fork(parentSessionID string, input ForkInput) (Metadata, error) {
	if !ValidSessionID(parentSessionID) {
		return Metadata{}, fmt.Errorf("invalid green session id %q", parentSessionID)
	}
	...
	if parent == nil {
		return Metadata{}, fmt.Errorf("green session not found: %s", parentSessionID)
	}
...
func (store *Store) RecordSpec(sessionID string, input RecordSpecInput) (Metadata, Event, error) {
	if !ValidSessionID(sessionID) {
		return Metadata{}, Event{}, fmt.Errorf("invalid green session id %q", sessionID)
	}
	status := normalizeSpecStatus(input.SpecStatus)
	if status == "" {
		return Metadata{}, Event{}, fmt.Errorf("green spec status is required")
	}
...
func (store *Store) AppendEventUnlessExists(sessionID string, input AppendEventInput, exists func([]Event) bool) (Event, bool, error) {
	if !ValidSessionID(sessionID) {
		return Event{}, false, fmt.Errorf("invalid green session id %q", sessionID)
	}
	if strings.TrimSpace(string(input.Type)) == "" {
		return Event{}, false, fmt.Errorf("green session event type is required")
	}
	unlock, err := store.lockSession(sessionID)
	if err != nil {
...
func (store *Store) appendPreparedEventsLocked(sessionID string, inputs []preparedAppendEvent) ([]Event, error) {
	...
	for index, input := range inputs {
		...
		if err != nil {
			return nil, fmt.Errorf("encode green session event: %w", err)
		}
		batch.Write(data)
		batch.WriteByte('\n')
	...
	if err != nil {
		return nil, fmt.Errorf("append green session event: %w", err)
	}
	...
	if err := file.Sync(); err != nil {
		_ = file.Close()
		return nil, fmt.Errorf("sync green session event: %w", err)
	}
	if err := file.Close(); err != nil {
		return nil, fmt.Errorf("close green session event file: %w", err)
	}
	last := events[len(events)-1]
...
func (store *Store) UpdateTitle(sessionID string, title string) (Metadata, error) {
	if !ValidSessionID(sessionID) {
		return Metadata{}, fmt.Errorf("invalid green session id %q", sessionID)
	}
	trimmed := strings.TrimSpace(title)
	if trimmed == "" {
		return Metadata{}, fmt.Errorf("green session title is required")
	}
	unlock, err := store.lockSession(sessionID)
	if err != nil {
		return Metadata{}, err
...
func (store *Store) ReadEvents(sessionID string) ([]Event, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("read green session events: %w", err)
	}
	...
	for index, line := range lines {
		...
		if err := json.Unmarshal(line, &event); err != nil {
			...
			return nil, fmt.Errorf("invalid json in green session %s %s at line %d: %w", sessionID, EventsFile, index+1, err)
		}
		events = append(events, event)
	}
	return events, nil
...
func (store *Store) createID() string {
	...
	return fmt.Sprintf("green_%s_%d_%d", timestamp.Format("20060102150405"), timestamp.UnixNano(), store.idCounter.Add(1))
}
...
func (store *Store) readMetadata(sessionID string) (Metadata, error) {
	...
	if err := json.Unmarshal(data, &session); err != nil {
		return Metadata{}, fmt.Errorf("invalid green session metadata %s: %w", sessionID, err)
	}
	return session, nil
}
...
func (store *Store) writeMetadata(session Metadata) error {
	...
	if err != nil {
		return fmt.Errorf("encode green session metadata: %w", err)
	}
	...
	if err := writeFileSync(tmp, append(data, '\n'), 0o600); err != nil {
		return fmt.Errorf("write green session metadata: %w", err)
	}
	if err := os.Rename(tmp, path); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("replace green session metadata: %w", err)
	}
	...
	if err := syncDir(filepath.Dir(path)); err != nil {
		return fmt.Errorf("sync green session dir: %w", err)
	}
	return nil
}
...
func rawPayload(payload any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encode green session payload: %w", err)
	}
	return data, nil
}
...
func prepareAppendEventInputs(inputs []AppendEventInput) ([]preparedAppendEvent, error) {
	...
	for _, input := range inputs {
		if strings.TrimSpace(string(input.Type)) == "" {
			return nil, fmt.Errorf("green session event type is required")
		}
