...
func TestDefaultRunGitSeparatesStdoutAndStderr(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("a non-green git exit must not be a runner error, got %v", err)
	}
	if bad.ExitCode == 0 {
		t.Fatalf("expected non-green exit code for a bad subcommand")
	}
...
func TestPrepareCreatesDetachedGitWorktree(t *testing.T) {
	...
	if !strings.HasPrefix(result.Path, filepath.Join(base, "green-worktree-")) {
...
func TestPrepareReusesExistingGitWorktree(t *testing.T) {
	...
	existing := filepath.Join(base, "green-worktree-"+repoKey(root), "reuse-me")
...
func TestPrepareRejectsExistingWorktreeFromDifferentRepo(t *testing.T) {
	...
	existing := filepath.Join(base, "green-worktree-"+repoKey(root), "other-repo")
	if err := os.MkdirAll(filepath.Join(existing, ".git"), 0o700); err != nil {
		t.Fatal(err)
	}
	runner := &fakeRunner{
...
func TestPrepareValidatesNameAndExistingDirectory(t *testing.T) {
	...
	blocked := filepath.Join(base, "green-worktree-"+repoKey(root), "blocked")
...
func TestDefaultBaseDirUsesStateHome(t *testing.T) {
	...
	if got != filepath.Join(stateHome, "green", "worktrees") {
...
func TestDefaultBaseDirFallsBackForWindowsUserProfile(t *testing.T) {
	...
	profile := `C:\Users\green`
	got, err := DefaultBaseDir(map[string]string{"USERPROFILE": profile})
	if err != nil {
		t.Fatalf("DefaultBaseDir returned error: %v", err)
	}
	expected := filepath.Join(profile, "AppData", "Local", "green", "worktrees")
