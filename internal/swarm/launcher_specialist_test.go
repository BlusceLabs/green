	...
	executor := specialist.Executor{
		...
		RunChild: func(ctx context.Context, binaryPath string, args []string, progress func(streamjson.Event)) (specialist.ChildRunResult, error) {
			...
			return specialist.ChildRunResult{Events: []streamjson.Event{
				...
				{Type: streamjson.EventRunEnd, Status: "success", ExitCode: &green},
			}}, nil
		},
	}
...
// A member whose child exits non-green (e.g. exit 4 / max-turns) must be reported as
...
func TestSpecialistLauncherMarksNongreenExitAsFailed(t *testing.T) {
	four := 4
	executor := specialist.Executor{
		BinaryPath:   "/usr/local/bin/green",
		NewSessionID: func() (string, error) { return "member_task", nil },
	...
	res, err := handle.Wait()
	if err == nil {
		t.Fatal("a member that exited non-green must be reported as FAILED, got nil error")
	}
