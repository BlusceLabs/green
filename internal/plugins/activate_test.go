...
func TestActivateRegistersToolThatInvokesCommand(t *testing.T) {
	...
	if result.Tools[0].ToolName != "lookup" || result.Tools[0].PluginID != "green.demo" {
		t.Fatalf("provenance = %#v", result.Tools)
	}
}

func TestActivateToolMapsNongreenExitToError(t *testing.T) {
	registry := tools.NewRegistry()
	...
	if res.Status != tools.StatusError {
		t.Fatalf("status = %q, want error on non-green exit", res.Status)
	}
...
func TestActivateSkipsMalformedPluginAndContinues(t *testing.T) {
	...
	if !containsSubstring(result.Warnings, "bad") && !containsSubstring(result.Warnings, "green.bad") {
...
func TestActivateIsDeterministic(t *testing.T) {
	...
	plugins := []LoadedPlugin{
		{ID: "green.b", Name: "B", Enabled: true, PluginDir: dirB, Hooks: []HookExtension{{Name: "hb", Event: HookSessionStart, Command: "b.sh"}}},
		{ID: "green.a", Name: "A", Enabled: true, PluginDir: dirA, Hooks: []HookExtension{{Name: "ha", Event: HookSessionStart, Command: "a.sh"}}},
	}
	...
	// Deterministic order is by plugin ID then extension name: green.a before green.b.
	if first.Hooks[0].ID != "green.a.ha" {
		t.Fatalf("first hook id = %q, want green.a.ha (deterministic by plugin then name)", first.Hooks[0].ID)
	}
}
