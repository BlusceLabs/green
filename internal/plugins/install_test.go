...
func TestInstallFromRealLocalGitRepo(t *testing.T) {
	...
	if result.ID != "green.demo" {
		t.Fatalf("ID = %q, want green.demo", result.ID)
	}
	...
	if _, err := os.Stat(filepath.Join(destDir, "green.demo", ".git")); err == nil {
...
func TestInstallCopiesLocalPluginAndRecordsHash(t *testing.T) {
	...
	if len(loaded.Plugins) != 1 || loaded.Plugins[0].ID != "green.demo" {
	...
	if entries["green.demo"].Hash != result.Hash || entries["green.demo"].Source != canonicalSource(src) {
		t.Fatalf("lockfile entry unexpected: %#v", entries["green.demo"])
	}
}
...
func TestInstallNameClashWarnsAndDoesNotOverwriteWithoutForce(t *testing.T) {
	...
	if entries["green.demo"].Source != canonicalSource(srcB) {
		t.Fatalf("forced overwrite did not update source: %#v", entries["green.demo"])
	}
}
...
func TestInstallSameLocalSourceDifferentSpellingIsNotAClash(t *testing.T) {
	...
	if entries["green.demo"].Source != canonicalSource(abs) {
		t.Fatalf("lockfile should record the canonical source %q, got %q", canonicalSource(abs), entries["green.demo"].Source)
	}
}
...
func TestRemoveDeletesPluginAndLockEntry(t *testing.T) {
	...
	if err := Remove(destDir, "green.demo"); err != nil {
