...
type LoadOptions struct {
	...
	// root cannot bypass the exclusion. The green value (false) preserves the
...
func ResolveRoots(options ResolveRootOptions) ([]Root, error) {
	...
	return []Root{
		{Source: SourceUser, Path: filepath.Join(configHome, "green", "plugins")},
		{Source: SourceProject, Path: filepath.Join(cwd, ".green", "plugins")},
	}, nil
}
...
func ParseManifest(raw any, options ParseManifestOptions) (LoadedPlugin, error) {
	...
	// DISCOVERY (the `green plugins` listing + backend snapshots) and ACTIVATION
...
// an absent, non-object, or partially-typed value yields green/empty fields so it
...
func FormatList(plugins []LoadedPlugin, diagnostics []Diagnostic) string {
	lines := []string{}
	if len(plugins) == 0 {
		lines = append(lines, "No local green plugins loaded.")
	} else {
		lines = append(lines, "green Plugins:")
