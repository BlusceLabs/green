...
// `green plugin list` and debugging.
...
func (tool skillTool) Description() string {
	return "Load a named green skill and return its instructions as the tool output. " +
...
func formatPluginToolOutput(output commandOutput) string {
	...
	if n := len(outFindings) + len(errFindings); n > 0 {
		parts = append(parts, fmt.Sprintf("[green] redacted %d likely secret(s) from this plugin tool output before showing it.", n))
	}
	if len(parts) == 0 {
...
// JSON args on stdin and capturing stdout/stderr. A non-green exit is reported via
