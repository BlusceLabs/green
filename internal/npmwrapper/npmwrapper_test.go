...
func TestPackageBinPointsToNodeWrapper(t *testing.T) {
	...
	if pkg.Module != "bin/green.js" {
		t.Fatalf("module = %q, want bin/green.js", pkg.Module)
	}
	...
	wantFiles := map[string]bool{"bin/green.js": false, "scripts/postinstall.mjs": false}
...
func TestPostinstallComputesAssetPlan(t *testing.T) {
	...
	cases := []struct {
		...
		{"linux", "x64", "green-v" + version + "-linux-x64.tar.gz", "green"},
		{"darwin", "arm64", "green-v" + version + "-macos-arm64.tar.gz", "green"},
		{"win32", "x64", "green-v" + version + "-windows-x64.zip", "green.exe"},
	}
	for _, tc := range cases {
		stdout, stderr, err := runPostinstall(t,
			"green_INSTALL_DRY_RUN=1",
			"green_INSTALL_PLATFORM="+tc.platform,
			"green_INSTALL_ARCH="+tc.arch,
		)
		if err != nil {
		...
		wantURL := "https://github.com/BlusceLabs/green/releases/download/v" + version + "/" + tc.wantAsset
...
func TestPostinstallSkipsUnsupportedPlatform(t *testing.T) {
	stdout, stderr, err := runPostinstall(t,
		"green_INSTALL_DRY_RUN=1",
		"green_INSTALL_PLATFORM=plan9",
		"green_INSTALL_ARCH=x64",
	)
	if err != nil {
...
func TestPostinstallSkipsWindowsArm64(t *testing.T) {
	...
	stdout, stderr, err := runPostinstall(t,
		"green_INSTALL_DRY_RUN=1",
		"green_INSTALL_PLATFORM=win32",
		"green_INSTALL_ARCH=arm64",
	)
	if err != nil {
...
func TestPostinstallHonorsSkipEnv(t *testing.T) {
	stdout, stderr, err := runPostinstall(t, "green_SKIP_DOWNLOAD=1")
	if err != nil {
		t.Fatalf("green_SKIP_DOWNLOAD should exit 0, got err=%v stderr=%s", err, stderr)
	}
	if strings.TrimSpace(stdout) != "" {
...
func TestNodeWrapperIsExecutableAndDoesNotImportBun(t *testing.T) {
	root := repoRoot(t)
	wrapperPath := filepath.Join(root, "bin", "green.js")
...
func TestNodeWrapperReportsMissingNativeBinary(t *testing.T) {
	...
	command.Env = append(withoutEnvKey(command.Env, "green_LOCAL_CONTROL_HELPERS"), "green_LOCAL_CONTROL_HELPERS=")
...
func TestNodeWrapperLaunchesNativeBinary(t *testing.T) {
	...
	nativePath := filepath.Join(root, "green")
	if err := os.WriteFile(nativePath, []byte("#!/usr/bin/env sh\nprintf 'mock-green'; for arg in \"$@\"; do printf ' %s' \"$arg\"; done; printf '\\n'\n"), 0o755); err != nil {
	...
	if got := strings.TrimSpace(string(output)); got != "mock-green --version" {
...
func TestNodeWrapperClearsInheritedLocalControlHelperManifestWhenNoHelpers(t *testing.T) {
	...
	root := filepath.Dir(filepath.Dir(wrapperPath))
	nativePath := filepath.Join(root, "green")
	if err := os.WriteFile(nativePath, []byte("#!/usr/bin/env sh\nif [ -n \"${green_LOCAL_CONTROL_HELPERS+x}\" ]; then printf 'set\\n'; else printf 'unset\\n'; fi\n"), 0o755); err != nil {
		t.Fatalf("WriteFile native fixture: %v", err)
	}
	...
	defer cancel()
	command := nodeWrapperCommand(ctx, node, wrapperPath, "--version")
	command.Env = append(withoutEnvKey(command.Env, "green_LOCAL_CONTROL_HELPERS"), `green_LOCAL_CONTROL_HELPERS={"version":1,"helpers":{"agent-browser":{"command":"stale"}}}`)
	output, err := command.CombinedOutput()
	if ctx.Err() != nil {
		t.Fatalf("wrapper timed out launching native binary: %v; output: %s", ctx.Err(), output)
	}
	...
	if got := strings.TrimSpace(string(output)); got != "unset" {
		t.Fatalf("green_LOCAL_CONTROL_HELPERS state = %q, want unset", got)
	}
}
...
func copyWrapperFixture(t *testing.T) string {
	t.Helper()
	root := repoRoot(t)
	bytes, err := os.ReadFile(filepath.Join(root, "bin", "green.js"))
	if err != nil {
		t.Fatalf("ReadFile wrapper: %v", err)
	}
	dir := t.TempDir()
	...
	wrapperPath := filepath.Join(binDir, "green.js")
