...
// trusted, so green can gate project-scoped executable config (hooks, plugins)
...
// own .green/. The store is a plaintext JSON file at
// <UserConfigDir>/green/trust.json; it holds workspace paths, not secrets, so it
...
// storeFilePath returns <UserConfigDir>/green/trust.json, reusing the config
...
func storeFilePath() (string, error) {
	...
	return filepath.Join(dir, "green", "trust.json"), nil
}
