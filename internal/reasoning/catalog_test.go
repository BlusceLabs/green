...
// TestCoversgreenShippedReasoningModels pins that every reasoning model green
// currently ships resolves in the catalog (by its api model id), so the
// models.dev fallback actually covers green's catalog rather than just well-known
// ids. The api ids mirror internal/modelregistry's curated entries.
func TestCoversgreenShippedReasoningModels(t *testing.T) {
	c := Embedded()
	shipped := []struct {
		provider, api string
