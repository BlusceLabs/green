...
func TestExtractTarGzRoundTrip(t *testing.T) {
	...
	writeTestTarGz(t, archivePath, map[string]string{
		"green":                 "main-binary",
		"helpers/green-seccomp": "helper-binary",
	})
	...
	data, err := os.ReadFile(filepath.Join(destDir, "green"))
	if err != nil {
		t.Fatalf("ReadFile green: %v", err)
	}
	if string(data) != "main-binary" {
		t.Fatalf("green content = %q", data)
	}
	data, err = os.ReadFile(filepath.Join(destDir, "helpers", "green-seccomp"))
	if err != nil {
		t.Fatalf("ReadFile helpers/green-seccomp: %v", err)
	}
	if string(data) != "helper-binary" {
		t.Fatalf("helpers/green-seccomp content = %q", data)
	}
}
...
func TestExtractZipRoundTrip(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(destDir, "green.exe"))
	if err != nil {
		t.Fatalf("ReadFile green.exe: %v", err)
	}
	if string(data) != "main-binary" {
		t.Fatalf("green.exe content = %q", data)
	}
}
...
func TestExtractZipRejectsSymlinkEntry(t *testing.T) {
	...
	zipWriter := zip.NewWriter(file)
	header := &zip.FileHeader{Name: "green"}
	...
	if _, err := os.Lstat(filepath.Join(destDir, "green")); err == nil {
...
func TestFindByBasenameSearchesRecursively(t *testing.T) {
	...
	wantPath := filepath.Join(nested, "green-seccomp")
	if err := os.WriteFile(wantPath, []byte("helper"), 0o755); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

	found, err := findByBasename(dir, "green-seccomp")
	if err != nil {
