...
func TestDetectInstallMethodStandaloneByDefault(t *testing.T) {
	dir := t.TempDir()
	exePath := filepath.Join(dir, "green")
...
func TestDetectInstallMethodNpmViaMarkerFile(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, ".green-binary-version"), []byte("0.1.0\n"), 0o644); err != nil {
...
func TestDetectInstallMethodNpmViaPackageJSON(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "package.json"), []byte(`{"name":"@BlusceLabs/green"}`), 0o644); err != nil {
