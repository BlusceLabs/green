...
// present, so `green upgrade` refreshes them too instead of leaving them stale.
var (
	windowsOptionalBinaries = []string{"green-windows-command-runner.exe", "green-windows-sandbox-setup.exe"}
	linuxOptionalBinaries   = []string{"green-linux-sandbox", "green-seccomp"}
)
...
func FormatApply(result ApplyResult) string {
	...
	lines := []string{
		fmt.Sprintf("[green] %s (%s -> %s)", result.Message, result.CurrentVersion, result.LatestVersion),
...
func applyStandaloneUpdate(ctx context.Context, result Result, executablePath string) ([]string, error) {
	...
	tempDir, err := os.MkdirTemp("", "green-update-*")
	if err != nil {
		return nil, err
	}
	defer func() {
