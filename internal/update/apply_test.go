...
func TestApplyStandaloneUpdateReplacesBinary(t *testing.T) {
	...
	case "windows":
		...
		optionalName = "green-windows-command-runner.exe"
	case "linux":
		optionalName = "green-seccomp"
	}
	...
	writeTestTarGz(t, archivePath, map[string]string{
		"green":                            "new-binary",
		"green.exe":                        "new-binary-exe",
		"green-seccomp":                    "new-helper",
		"green-windows-command-runner.exe": "new-helper-exe",
	})
...
func TestApplyStandaloneUpdateRejectsChecksumMismatch(t *testing.T) {
	...
	archiveDir := t.TempDir()
	archivePath := filepath.Join(archiveDir, archiveName)
	writeTestTarGz(t, archivePath, map[string]string{"green": "new-binary", "green.exe": "new-binary-exe"})
