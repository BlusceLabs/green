...
const (
	DefaultRepository = "BlusceLabs/green"
...
func Check(ctx context.Context, options Options) (Result, error) {
	...
	endpoint, err := resolveEndpoint(firstNonEmpty(options.Endpoint, os.Getenv("green_UPDATE_RELEASE_URL")), repository)
	if err != nil {
		return Result{}, err
	}
...
func Format(result Result) string {
	if result.UpdateAvailable {
		lines := []string{
			fmt.Sprintf("[green] Update available: %s -> %s", result.CurrentVersion, result.LatestVersion),
		...
		if target := releaseAssetTarget(result.ReleaseAsset); target != "" {
			lines = append(lines, "Download the verified "+target+" release asset, then replace the current green binary.")
		} else {
			lines = append(lines, "Download the verified release asset, then replace the current green binary.")
		}
		return strings.Join(lines, "\n")
	}
	lines := []string{
		fmt.Sprintf("[green] up to date (%s)", result.CurrentVersion),
...
func fetchRelease(ctx context.Context, endpoint string) (release Release, err error) {
	...
	request.Header.Set("User-Agent", "green/update")
...
func expectedAssetCheck(version string, goos string, goarch string) (AssetCheck, error) {
	...
	archiveName := fmt.Sprintf("green-v%s-%s-%s.%s", version, platform, arch, extension)
	return AssetCheck{
		Platform:     platform,
		Arch:         arch,
...
func releasePlatform(goos string) (string, error) {
	...
	default:
		...
		// Termux runs green fine via the npm wrapper -- it just has no
		...
		return "", fmt.Errorf("no published release for %q (release assets: linux, macos, windows). Your build is the current version of record. Upgrade with `npm update -g @bluscelabs/green` to get the latest.", goos)
	}
}
