...
const Marker = "<!-- green-auto-review -->"
...
var defaultCheckSpecs = []checkSpec{
	{Env: "green_REVIEW_DIFF_CHECK", Label: "Diff hygiene", Command: "git diff --check"},
	{Env: "green_REVIEW_TEST", Label: "Tests", Command: "go test ./..."},
	{Env: "green_REVIEW_BUILD", Label: "Build", Command: "go run ./cmd/green-release build"},
	{Env: "green_REVIEW_SMOKE", Label: "Smoke build", Command: "go run ./cmd/green-release smoke"},
}
...
func BuildSummaryInputFromEnv(env map[string]string) SummaryInput {
	return SummaryInput{
		...
		HeadSHA:      firstNonEmpty(env["green_REVIEW_HEAD_SHA"], env["GITHUB_SHA"]),
		Checks:       BuildChecksFromEnv(env),
		ChangedFiles: ParseChangedFiles(env["green_CHANGED_FILES"]),
	}
}
...
func parsePRNumber(env map[string]string) int {
	value := firstNonEmpty(env["green_PR_NUMBER"], strings.Split(env["GITHUB_REF_NAME"], "/")[0])
