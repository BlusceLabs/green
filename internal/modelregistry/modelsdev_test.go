...
func TestApplyModelsDevOverrides(t *testing.T) {
	...
	t.Setenv("green_MODELS_CACHE_PATH", filepath.Join(t.TempDir(), "absent.json"))
...
func TestRefreshModelsDevCacheFetchesAndCaches(t *testing.T) {
	...
	t.Setenv("green_MODELS_CACHE_PATH", cachePath)
	t.Setenv("green_MODELS_URL", server.URL)
	t.Setenv("green_DISABLE_MODELS_FETCH", "")
...
func TestRefreshModelsDevCacheDisabledByEnv(t *testing.T) {
	...
	defer server.Close()
	t.Setenv("green_MODELS_CACHE_PATH", filepath.Join(t.TempDir(), "modelsdev.json"))
	t.Setenv("green_MODELS_URL", server.URL)
	t.Setenv("green_DISABLE_MODELS_FETCH", "1")
	if err := RefreshModelsDevCache(t.Context()); err != nil {
		t.Fatal(err)
	}
}
