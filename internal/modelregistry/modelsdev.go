...
// modelsDevCachePath returns the on-disk cache location. green_MODELS_CACHE_PATH
// overrides it (used by tests and unusual setups).
func modelsDevCachePath() (string, error) {
	if override := strings.TrimSpace(os.Getenv("green_MODELS_CACHE_PATH")); override != "" {
		return override, nil
	}
	...
	return filepath.Join(base, "green", "modelsdev.json"), nil
}
...
// green_DISABLE_MODELS_FETCH disables both the overlay and the fetch.
...
func cachedModelsDevProviders() map[string]map[string]modelsDevModel {
	if !modelsDevEnabled.Load() || strings.TrimSpace(os.Getenv("green_DISABLE_MODELS_FETCH")) != "" {
		return nil
	}
...
// by green_DISABLE_MODELS_FETCH. The URL can be overridden with green_MODELS_URL.
func RefreshModelsDevCache(ctx context.Context) error {
	if strings.TrimSpace(os.Getenv("green_DISABLE_MODELS_FETCH")) != "" {
		return nil
	}
	path, err := modelsDevCachePath()
	if err != nil {
	...
	url := strings.TrimSpace(os.Getenv("green_MODELS_URL"))
	if url == "" {
	...
	request.Header.Set("User-Agent", "green-models-refresh/0.1")
