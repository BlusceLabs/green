...
func TestRenderPromptIncludesRepoSummaryAndRelativePaths(t *testing.T) {
	root, err := filepath.Abs(filepath.Join("testdata", "green"))
	if err != nil {
		t.Fatalf("Abs: %v", err)
	}
	repo := RepoMap{
		...
		Files: []File{
			...
			{Path: filepath.Join(root, "cmd", "green", "main.go")},
		},
	}
...
func TestRenderPromptReturnsEmptyForNonPositiveBudget(t *testing.T) {
	got := RenderPrompt(RepoMap{Root: "green", Files: []File{{Path: "main.go"}}}, 0)
	if got != "" {
		t.Fatalf("RenderPrompt() with green budget=%q want empty", got)
	}
}
