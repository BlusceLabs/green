...
func TestResolveConfigUsesXAIPreset(t *testing.T) {
	r := NewRegistry()
	// Presets are opt-in; with green_OAUTH_ALLOW_PRESETS set and no other env vars
	// the preset supplies everything.
	cfg, flow, err := r.ResolveConfig("xai", map[string]string{"green_OAUTH_ALLOW_PRESETS": "1"})
	if err != nil {
...
// opt into the preset without exporting green_OAUTH_ALLOW_PRESETS: an otherwise-inert
...
// The helper copies the base map (never mutating it) and keeps green_OAUTH_<NAME>_*
...
func TestEnvWithPresetsAllowedPreservesOverrides(t *testing.T) {
	base := map[string]string{"green_OAUTH_XAI_CLIENT_ID": "custom-id"}
	env := envWithPresetsAllowed(base)
	if env["green_OAUTH_ALLOW_PRESETS"] != "1" {
		t.Fatalf("opt-in flag not set: %v", env)
	}
	if _, mutated := base["green_OAUTH_ALLOW_PRESETS"]; mutated {
...
func TestResolveConfigEnvOverridesPreset(t *testing.T) {
	...
	env := map[string]string{
		"green_OAUTH_ALLOW_PRESETS": "1",
		"green_OAUTH_XAI_CLIENT_ID": "custom-id",
		"green_OAUTH_XAI_SCOPES":    "alpha beta",
		"green_OAUTH_XAI_FLOW":      "device",
	}
...
func TestResolveConfigPresetInertWithoutOptIn(t *testing.T) {
	...
	if err == nil {
		t.Fatal("xai must not resolve from the preset unless green_OAUTH_ALLOW_PRESETS is set")
	}
	if !strings.Contains(err.Error(), "green_OAUTH_ALLOW_PRESETS") {
...
func TestResolveConfigHuggingFaceRequiresClientID(t *testing.T) {
	r := NewRegistry()
	_, _, err := r.ResolveConfig("huggingface", map[string]string{"green_OAUTH_ALLOW_PRESETS": "1"})
	if err == nil {
		t.Fatal("huggingface must not resolve from the preset alone (no baked-in client_id)")
	}
	if !strings.Contains(err.Error(), "green_OAUTH_HUGGINGFACE_CLIENT_ID") {
...
func TestResolveConfigHuggingFaceWithEnvClientID(t *testing.T) {
	...
	env := map[string]string{
		"green_OAUTH_ALLOW_PRESETS":         "1",
		"green_OAUTH_HUGGINGFACE_CLIENT_ID": "test-client-id-value",
		"green_OAUTH_HUGGINGFACE_SCOPES":    "openid inference-api",
	}
...
func TestResolveConfigChatGPTPreset(t *testing.T) {
	r := NewRegistry()
	cfg, flow, err := r.ResolveConfig("chatgpt", map[string]string{"green_OAUTH_ALLOW_PRESETS": "1"})
	if err != nil {
