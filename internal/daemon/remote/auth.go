// Package remote adds an OPT-IN, TLS-only network bridge on top of green's local
...
// the default; nothing here activates unless `green daemon serve-remote` is run.
//
...
const (
	EnvToken     = "green_DAEMON_REMOTE_TOKEN"
	EnvTokenFile = "green_DAEMON_REMOTE_TOKEN_FILE"
)
