// Package daemon implements green's long-running daemon/server mode: a local
// control server that supervises a pool of headless `green exec` worker processes
// and routes multiple agent sessions to them over an owner-only local socket.
//
// It reuses green's existing building blocks rather than inventing new ones:
...
//   - internal/cli (exec) : a worker is a `green exec -i/-o stream-json` process.
//
