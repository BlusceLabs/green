...
func TestLinuxHelperRealSandboxSmoke(t *testing.T) {
	if os.Getenv("green_SANDBOX_REAL_SMOKE") != "1" {
		t.Skip("set green_SANDBOX_REAL_SMOKE=1 to run real sandbox smoke tests")
	}
	...
	output, runErr := runLinuxSandboxSmokeCommand(t, engine, CommandSpec{
		...
		Args: []string{"-c", strings.Join([]string{
			...
			"echo tmp > /tmp/green-sandbox-smoke",
			"test \"$(cat /tmp/green-sandbox-smoke)\" = tmp",
	...
	for _, tc := range []struct {
		...
		{
			name:   "outside write",
			script: "if echo leak > /etc/green_sandbox_smoke 2>/dev/null; then echo OUTSIDE_WRITE_SUCCEEDED; exit 42; fi",
