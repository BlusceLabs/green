...
func TestBuildCommandPlanWrapsLinuxHelper(t *testing.T) {
	...
	engine := NewEngine(EngineOptions{
		...
		Backend: Backend{
			...
			Available:  true,
			Executable: "/usr/bin/green-linux-sandbox",
			Platform:   "linux",
	...
	if !plan.Wrapped || plan.Name != "/usr/bin/green-linux-sandbox" || plan.Backend.Name != BackendLinuxBwrap {
...
func TestSandboxExecProfileAllowsDevNullAndTemp(t *testing.T) {
	...
	if out, err := run("echo leak > /etc/green_sandbox_should_fail 2>/dev/null"); err == nil {
...
func TestSeatbeltProfileProtectsMetadataAndDenyOrdering(t *testing.T) {
	profile := PermissionProfile{
		FileSystem: FileSystemPolicy{
			...
			WriteRoots: []WritableRoot{{
				...
				ProtectedMetadataNames: []string{".git", ".green"},
			}},
	...
	for _, want := range []string{
		...
		`(deny file-write* (regex #"^/repo/\.green(/.*)?$"))`,
		denySecretReadRule,
...
func TestLinuxHelperPlanCarriesExtraWriteRoots(t *testing.T) {
	...
	engine := NewEngine(EngineOptions{
		...
		Policy:        DefaultPolicy(),
		Scope:         scope,
		Backend:       Backend{Name: BackendLinuxBwrap, Available: true, Executable: "/usr/bin/green-linux-sandbox"},
	})
...
func TestLinuxHelperPlanPreservesRealExtraRootCwd(t *testing.T) {
	...
	bwrapArgs, err := BuildLinuxSandboxBwrapArgs(LinuxSandboxBwrapOptions{Config: config, HelperPath: "/usr/bin/green-linux-sandbox"})
	if err != nil {
