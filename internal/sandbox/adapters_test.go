...
func TestSelectBackendChoosesPlatformAdapterWithFallback(t *testing.T) {
	t.Run("linux helper available", func(t *testing.T) {
		...
		if backend.Name != BackendLinuxBwrap || !backend.Available || backend.Executable != "/usr/bin/green-linux-sandbox" {
		...
		backend := SelectBackend(BackendOptions{
			...
			LookupExecutable: func(name string) (string, error) {
				if name == WindowsSandboxCommandRunnerName {
					return `C:\green\green-windows-command-runner.exe`, nil
				}
				if name == WindowsSandboxSetupName {
					return `C:\green\green-windows-sandbox-setup.exe`, nil
				}
				return "", errors.New("missing")
			},
		})
		if backend.Name != BackendWindowsRestrictedToken || !backend.Available || backend.Executable != `C:\green\green-windows-command-runner.exe` {
	...
	t.Run("windows missing helper exes self-dispatch", func(t *testing.T) {
		...
		// backend self-dispatches via the running green binary, so it is AVAILABLE
		// rather than failing every command. Pin os.Executable for determinism.
		restore := osExecutable
		osExecutable = func() (string, error) { return `C:\green\green.exe`, nil }
		...
		if backend.Executable != `C:\green\green.exe` {
