...
// EnvSandboxed marks a process that green has already wrapped in a sandbox: every
// wrapped command carries green_SANDBOXED=1 in its environment. When such a
...
// default.
const EnvSandboxed = "green_SANDBOXED"
...
// ambient flag (a stray or hand-exported green_SANDBOXED=1 with no backend marker
// no longer forces an unsandboxed pass-through).
const EnvSandboxBackend = "green_SANDBOX_BACKEND"

// IsAlreadySandboxed reports whether the current process is already running
// inside a green-created sandbox. It requires BOTH correlated markers that
// sandboxEnvironment sets together — EnvSandboxed == "1" AND a non-empty
// EnvSandboxBackend — so a single user-set/inherited green_SANDBOXED=1 cannot by
// itself disable wrapping. green sets both only on genuinely wrapped commands;
...
const (
	...
	// elevated `green sandbox setup` has not run: commands still wrap through the
