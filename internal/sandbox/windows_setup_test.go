...
func TestWindowsSandboxSetupPathForRunner(t *testing.T) {
	got := WindowsSandboxSetupPathForRunner(filepath.Join("C:", "green", WindowsSandboxCommandRunnerName))
	want := filepath.Join("C:", "green", WindowsSandboxSetupName)
	if got != want {
...
func TestWindowsSandboxSetupConfigFromCommandPreservesProfileInputs(t *testing.T) {
	command := WindowsSandboxCommandConfig{
		...
		Env:     map[string]string{"green_SANDBOXED": "1"},
