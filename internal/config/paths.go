...
func DefaultResolveOptions(workspaceRoot string) (ResolveOptions, error) {
	...
	projectConfigPath, err := existingConfigFile(filepath.Join(workspaceRoot, ".green", "config.json"))
	if err != nil {
		return ResolveOptions{}, err
	}
	...
	return ResolveOptions{
		...
		ProviderCommand:   strings.TrimSpace(os.Getenv("green_PROVIDER_COMMAND")),
	}, nil
}
...
func DefaultUserConfigPath() (string, error) {
	...
	return filepath.Join(userConfigDir, "green", "config.json"), nil
}

// UserConfigDir returns the base directory green stores per-user config under.
// It mirrors os.UserConfigDir everywhere except macOS: there Go defaults to
// ~/Library/Application Support, but green deliberately uses ~/.config (XDG-style,
