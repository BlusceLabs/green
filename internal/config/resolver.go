...
// openai-/anthropic-compatible endpoints — green cannot guess a gateway's model).
...
// (green config, green exec) still fail with the actionable message.
...
func ResolveMCP(options ResolveOptions) (MCPConfig, error) {
	// Seed green's built-in default MCP servers (e.g. keyless Firecrawl for free,
	...
	// ./.green/config.json cannot register (and spawn) MCP servers. Fail-closed:
...
func mergeProjectConfig(dst *FileConfig, src FileConfig) error {
	...
	// config: a cloned repo's .green/config.json must not be able to grant
...
// every command that resolves config up front (green config, bare green setup).
...
const ActiveProviderEnv = "green_PROVIDER"
...
const MaxTurnsEnv = "green_MAX_TURNS"
...
func normalizeProvidersWithOptions(providers []ProviderProfile, activeName string, options normalizeOptions, envMaps ...map[string]string) ([]ProviderProfile, ProviderProfile, error) {
	...
	if active.Model == "" {
		return nil, ProviderProfile{}, &setupFixableError{
			err:      providerError(active, "provider %s requires model — add \"model\" to its entry in config.json, or re-run: green setup <catalog-id> --model <model>", active.Name),
