...
// DefaultMCPServers returns the MCP servers green ships ENABLED by default so web
...
// `green mcp disable <name>` (which writes `"disabled": true`).
//
...
// use. green only calls it over the network, so Firecrawl's license never reaches
// into green's own code.
...
// IsDefaultMCPServer reports whether name is one of green's built-in default MCP
...
// IsUnconfiguredDefault reports whether server is one of green's built-in
// defaults that the user never wrote an entry for in their config — i.e. it is
// running with whatever green ships (e.g. keyless Firecrawl, no credentials).
//
...
//     disable/enable toggle like `green mcp enable firecrawl` that leaves the
