...
func TestDefaultResolveOptionsUsesExistingConfigPathsAndProviderCommand(t *testing.T) {
	...
	userPath := filepath.Join(userConfigRoot, "green", "config.json")
	projectPath := filepath.Join(workspaceRoot, ".green", "config.json")
	writeFileAt(t, userPath, "{}")
	writeFileAt(t, projectPath, "{}")
	t.Setenv("green_PROVIDER_COMMAND", "  green-provider  ")
	...
	if options.ProviderCommand != "green-provider" {
		t.Fatalf("ProviderCommand = %q, want green-provider", options.ProviderCommand)
	}
}
...
func TestDefaultResolveOptionsRejectsDirectoryConfigPaths(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(userConfigRoot, "green", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(workspaceRoot, ".green", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
...
func TestDefaultUserConfigPathUsesXDGConfigOnMacOS(t *testing.T) {
	...
	want := filepath.Join(home, ".config", "green", "config.json")
	if path != want {
