...
func TestLoadProviderCommandDoesNotResolveAPIKeyEnvFromProcess(t *testing.T) {
	t.Setenv("green_CMD_API_KEY", "sk-process")
	command := writeCommand(t, commandScript{
		Stdout: `{"name":"cmd","provider":"openai","apiKeyEnv":"green_CMD_API_KEY","model":"gpt-command"}`,
	})
	...
	if provider.APIKeyEnv != "green_CMD_API_KEY" {
