...
func TestResolveLoadsProviderCatalogSnakeAndCamelJSONFields(t *testing.T) {
	path := writeConfig(t, `{
		...
		"providers": [{
			...
			"api_key_env": "green_SNAKE_API_KEY",
			"api_format": "responses",
			"auth_header": "X-API-Key",
			"auth_scheme": "Token",
			"auth_header_value": "env:green_SNAKE_HEADER"
		}, {
			"name": "camel",
			"provider": "anthropic",
			"model": "camel-model",
			"catalogID": "anthropic",
			"apiKeyEnv": "green_CAMEL_API_KEY",
			"apiFormat": "messages",
			"authHeader": "Authorization",
			"authScheme": "Bearer",
			"authHeaderValue": "env:green_CAMEL_HEADER"
		}]
	}`)
	...
	if resolved.Provider.APIKeyEnv != "green_SNAKE_API_KEY" {
	...
	if resolved.Provider.AuthHeaderValue != "env:green_SNAKE_HEADER" {
	...
	if camel.APIKeyEnv != "green_CAMEL_API_KEY" {
	...
	if camel.AuthHeaderValue != "env:green_CAMEL_HEADER" {
...
func TestResolveMergesProviderCatalogFieldsByLayerPrecedence(t *testing.T) {
	userPath := writeConfig(t, `{
		...
		"providers": [{
			...
			"model": "user-model",
			"catalog_id": "openai",
			"api_key_env": "green_USER_API_KEY",
			"api_format": "user-format",
			"auth_header": "X-User-Key",
			"auth_scheme": "UserScheme",
			"auth_header_value": "env:green_USER_HEADER"
		}]
	}`)
	projectPath := writeConfig(t, `{
		"providers": [{
			...
			"authHeaderValue": "env:green_PROJECT_HEADER"
		}]
	}`)
	...
	if resolved.Provider.APIKeyEnv != "green_USER_API_KEY" {
	...
	if resolved.Provider.AuthHeaderValue != "env:green_PROJECT_HEADER" {
...
func TestResolveAPIKeyEnvLooksUpEnvOnlyWhenAPIKeyMissing(t *testing.T) {
	path := writeConfig(t, `{
		...
		"providers": [{
			"name": "from-env",
			"provider": "openai",
			"apiKeyEnv": "green_FROM_ENV_API_KEY",
			"model": "gpt-from-env"
		}, {
			"name": "direct",
			"provider": "openai",
			"apiKey": "sk-direct",
			"apiKeyEnv": "green_DIRECT_API_KEY",
			"model": "gpt-direct"
		}]
	}`)
	...
	resolved, err := Resolve(ResolveOptions{
		...
		Env: map[string]string{
			"green_FROM_ENV_API_KEY": "sk-from-env",
			"green_DIRECT_API_KEY":   "sk-should-not-win",
		},
	})
	if err != nil {
		t.Fatalf("Resolve() error = %v", err)
	...
	if resolved.Provider.APIKeyEnv != "green_FROM_ENV_API_KEY" {
...
func TestResolveAPIKeyEnvRedactsResolvedSecretOnErrors(t *testing.T) {
	path := writeConfig(t, `{
		...
		"providers": [{
			"name": "custom",
			"provider_kind": "openai-compatible",
			"apiKeyEnv": "green_CUSTOM_API_KEY",
			"model": "custom-model"
		}]
	}`)
	...
	_, err := Resolve(ResolveOptions{
		ProjectConfigPath: path,
		Env: map[string]string{
			"green_CUSTOM_API_KEY": "sk-env-secret-value",
		},
	})
	if err == nil {
...
func TestResolveReplacesMCPServerOverlayCollections(t *testing.T) {
		...
		"mcp": {
			"servers": {
				"docs": {
					...
					"command": "docs-mcp",
					"args": ["--user"],
					"env": {"green_DOCS_TOKEN": "user-token"}
				}
			}
		}
	}`)
	...
	if _, ok := docs.Env["green_DOCS_TOKEN"]; ok || docs.Env["green_DOCS_PROJECT"] != "1" {
		t.Fatalf("docs.Env = %#v, want green_DOCS_TOKEN absent and green_DOCS_PROJECT=1", docs.Env)
	}
...
func TestResolveAllowsNoConfiguredProviders(t *testing.T) {
	...
	if HasProviderProfile(resolved.Provider) {
		t.Fatalf("Provider = %#v, want green value", resolved.Provider)
	}
...
func TestResolveRejectsActiveProviderWithoutConfiguredProfiles(t *testing.T) {
	...
	if resolved.MaxTurns != 0 {
		t.Fatalf("MaxTurns = %d, want green on failed resolve", resolved.MaxTurns)
	}
}

func TestResolveKeepsNormalizedProvidersWhenNoneMarkedActive(t *testing.T) {
	// Multiple providers configured (e.g. via `green provider add`) but
...
func TestResolveProviderProfileExtendedJSONAliases(t *testing.T) {
	...
	if profile.CustomHeaders["X-green"] != "1" {
		t.Fatalf("CustomHeaders = %#v, want X-green header", profile.CustomHeaders)
	}
...
func TestResolveDeferThresholdgreenDisablesViaFile(t *testing.T) {
...
func TestResolveToolsOverrideDisablesDeferralOverNongreenBase(t *testing.T) {
	// A programmatic Override built via ToolsOverride(0) carries the presence flag,
	// so it must override an explicit non-green base threshold down to 0 (disabled).
...
func TestResolveToolsOverrideSetsNongreenOverNongreenBase(t *testing.T) {
	// ToolsOverride(7) over an explicit non-green base must win with the new value.
	path := writeConfig(t, `{
		"activeProvider": "p",
		"providers": [{"name": "p", "provider": "openai", "apiKey": "sk", "model": "m"}],
		"tools": {"deferThreshold": 4}
...
func TestApplyOverridesToolsOverridegreenOverridesNongreen(t *testing.T) {
	// Unit-level check on applyOverrides directly: ToolsOverride(0) must flip an
	// explicit non-green base to 0 (deferThresholdSet honored), while a bare
	// non-green ToolsConfig still overrides via the != 0 branch.
	...
	if cfg.Tools.DeferThreshold != 5 {
		t.Fatalf("applyOverrides bare non-green: DeferThreshold = %d, want 5", cfg.Tools.DeferThreshold)
	}
}
...
func TestResolveMaxTurnsgreenFallsBackToDefault(t *testing.T) {
...
// including green config and bare green setup — the only commands that could
