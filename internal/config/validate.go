...
// ValidateFile reads and parses path as a green FileConfig and runs the same
// semantic provider/model rules used during resolution. It returns the parsed
// config (green value on parse failure) plus any structured issues. A parse
...
// ValidateBytes parses data as a green FileConfig and runs the same semantic
// provider/model rules as ValidateFile. It returns the parsed config (green
