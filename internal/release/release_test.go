...
func TestSHA256FileHashesArchiveBytes(t *testing.T) {
	dir := t.TempDir()
	archivePath := filepath.Join(dir, "green-v0.1.0-linux-x64.tar.gz")
	archiveBytes := []byte("green archive bytes")
...
func TestWriteAndVerifyReleaseChecksums(t *testing.T) {
	dir := t.TempDir()
	archiveName := "green-v0.1.0-linux-x64.tar.gz"
	archivePath := filepath.Join(dir, archiveName)
	if err := os.WriteFile(archivePath, []byte("green archive bytes"), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestChecksumParsingRejectsMalformedAndUnsafeNames(t *testing.T) {
	...
	if _, err := FormatSHA256Checksum("abc", "green.tar.gz"); err == nil || !strings.Contains(err.Error(), "64 hexadecimal") {
		t.Fatalf("FormatSHA256Checksum invalid checksum error = %v", err)
	}
	if _, err := ParseSHA256Checksum(strings.Repeat("a", 64) + "  ../green.tar.gz\n"); err == nil || !strings.Contains(err.Error(), "same-directory") {
...
func TestVerifyReleaseChecksumsRequiresMatchingFiles(t *testing.T) {
	...
	strayChecksum := filepath.Join(dir, "green-v0.1.0-macos-arm64.tar.gz.sha256")
	if err := os.WriteFile(strayChecksum, []byte(strings.Repeat("a", 64)+"  green-v0.1.0-macos-arm64.tar.gz\n"), 0o644); err != nil {
...
func TestReleaseArchiveNamesMatchInstallerContracts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			goos:        "linux",
			goarch:      "amd64",
			packageName: "green-v0.1.0-linux-x64",
			archiveName: "green-v0.1.0-linux-x64.tar.gz",
		},
		{
			name:        "macos arm64",
			...
			goos:        "darwin",
			goarch:      "arm64",
			packageName: "green-v0.1.0-macos-arm64",
			archiveName: "green-v0.1.0-macos-arm64.tar.gz",
		},
		{
			name:        "windows amd64",
			version:     "0.1.0",
			goos:        "windows",
			goarch:      "amd64",
			packageName: "green-v0.1.0-windows-x64",
			archiveName: "green-v0.1.0-windows-x64.zip",
		},
	}
...
func TestBuildHelpersMatchScriptContracts(t *testing.T) {
	...
	if got := DefaultBuildOutput(root, "windows"); got != filepath.Join(root, "green.exe") {
		t.Fatalf("DefaultBuildOutput(windows) = %q", got)
	}
	if got := DefaultBuildOutput(root, "linux"); got != filepath.Join(root, "green") {
		t.Fatalf("DefaultBuildOutput(linux) = %q", got)
	}
	if got := WindowsSandboxCommandRunnerArtifactName("windows"); got != "green-windows-command-runner.exe" {
		t.Fatalf("WindowsSandboxCommandRunnerArtifactName(windows) = %q", got)
	}
	if got := WindowsSandboxSetupArtifactName("windows"); got != "green-windows-sandbox-setup.exe" {
		t.Fatalf("WindowsSandboxSetupArtifactName(windows) = %q", got)
	}
	if got := BuildLdflags(version); !strings.Contains(got, "-X github.com/BlusceLabs/green/internal/cli.version=0.1.0") {
...
func TestSmokeRejectsMissingDefaultArtifact(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "build artifact not found: green") {
...
func TestCreateArchivesWithRootPackageFiles(t *testing.T) {
	t.Run("tar gz", func(t *testing.T) {
		stagingDir := packageStagingFixture(t, "green")
		archivePath := filepath.Join(t.TempDir(), "green-v0.1.0-linux-x64.tar.gz")
		...
		for _, want := range []string{"green", "README.md", "bin/green.js", "VERSION"} {
			if !names[want] {
	...
	t.Run("zip", func(t *testing.T) {
		stagingDir := packageStagingFixture(t, "green.exe")
		archivePath := filepath.Join(t.TempDir(), "green-v0.1.0-windows-x64.zip")
		if err := createArchive(stagingDir, archivePath, "windows"); err != nil {
			t.Fatalf("createArchive returned error: %v", err)
		}
		names := zipArchiveNames(t, archivePath)
		for _, want := range []string{"green.exe", "README.md", "bin/green.js", "VERSION"} {
			if !names[want] {
...
func TestCopyPackageFilesStagesLinuxSandboxHelper(t *testing.T) {
	root := t.TempDir()
	staging := t.TempDir()
	artifact := filepath.Join(root, "green")
	helper := filepath.Join(root, "green-linux-sandbox")
	seccomp := filepath.Join(root, "green-seccomp")
	for path, content := range map[string]string{
		...
		filepath.Join(root, "bin", "green.js"): "wrapper",
	} {
		mustWriteFile(t, path, content)
	}
	if err := copyPackageFiles(root, staging, artifact, filepath.Join(staging, "green"), "linux", "0.1.0", map[string]string{
		"green-linux-sandbox": helper,
		"green-seccomp":       seccomp,
	}); err != nil {
		t.Fatalf("copyPackageFiles: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "green-linux-sandbox")); err != nil {
		t.Fatalf("staged helper missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "green-seccomp")); err != nil {
...
func TestCopyPackageFilesStagesWindowsSandboxHelpers(t *testing.T) {
	root := t.TempDir()
	staging := t.TempDir()
	artifact := filepath.Join(root, "green.exe")
	runner := filepath.Join(root, "green-windows-command-runner.exe")
	setup := filepath.Join(root, "green-windows-sandbox-setup.exe")
	for path, content := range map[string]string{
		artifact:                              "green",
		runner:                                "runner",
		setup:                                 "setup",
	...
	if err := copyPackageFiles(root, staging, artifact, filepath.Join(staging, "green.exe"), "windows", "0.1.0", map[string]string{
		"green-windows-command-runner.exe": runner,
		"green-windows-sandbox-setup.exe":  setup,
	}); err != nil {
		t.Fatalf("copyPackageFiles: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "green-windows-command-runner.exe")); err != nil {
		t.Fatalf("staged runner missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "green-windows-sandbox-setup.exe")); err != nil {
