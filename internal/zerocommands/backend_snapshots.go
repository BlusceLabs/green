...
// `green mcp` command, and PR/CI automation. The snapshot strips
...
// is exposed to the TUI render path, the headless `green hooks`
...
// is exposed to the TUI render path, the headless `green plugins`
...
// lane: MCP servers, hooks, and plugins. `green doctor` and the
// headless `green config` command can return one of these to give
...
// as green values so callers that do not have a live registry can
