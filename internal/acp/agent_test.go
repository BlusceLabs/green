...
func (f fakeProvider) StreamCompletion(_ context.Context, _ greenruntime.CompletionRequest) (<-chan greenruntime.StreamEvent, error) {
	ch := make(chan greenruntime.StreamEvent, 4)
	go func() {
		defer close(ch)
		ch <- greenruntime.StreamEvent{Type: greenruntime.StreamEventText, Content: f.text}
...
func testDeps(t *testing.T) Deps {
	...
	return Deps{
		...
		NewProvider: func(config.ProviderProfile) (greenruntime.Provider, error) {
			return fakeProvider{text: "Hello from green"}, nil
		},
		RunAgent: agent.Run,
		...
		Store:                store,
		AgentInfo:            Implementation{Name: "green", Version: "test"},
	}
}
...
func TestACPEndToEndPrompt(t *testing.T) {
	...
	if got := drainText(t, h.updates); !strings.Contains(got, "Hello from green") {
...
func TestACPRunTurnWiresSandboxAndScopedRegistry(t *testing.T) {
	...
	deps.RunAgent = func(_ context.Context, _ string, _ greenruntime.Provider, opts agent.Options) (agent.Result, error) {
		captured = opts
...
func TestACPPromptWarnsWhenTurnPersistenceFails(t *testing.T) {
	...
	got := drainTextUntil(t, h.updates, func(text string) bool {
		return strings.Contains(text, "Hello from green") &&
