...
// translate.go maps green's agent events onto ACP session/update payloads. The
...
// toolKindFor maps a green tool name to the closest ACP ToolKind so editors can
...
// toolCallStart maps an advertised green tool call to the initial ACP "tool_call"
// update (status in_progress — green executes immediately after advertising).
...
// toolCallResult maps a finished green tool result to a "tool_call_update".
...
// planUpdate maps green's plan items to an ACP "plan" update.
...
// planStatusToACP maps green's plan status (pending/in_progress/completed/failed)
