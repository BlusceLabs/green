...
// Deps are the green capabilities the ACP Agent drives. The CLI fills these with
...
// means the editor only hosts the thread — green owns BYOK and telemetry-free
// operation.
type Deps struct {
	ResolveConfig func(workspaceRoot string, overrides config.Overrides) (config.ResolvedConfig, error)
	NewProvider   func(profile config.ProviderProfile) (greenruntime.Provider, error)
	RunAgent      func(ctx context.Context, prompt string, provider greenruntime.Provider, opts agent.Options) (agent.Result, error)
...
func NewAgent(conn *Conn, deps Deps) *Agent {
	...
	conn.Handle(MethodgreenSetModel, a.handlegreenSetModel)
...
func (a *Agent) handleInitialize(_ context.Context, params json.RawMessage) (any, error) {
	...
	return InitializeResult{
		...
		AgentCapabilities: AgentCapabilities{
			// Only advertise what green actually implements: session/load (loadSession)
		...
		AgentInfo: &info,
		// green owns credentials (BYOK) and does not delegate auth to the editor.
...
func (a *Agent) runTurn(ctx context.Context, sess *acpSession, userText string, images []greenruntime.ImageBlock) (string, error) {
...
// requestPermission forwards a green permission prompt to the client as an ACP
...
func (a *Agent) handlegreenSetModel(_ context.Context, params json.RawMessage) (any, error) {
	var p greenSetModelParams
	if err := json.Unmarshal(params, &p); err != nil {
		return nil, RPCError(codeInvalidParams, "invalid _green/set_model params")
	}
	sess := a.session(p.SessionID)
	if sess == nil {
		return nil, RPCError(codeInvalidParams, "unknown session: "+p.SessionID)
	...
	return greenSetModelResult{Model: p.Model}, nil
}
...
func (a *Agent) warnPersistence(note *notifier, action string, message string, err error) {
	...
	log.Printf("green acp: failed to %s for session %s: %v", action, sessionID, err)
	if note != nil {
		note.text("\n\n[green warning] " + message + "\n")
	}
}
...
func promptImages(blocks []ContentBlock) []greenruntime.ImageBlock {
	var images []greenruntime.ImageBlock
	for _, blk := range blocks {
		...
		images = append(images, greenruntime.ImageBlock{MediaType: blk.MimeType, Data: data})
	}
	return images
}
