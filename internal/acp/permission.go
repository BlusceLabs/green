...
// permission.go maps green's permission prompt model onto ACP's
// session/request_permission request/option/outcome model. The option id on the
// wire carries the green decision action verbatim, so mapping the client's
// selection back to a green decision is exact and lossless.

// buildPermissionOptions turns the decisions green offers for a tool call into ACP
// PermissionOptions. Only the actions green actually presented (AvailableDecisions)
// are surfaced; the optionId is the green action string for a clean round-trip.
func buildPermissionOptions(req agent.PermissionRequest) []PermissionOption {
	actions := req.AvailableDecisions
	if len(actions) == 0 {
		// Sensible default if green didn't enumerate: allow once / reject.
...
// optionKindFor maps a green decision action to an ACP PermissionOptionKind and a
...
// decisionFromOutcome maps the client's permission outcome back to a green
// decision. A cancelled outcome cancels the run; a selected option id is the green
...
// session/request_permission request from a green permission request.
