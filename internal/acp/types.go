...
// ProtocolVersion is the ACP protocol version green speaks. Wire compatibility is
...
const (
	...
	// Vendor-prefixed green extensions (clients that don't support them ignore the
	// method and degrade cleanly, per the spec's _-prefixed convention).
	MethodgreenSetModel = "_green/set_model"
)
...
// ContentBlock is the polymorphic content type. green emits "text" (and "image"
...
// directions since the field names do not collide across the variants green uses.
...
// McpServer mirrors the editor-provided MCP server entry. green owns its own MCP
// configuration (BYOK), so these are accepted for spec compliance; green's
...
// ToolCallContent is a tool call's rendered output. green emits "content" (a
// text/image block) and "diff" (a file change); "terminal" is part of the spec
// but unused because green executes locally.
...
// ---- vendor: _green/set_model ----

type greenSetModelParams struct {
	SessionID string `json:"sessionId"`
	Model     string `json:"model"`
}

type greenSetModelResult struct {
	Model string `json:"model"`
}

// configIDModel is the SessionConfigOption id green uses to expose model choice
