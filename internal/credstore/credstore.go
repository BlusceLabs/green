...
// headless/CI machines, so it is opt-in via green_CRED_STORAGE=keyring). A plaintext
// file is available only as an explicit green_CRED_STORAGE=file opt-out. OAuth tokens
...
import (
	...
	"github.com/BlusceLabs/green/internal/securefile"
)
...
type Options struct {
	...
	// (plaintext opt-out). When empty it falls back to green_CRED_STORAGE, then auto:
	...
	// users can still opt into the keyring with green_CRED_STORAGE=keyring.
	Storage string
...
func New(options Options) (*Store, error) {
	...
	if storage == "" {
		storage = strings.TrimSpace(env("green_CRED_STORAGE"))
	}
	if storage == "" {
		...
		// encrypted file; keyring stays available via an explicit green_CRED_STORAGE.
