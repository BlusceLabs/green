...
func TestBuildOrdersAndClassifiesWorkspaceSeed(t *testing.T) {
	...
	if got.CWD != filepath.Clean(filepath.Join("tmp", "green")) {
	...
	if want := []string{"AGENTS.md", "green.md"}; !reflect.DeepEqual(got.MemoryFiles, want) {
		t.Fatalf("MemoryFiles=%v want %v", got.MemoryFiles, want)
	}
}

func TestBuildKeepsPathsRelativeToCWD(t *testing.T) {
	root := filepath.Join(t.TempDir(), "green")
	dirty := false
	got := Build(Input{
		CWD:       root,
		GitBranch: "main",
	...
	if !strings.Contains(rendered, "cwd: green") {
...
func TestBuildFromWorkspaceUsesWorkspaceIndexWithoutGit(t *testing.T) {
	root := t.TempDir()
	writeSeedFile(t, root, "go.mod", "module example.test/green\n")
	writeSeedFile(t, root, "cmd/green/main.go", "package main\n")
