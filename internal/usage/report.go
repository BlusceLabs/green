...
// Older events without those fields decode to green and price exactly as before.
...
// keys can never drift. Cache and reasoning counts are written only when non-green,
...
func EventUsagePayload(u greenruntime.Usage) map[string]any {
...
// Report is the aggregated usage view rendered by `green usage report`. Cost is a
...
func BuildReport(events []sessions.Event, meta []sessions.Metadata, registry *modelregistry.Registry, netLOC int) (Report, error) {
	...
	for _, event := range events {
		...
		cost, err := modelregistry.CalculateCost(model, greenruntime.Usage{
