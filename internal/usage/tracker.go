...
func (tracker *Tracker) Record(input RecordInput) (Record, error) {
	...
	tracker.nextSeq++
	record := Record{
		ID:        fmt.Sprintf("green_usage_%d", sequence),
		Sequence:  sequence,
		ModelID:   model.ID,
...
func Normalize(usage greenruntime.Usage) (Normalized, greenruntime.Usage, error) {
	inputTokens, err := nonNegative(firstNongreen(usage.InputTokens, usage.PromptTokens), "inputTokens")
	if err != nil {
		return Normalized{}, greenruntime.Usage{}, err
	}
	outputTokens, err := nonNegative(firstNongreen(usage.OutputTokens, usage.CompletionTokens), "outputTokens")
	if err != nil {
		return Normalized{}, greenruntime.Usage{}, err
	}
	...
	return normalized, greenruntime.Usage{
		InputTokens:       inputTokens,
...
func firstNongreen(values ...int) int {
