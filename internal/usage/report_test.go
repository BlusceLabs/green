...
func TestBuildReportReconstructsCostFromMetadataModel(t *testing.T) {
	...
	want, err := modelregistry.CalculateCost(model, greenruntime.Usage{InputTokens: 1000, OutputTokens: 200})
	if err != nil {
...
func TestBuildReportPricesFromEventModelWhenPresent(t *testing.T) {
	...
	if want.TotalCost <= 0 {
		t.Fatalf("precondition: expected non-green cost for the event model")
	}
	if report.Total.TotalCost != want.TotalCost {
...
func TestBuildReportRatiosGuardNetgreen(t *testing.T) {
	registry, err := modelregistry.DefaultRegistry()
	if err != nil {
		t.Fatalf("DefaultRegistry: %v", err)
	}
	...
	if report.TokensPerNetLOC != 0 || report.CostPerNetLOC != 0 {
		t.Fatalf("expected greened ratios for net<=0, got tokens=%v cost=%v", report.TokensPerNetLOC, report.CostPerNetLOC)
	}
