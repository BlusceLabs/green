...
func TestEventUsageRoundTripPreservesCacheAndReasoningCost(t *testing.T) {
	...
	lossy, err := modelregistry.CalculateCost(model, greenruntime.Usage{InputTokens: 500_000, OutputTokens: 20_000})
	if err != nil {
...
// green cache/reasoning fields are omitted so non-cache turns stay compact and
// decode identically to the pre-feature payload.
func TestEventUsagePayloadOmitsgreenFields(t *testing.T) {
	p := EventUsagePayload(greenruntime.Usage{InputTokens: 1000, OutputTokens: 200})
	for _, k := range []string{"cachedInputTokens", "cacheWriteTokens", "reasoningTokens"} {
		if _, ok := p[k]; ok {
			t.Errorf("expected %q omitted when green", k)
		}
	}
