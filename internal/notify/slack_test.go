...
func TestWebhookSinkPostsExpectedJSON(t *testing.T) {
	...
	sink.Emit(Completion, "green: ready")
	...
	case r := <-got:
		...
		if r.body.Message != "green: ready" {
			t.Fatalf("payload message = %q, want %q", r.body.Message, "green: ready")
		}
...
func TestWebhookSinkIncludesSummaryAndLinks(t *testing.T) {
	...
	sink.Emit(AwaitingInput, "green: needs input")
...
func TestWebhookSinkRedactsSecretsInPayloadAndLog(t *testing.T) {
	const token = "xoxb-EXAMPLE-EXAMPLETOKEN"
...
func TestWebhookSinkRedactsWebhookURLInLog(t *testing.T) {
	...
	logf, getLog := captureLog()
	const secretURL = "http://127.0.0.1:0/services/T000/B000/xoxb-EXAMPLE-EXAMPLETOKEN"
	sink := NewWebhookSink(WebhookConfig{
		URL:    secretURL,
		Logf:   logf,
		Client: &http.Client{Timeout: 200 * time.Millisecond},
	...
	sink.Emit(Completion, "hi")
	if log := getLog(); strings.Contains(log, "xoxb-EXAMPLE-EXAMPLETOKEN") {
