...
func TestNotifyWritesAndRespectsPolicy(t *testing.T) {
	...
	n.Notify(Completion, "green: ready")
	if buf.String() != "\x07\x1b]9;green: ready\x07" {
...
func TestDefaultMessage(t *testing.T) {
	if DefaultMessage(Completion) != "green: ready" {
		t.Fatal("completion message")
	}
	if DefaultMessage(AwaitingInput) != "green: needs input" {
