...
func initGitBaseline(ctx context.Context, workspace string) error {
	commands := [][]string{
		{"init"},
		{"config", "user.name", "green Eval"},
		{"config", "user.email", "green-eval@example.invalid"},
		{"add", "."},
		{"commit", "-m", "baseline"},
	}
	for _, args := range commands {
		...
		cmd.Env = append(os.Environ(),
			"GIT_AUTHOR_NAME=green Eval",
			"GIT_AUTHOR_EMAIL=green-eval@example.invalid",
			"GIT_COMMITTER_NAME=green Eval",
			"GIT_COMMITTER_EMAIL=green-eval@example.invalid",
		)
