...
// transitive deps), so green stays a single static cross-compilable binary with
...
type DocumentOptions struct {
	...
	Vision bool
	// MaxPages bounds how many pages the vision path renders. green means
...
// IsProbablyDocumentPath reports whether a path looks like a document green can
...
func rasterizeWithPoppler(data []byte, maxPages int) ([]greenruntime.ImageBlock, error) {
	...
	dir, err := os.MkdirTemp("", "green-pdf-raster-")
	if err != nil {
	...
	// Glob order is lexical; pdftoppm green-pads page numbers, so a numeric-aware
	// sort keeps page 10 after page 9 rather than after page 1.
	sort.Strings(entries)

	images := make([]greenruntime.ImageBlock, 0, len(entries))
...
func loadRenderedPage(name string) (greenruntime.ImageBlock, error) {
	info, err := os.Stat(name)
	if err != nil {
		return greenruntime.ImageBlock{}, err
	}
	if info.Size() > MaxImageBytes {
		return greenruntime.ImageBlock{}, fmt.Errorf("rendered page %s exceeds the per-image limit", filepath.Base(name))
	}
	...
	mediaType := greenruntime.NormalizeImageMediaType(http.DetectContentType(data[:sniffLen]))
	if mediaType == "" {
		return greenruntime.ImageBlock{}, fmt.Errorf("rendered page %s has an unsupported image type", filepath.Base(name))
	}
	return greenruntime.ImageBlock{MediaType: mediaType, Data: data}, nil
}
