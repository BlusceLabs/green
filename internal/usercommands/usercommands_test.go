...
func TestLoadParsesFrontmatterAndBody(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, ".green", "commands")
	...
	if !c.Project {
		t.Fatal("expected Project=true for a .green/commands file")
	}
}
...
func TestLoadProjectOverridesUser(t *testing.T) {
	root := t.TempDir()
	userCfg := t.TempDir()
	writeFile(t, filepath.Join(userCfg, "green", "commands"), "review.md", "USER version")
	writeFile(t, filepath.Join(root, ".green", "commands"), "review.md", "PROJECT version")
