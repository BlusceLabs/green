...
func TestModelsDevProviderIDMapsgreenAliases(t *testing.T) {
	...
	for greenID, want := range tests {
		provider, ok := providercatalog.Get(greenID)
		if !ok {
			t.Fatalf("provider %q missing from catalog", greenID)
		}
		if got := ModelsDevProviderID(provider); got != want {
			t.Fatalf("ModelsDevProviderID(%q) = %q, want %q", greenID, got, want)
		}
	}
}
